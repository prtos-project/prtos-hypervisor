/*
 * Mixed-OS Demo - FreeRTOS Partition (Partition 1) [LoongArch64, para-virt]
 *
...
/* External references (from freertos_para_virt_loongarch64) */
extern void _prtos_trap_dispatch(void);
extern unsigned long g_pct_addr;

/* PCT arch trap_entry offset for loongarch64 */
...
static void verification_task(void *p)
{
    ...
    if (control_loop_count > 100 && sample_count > 10) {
...
int main(void)
{
    ...
    uart_puts("[RTOS] Mixed-OS Demo - Partition 1 (LoongArch64)\n");
    uart_puts("========================================\n");
