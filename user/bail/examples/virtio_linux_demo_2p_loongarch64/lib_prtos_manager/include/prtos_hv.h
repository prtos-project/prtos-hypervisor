 ...
 * FILE: prtos_hv.h
 *
 * PRTOS Hypervisor Interface for Linux guests under RISC-V 64 hw-virt.
 *
 * On RISC-V, ecall from VU-mode (userspace) traps to VS-mode (kernel)
 * as a normal syscall, not to the hypervisor. Therefore the hypercall
 * interface is stubbed out in userspace. The virtio demo works in
 * polling mode (shared memory only, no IPVI doorbells).
 */
...
/* ============================================================================
 * Shared Memory Hypercall Mailbox (updated for riscv64 addresses)
 * ============================================================================ */

#define HC_MAILBOX_SHM_BASE     0x98500000UL
#define HC_MAILBOX_SHM_SIZE     0x00040000UL
#define HC_MAILBOX_OFFSET       (HC_MAILBOX_SHM_SIZE - 0x1000)
#define HC_MAILBOX_GPA          (HC_MAILBOX_SHM_BASE + HC_MAILBOX_OFFSET)

/* ============================================================================
 * Hypervisor Call Interface
 *
 * RISC-V stub: ecall from VU-mode traps to VS-mode (kernel syscall),
 * not to the hypervisor. Return -1.
 * ============================================================================ */
