#!/usr/bin/env python3
"""Test Guest console access via TCP bridge through System partition (AArch64).

On AArch64, the Guest partition has no direct UART. Instead:
...
if not os.path.exists("u-boot/u-boot.bin"):
    ...
    if ret.returncode != 0 or not os.path.exists("u-boot/u-boot.bin"):
        print("[FAIL] u-boot/u-boot.bin build failed")
        sys.exit(1)
