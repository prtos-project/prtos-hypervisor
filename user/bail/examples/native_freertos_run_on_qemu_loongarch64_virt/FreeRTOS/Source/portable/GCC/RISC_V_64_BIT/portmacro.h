 ...
 *
 * portmacro.h for LoongArch64 (PLV0 / Guest mode via LVZ)
 */
...
/* portYIELD: trigger a software interrupt via ESTAT.IS[0] (SWI0).
 * Write bit 0 of CSR.ESTAT to raise a software interrupt. */
#define portYIELD()  do {									\
	unsigned long __val;									\
	__asm volatile ( "csrrd %0, 0x5" : "=r"(__val) );		\
	__val |= (1UL << 0);									\
	__asm volatile ( "csrwr %0, 0x5" :: "r"(__val) );		\
...
/* LoongArch64 CSR.CRMD: IE is bit 2.
 * Disable/enable global interrupts via CRMD.IE. */
#define portDISABLE_INTERRUPTS()  do {								\
	unsigned long __crmd;											\
	__asm volatile ( "csrrd %0, 0x0" : "=r"(__crmd) );				\
	__crmd &= ~(1UL << 2);											\
	__asm volatile ( "csrwr %0, 0x0" :: "r"(__crmd) );				\
	__asm volatile ( "dbar 0" ::: "memory" );						\
} while(0)
...
/* Critical section macros. LoongArch64 does not have priority masking like ARM GIC,
 ...
 * concepts. On LoongArch64 these are not used. */
