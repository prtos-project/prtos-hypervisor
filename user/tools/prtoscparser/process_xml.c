...
static struct attr_xml mapped_at_mem_area_attr = {BAD_CAST "mapped_at", virt_addr_mem_area_attr_handle};
...
static struct prtos_conf_hm_slot *hm_tablePtr = 0;
...
static void action_hm_event_attr_handle(xmlNodePtr node, const xmlChar *val) {
    hm_tablePtr[hm_slot_number].action = to_hm_action((char *)val, node->line);
}
...
static void log_hm_event_attr_handle(xmlNodePtr node, const xmlChar *val) {
    hm_tablePtr[hm_slot_number].log = to_yes_no_true_false((char *)val, node->line);
}
...
static void event_hm_node_handle0(xmlNodePtr node) {
    ...
    if (count_partition < 0)
        hm_hpv_is_action_permitted_on_event(hm_slot_number, hm_tablePtr[hm_slot_number].action, hm_slot_line);
    else
        hm_part_is_action_permitted_on_event(hm_slot_number, hm_tablePtr[hm_slot_number].action, hm_slot_line);
}
...
static void slot_node_handle0(xmlNodePtr node) {
    prtos_conf_sched_cyclic_plan_table[C_CYCLICPLAN].num_of_slotss++;
...
static void ext_sync_cyclic_plan_attr_handle(xmlNodePtr node, const xmlChar *val) {
    prtos_conf_sched_cyclic_plan_table[C_CYCLICPLAN].ext_sync = to_u32((char *)val, 10);
    prtos_conf_sched_cyclic_plan_table_line_number[C_CYCLICPLAN].ext_sync = node->line;
}

static struct attr_xml ext_sync_cyclic_plan_attr = {BAD_CAST "ext_sync", ext_sync_cyclic_plan_attr_handle};
#endif
...
#ifdef CONFIG_PLAN_EXTSYNC
    prtos_conf_sched_cyclic_plan_table[C_CYCLICPLAN].ext_sync = -1;
#endif
...
static struct node_xml plan_N = {BAD_CAST "Plan",
                                 plan_node_handle0,
                                 0,
                                 plan_node_hangle2,
                                 (struct attr_xml *[]){&name_cyclic_plan_attr, &id_cyclic_plan_attr, &major_frame_cyclic_plan_attr,
#ifdef CONFIG_PLAN_EXTSYNC
                                                       &ext_sync_cyclic_plan_attr,
#endif
                                                       0},
                                 (struct node_xml *[]){&slot_node, 0}};

static void cyclicplan_node_handle0(xmlNodePtr node) {
    prtos_conf.hpv.cpu_table[C_CPU].sched_cyclic_plans_offset = prtos_conf.num_of_sched_cyclic_plans;
    prtos_conf.hpv.cpu_table[C_CPU].sched_policy = CYCLIC_SCHED;
}

static struct node_xml cyclic_plan_node = {BAD_CAST "CyclicPlanTable", cyclicplan_node_handle0, 0, 0, 0, (struct node_xml *[]){&plan_N, 0}};
...
static struct node_xml memory_block_device_node = {BAD_CAST "memory_block",
...
static void dst_id_ipvi_attr_handle(xmlNodePtr node, const xmlChar *val) {
    void _CB(int line, char *val) {
        ...
        if (e >= src_ipvi->num_of_dsts) {
            ...
            dst_ipvi->id = id;
            dst_ipvi_line_number->id = node->line;
        } else {
            line_error(node->line, "Duplicated ipvi partition destination");
        }
    }
    process_id_list((char *)val, _CB, node->line);
}
...
static void comm_port_node_handle0(xmlNodePtr node) {
    ...
    prtos_conf_part_table[C_PARTITION].noPorts++;
...
static void port_table_node_handle0(xmlNodePtr node) {
    prtos_conf_part_table[C_PARTITION].comm_ports_offset = prtos_conf.num_of_comm_ports;
}
...
static void id_partition_attr_handle(xmlNodePtr node, const xmlChar *val) {
    ...
    prtos_conf_part_table[C_PARTITION].id = d;
    prtos_conf_part_table_line_number[C_PARTITION].id = node->line;
}
...
static void name_partition_attr_handle(xmlNodePtr node, const xmlChar *val) {
    ...
    prtos_conf_part_table[C_PARTITION].name_offset = add_string((char *)val);
    prtos_conf_part_table_line_number[C_PARTITION].name = node->line;
}
...
static void console_partition_attr_handle(xmlNodePtr node, const xmlChar *val) {
    prtos_conf_part_table[C_PARTITION].console_device.id = add_dev_name((char *)val);
    prtos_conf_part_table_line_number[C_PARTITION].console_device = node->line;
}
...
static void num_of_vcpus_partition_attr_handle(xmlNodePtr node, const xmlChar *val) {
    int d = to_u32((char *)val, 10);
    prtos_conf_part_table[C_PARTITION].num_of_vcpus = d;
    prtos_conf_part_table_line_number[C_PARTITION].num_of_vcpus = node->line;
    if (prtos_conf_part_table[C_PARTITION].num_of_vcpus > CONFIG_MAX_NO_VCPUS)
        line_error(node->line, "too many virtual Cpus (%d) allocated to the partition", d);
}

static struct attr_xml num_of_vcpus_partition_attr = {BAD_CAST "num_of_vcpus", num_of_vcpus_partition_attr_handle};

static void flags_partition_attr_handle(xmlNodePtr node, const xmlChar *val) {
    prtos_conf_part_table[C_PARTITION].flags = to_partition_flags((char *)val, node->line);
    prtos_conf_part_table_line_number[C_PARTITION].flags = node->line;
}
...
static void partition_node_handle0(xmlNodePtr node) {
    ...
    DO_REALLOC(prtos_conf_part_table, prtos_conf.num_of_partitions * sizeof(struct prtos_conf_part));
    DO_REALLOC(prtos_conf_part_table_line_number, prtos_conf.num_of_partitions * sizeof(struct prtos_conf_part_line_number));
    memset(&prtos_conf_part_table[C_PARTITION], 0, sizeof(struct prtos_conf_part));
    memset(&prtos_conf_part_table_line_number[C_PARTITION], 0, sizeof(struct prtos_conf_part_line_number));
    prtos_conf_part_table[C_PARTITION].console_device.id = PRTOS_DEV_INVALID_ID;
    prtos_conf_part_table[C_PARTITION].physical_memory_areas_offset = prtos_conf.num_of_physical_memory_areas;
    num_of_mem_area_ptr = &prtos_conf_part_table[C_PARTITION].num_of_physical_memory_areas;
    hm_tablePtr = prtos_conf_part_table[C_PARTITION].hm_table;
    trace_ptr = &prtos_conf_part_table[C_PARTITION].trace;
    trace_line_number_ptr = &prtos_conf_part_table_line_number[C_PARTITION].trace;
    trace_ptr->dev.id = PRTOS_DEV_INVALID_ID;
    setup_default_part_hm_actions(prtos_conf_part_table[C_PARTITION].hm_table);
}
...
static struct node_xml part_table_node = {BAD_CAST "PartitionTable", 0, 0, 0, 0, (struct node_xml *[]){&partition_node, 0}};
...
static void prtos_hypervisor_node_handle0(xmlNodePtr node) {
    ...
    hm_tablePtr = prtos_conf.hpv.hm_table;
...
static struct node_xml system_description_node = {
    ...
    (struct node_xml *[]){&hw_description_node, &prtos_hypervisor_node, &resident_sw_node, &part_table_node, &channels_node, 0}};
