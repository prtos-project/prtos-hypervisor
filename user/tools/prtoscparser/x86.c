...
static void num_of_ports_range_attr_handle(xmlNodePtr node, const xmlChar *val) {
    ...
    prtos_conf_part_table[C_PARTITION].num_of_io_ports += num_of_io_ports;
}
...
static void io_port_range_node_handle1(xmlNodePtr node) {
    ...
    for (e = 0; e < noRsvIoPorts; e++) {
        ...
        if (!((d < a) || (c >= b)))
            line_error(node->line, "io-ports [0x%lx:%d] reserved for prtos ([0x%lx:%d])", io_port_base, num_of_io_ports, rsvIoPorts[e].base,
                       rsvIoPorts[e].offset);
    }
...
static void io_ports_node_handle0(xmlNodePtr node) {
    prtos_s32_t e;
    prtos_conf_part_table[C_PARTITION].io_ports_offset = prtos_conf.num_of_io_ports;
...
#ifdef CONFIG_MPU
void arch_mpu_rsv_mem(FILE *out_file) {}
#endif
