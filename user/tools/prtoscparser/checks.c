...
int check_phys_mem_area(int memArea) {
    extern prtos_s32_t noRsvPhysPages;
    prtos_address_t s0, e0, s1, e1;
    int e, found;

    s0 = prtos_conf_mem_area_table[memArea].start_addr;
    e0 = s0 + prtos_conf_mem_area_table[memArea].size - 1;

    if (s0 & (PAGE_SIZE - 1))
        line_error(prtos_conf_mem_area_table_line_number[memArea].start_addr, "memory area start address (0x%x) shall be aligned to 0x%x", s0, PAGE_SIZE);
    if (prtos_conf_mem_area_table[memArea].size & (PAGE_SIZE - 1))
        line_error(prtos_conf_mem_area_table[memArea].size, "memory area size (%d) is not multiple of %d", prtos_conf_mem_area_table[memArea].size, PAGE_SIZE);

    for (e = 0; e < noRsvPhysPages; e++) {
        s1 = rsv_phys_pages[e].address;
        e1 = s1 + (rsv_phys_pages[e].noPag * PAGE_SIZE) - 1;
        if (!((e1 < s0) || (s1 >= e0)))
            line_error(prtos_conf_mem_area_table_line_number[memArea].line, "memory area [0x%x - 0x%x] overlaps a memory area [0x%x - 0x%x] reserved for PRTOS",
                       s0, e0, s1, e1);
    }
    ...
    if (found < 0) line_error(prtos_conf_mem_area_table_line_number[memArea].line, "memory area [0x%x - 0x%x] is not covered by any memory region", s0, e0);
    ...
    for (e = 0; e < prtos_conf.num_of_physical_memory_areas - 1; e++) {
        ...
        if ((prtos_conf_mem_area_table[e].flags & prtos_conf_mem_area_table[memArea].flags & PRTOS_MEM_AREA_SHARED) == PRTOS_MEM_AREA_SHARED)
            // if (prtos_conf_mem_area_table_line_number[memArea].partition_id!=prtos_conf_mem_area_table_line_number[e].partition_id)
            continue;
        line_error(prtos_conf_mem_area_table_line_number[memArea].line, "memory area [0x%x - 0x%x] overlaps [0x%x - 0x%x] (line %d)", s0, e0, s1, e1,
...
void check_mem_area_per_partition(void) {
    ...
    for (i = 0; i < prtos_conf_part_table[C_PARTITION].num_of_physical_memory_areas - 1; i++) {
        offset = prtos_conf_part_table[C_PARTITION].physical_memory_areas_offset;
        s0 = prtos_conf_mem_area_table[i + offset].mapped_at;
        e0 = s0 + prtos_conf_mem_area_table[i + offset].size - 1;
        for (j = i + 1; j < prtos_conf_part_table[C_PARTITION].num_of_physical_memory_areas; j++) {
...
void check_hw_irq(int line, int line_number) {
    ...
    for (e = 0; e < noRsvHwIrqs; e++)
        if (line == rsvHwIrqs[e]) line_error(line_number, "hw interrupt line %d reserved for PRTOS", rsvHwIrqs[e]);
}

void check_port_name(int port, int partition) {
    int e, offset;
    offset = prtos_conf_part_table[partition].comm_ports_offset;
    for (e = 0; e < prtos_conf_part_table[partition].noPorts - 1; e++)
...
void check_sched_cyclic_plan(struct prtos_conf_sched_cyclic_plan *plan, struct prtos_conf_sched_cyclic_plan_line_number *plan_line_number) {
    ...
    for (t = 0, e = 0; e < plan->num_of_slotss; e++) {
...
void check_cyclic_plan_partition_id(void) {
    ...
    for (j = 0; j < prtos_conf.num_of_sched_cyclic_plans; j++) {
        ...
        for (k = 0; k < plan->num_of_slotss; k++) {
...
void check_part_not_alloc_to_more_than_a_cpu(void) {
    ...
    for (e = 0; e < prtos_conf.num_of_partitions; e++) {
        vcpu_to_cpu_table[e] = malloc(prtos_conf_part_table[e].num_of_vcpus * sizeof(struct vcpu_to_cpu));
        memset(vcpu_to_cpu_table[e], -1, prtos_conf_part_table[e].num_of_vcpus * sizeof(struct vcpu_to_cpu));
    }

    for (e = 0; e < prtos_conf.hpv.num_of_cpus; e++) {
        if (prtos_conf.hpv.cpu_table[e].sched_policy == CYCLIC_SCHED) {
            for (i = 0; i < prtos_conf.hpv.cpu_table[e].num_of_sched_cyclic_plans; i++) {
                ...
                for (j = 0; j < plan->num_of_slotss; j++) {
                    ...
                    if (vcpu_to_cpu_table[slot->partition_id][slot->vcpu_id].cpu == -1) {
                        ...
                        vcpu_to_cpu_table[slot->partition_id][slot->vcpu_id].line =
                            prtos_conf_sched_cyclic_slot_table_line_number[j + plan->slots_offset].vCpuId;
                        continue;
                    }
        ...
        for (k = 0; k < plan->num_of_slotss; k++) {
            if ((prtos_conf_sched_cyclic_slot_table[k + plan->slots_offset].vcpu_id < 0) ||
                (prtos_conf_sched_cyclic_slot_table[k + plan->slots_offset].vcpu_id >=
                 prtos_conf_part_table[prtos_conf_sched_cyclic_slot_table[k + plan->slots_offset].partition_id].num_of_vcpus))
...
void check_partition_name(char *name, int line) {
    int e;
    for (e = 0; e < prtos_conf.num_of_partitions; e++)
        if (!strcmp(&str_tables[prtos_conf_part_table[e].name_offset], name))
            line_error(line, "partition name \"%s\" duplicated (line %d)", name, prtos_conf_part_table_line_number[e].name);
}

void check_max_num_of_kthreads(void) {
    int e, num_of_kthreads = prtos_conf.num_of_partitions;
    for (e = 0; e < prtos_conf.num_of_partitions; e++) num_of_kthreads += prtos_conf_part_table[e].num_of_vcpus;
