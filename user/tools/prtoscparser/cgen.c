...
static inline void generate_cyclic_plans(FILE *out_file) {
    ...
    for (j = 0; j < prtos_conf.num_of_sched_cyclic_plans; j++) {
        ...
        fprintf(out_file,
                ...
                prtos_conf_sched_cyclic_plan_table[j].slots_offset);
#else
        fprintf(out_file,
                ADDNTAB(1, "[%d] = {\n") ADDNTAB(2, ".name_offset = %d,\n") ADDNTAB(2, ".id = 0x%x,\n") ADDNTAB(2, ".major_frame = %u,\n")
                    ADDNTAB(2, ".num_of_slots = %d,\n") ADDNTAB(2, ".slots_offset = %d,\n") ADDNTAB(1, "},\n"),
...
static inline void generate_partition_tab(FILE *out_file) {
    ...
    fprintf(out_file, "struct prtos_conf_boot_part prtos_conf_boot_partition_table[%d];\n", prtos_conf.num_of_partitions);
    ...
    for (e = 0, num_of_vcpus = 0; e < prtos_conf.num_of_partitions; e++, num_of_vcpus += prtos_conf_partition_table[e].num_of_vcpus) {
        ...
        generate_dev(out_file, "console_device", &prtos_conf_partition_table[e].console_device);
        fprintf(out_file, ADDNTAB(2, ".num_of_physical_memory_areas = %d,\n") ADDNTAB(2, ".physical_memory_areas_offset = %d,\n"),
                prtos_conf_partition_table[e].num_of_physical_memory_areas, prtos_conf_partition_table[e].physical_memory_areas_offset);
        generate_hm(out_file, prtos_conf_partition_table[e].hm_table);
        fprintf(out_file, ADDNTAB(2, ".num_of_ports = %d,\n") ADDNTAB(2, ".comm_ports_offset = %d,\n"), prtos_conf_partition_table[e].num_of_ports,
                prtos_conf_partition_table[e].comm_ports_offset);

        fprintf(out_file, ADDNTAB(2, ".io_ports_offset = %d,\n"), prtos_conf_partition_table[e].io_ports_offset);
        fprintf(out_file, ADDNTAB(2, ".num_of_io_ports = %d,\n"), prtos_conf_partition_table[e].num_of_io_ports);
        generate_trace(out_file, &prtos_conf_partition_table[e].trace);
        fprintf(out_file, ADDNTAB(2, ".ipvi_table = {\n"));
        for (i = 0; i < CONFIG_PRTOS_MAX_IPVI; i++) {
            fprintf(out_file, ADDNTAB(3, "[%d] = {\n"), i);
            fprintf(out_file, ADDNTAB(4, ".dst_offset = %d,\n"), prtos_conf_partition_table[e].ipvi_table[i].dst_offset);
            fprintf(out_file, ADDNTAB(4, ".num_of_dsts = %d,\n"), prtos_conf_partition_table[e].ipvi_table[i].num_of_dsts);
            fprintf(out_file, ADDNTAB(3, "},\n"));
        }
        fprintf(out_file, ADDNTAB(2, "},\n"));
        fprintf(out_file, ADDNTAB(1, "},\n"));
...
static inline void generate_prtos_config(FILE *out_file) {
    ...
    fprintf(out_file, ADDNTAB(1, ".partition_table_offset = (prtos_address_t)prtos_conf_partition_table,\n"));
    fprintf(out_file, ADDNTAB(1, ".num_of_partitions = %d,\n"), prtos_conf.num_of_partitions);

    fprintf(out_file, ADDNTAB(1, ".boot_partition_table_offset = (prtos_address_t)prtos_conf_boot_partition_table,\n"));
...
static void generate_rsv_mem(FILE *out_file) {
    ...
    for (i = 0, num_of_vcpus = 0; i < prtos_conf.num_of_partitions; i++) {
        ...
        for (j = 0; j < prtos_conf_partition_table[i].num_of_vcpus; j++) {
...
void generate_vcpu_table(FILE *out_file) {
    ...
    for (part = 0; part < prtos_conf.num_of_partitions; part++) {
        fprintf(out_file, "{\n");
        for (vcpu = 0; vcpu < prtos_conf_partition_table[part].num_of_vcpus && vcpu < prtos_conf.hpv.num_of_cpus; vcpu++) {
...
void generate_c_file(FILE *out_file) {
    ...
    fprintf(out_file, "extern char _prtos_c_size[];\n\n");
    fprintf(out_file, "extern char _data_size[];\n\n");
