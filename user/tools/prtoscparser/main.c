        ...
        if (node->type == XML_ELEMENT_NODE) {
            for (e = 0; handlers[e]; e++) {
                if (!xmlStrcasecmp(node->name, handlers[e]->name)) {
                    if (handlers[e]->handlerBNode) (handlers[e]->handlerBNode)(node);
                    ...
                    if (handlers[e]->handlerEAttr) (handlers[e]->handlerEAttr)(node);
                    process_xml_tree(node->children, handlers[e]->children);

                    if (handlers[e]->handlerENode) (handlers[e]->handlerENode)(node);
                }
            }
        }
    }
