...
struct prtos_conf_part *prtos_conf_part_table = 0;
struct prtos_conf_part_line_number *prtos_conf_part_table_line_number = 0;
...
#define IS_DEFINED(_x) (((_x).id != PRTOS_DEV_INVALID_ID) && ((_x).id < dev_name_table_length))
    ...
    for (e = 0; e < prtos_conf.num_of_partitions; e++) {
        if (IS_DEFINED(prtos_conf_part_table[e].console_device))
            prtos_conf_part_table[e].console_device =
                look_up_device(dev_name_table[prtos_conf_part_table[e].console_device.id], prtos_conf_part_table_line_number[e].console_device);

        if (IS_DEFINED(prtos_conf_part_table[e].trace.dev))
            prtos_conf_part_table[e].trace.dev =
                look_up_device(dev_name_table[prtos_conf_part_table[e].trace.dev.id], prtos_conf_part_table_line_number[e].trace.dev);
    }
}
...
void link_channels_to_ports(void) {
    ...
    for (i = 0; i < num_of_ipc_ports; i++) {
        ...
        if (ipc_port_table[i].partition_name &&
            strcmp(ipc_port_table[i].partition_name, &str_tables[prtos_conf_part_table[ipc_port_table[i].partition_id].name_offset]))
            line_error(ipc_port_table_line_number[i].partition_name, "partition name \"%s\" mismatches with the expected one \"%s\"",
                       ipc_port_table[i].partition_name, &str_tables[prtos_conf_part_table[ipc_port_table[i].partition_id].name_offset]);
        for (e = 0; e < prtos_conf_part_table[ipc_port_table[i].partition_id].noPorts; e++) {
            port = &prtos_conf_comm_port_table[prtos_conf_part_table[ipc_port_table[i].partition_id].comm_ports_offset + e];
        ...
        if (e >= prtos_conf_part_table[ipc_port_table[i].partition_id].noPorts)
...
void setup_hw_irq_mask(void) {
    ...
    for (e = 0; e < prtos_conf.num_of_partitions; e++) {
        for (i = 0; i < CONFIG_NO_HWIRQS; i++) {
            if (prtos_conf.hpv.hw_irq_table[i].owner == prtos_conf_part_table[e].id) prtos_conf_part_table[e].hw_irqs |= (1 << i);
        }
    }
}
...
void sort_phys_mem_areas(void) {
    prtos_s32_t e;
    for (e = 0; e < prtos_conf.num_of_partitions; e++) {
        qsort(&prtos_conf_mem_area_table[prtos_conf_part_table[e].physical_memory_areas_offset], prtos_conf_part_table[e].num_of_physical_memory_areas,
              sizeof(struct prtos_conf_memory_area), (int (*)(const void *, const void *))cmp_mem_area);
        check_all_mem_areas(&prtos_conf_mem_area_table[prtos_conf_part_table[e].physical_memory_areas_offset],
                            &prtos_conf_mem_area_table_line_number[prtos_conf_part_table[e].physical_memory_areas_offset],
                            prtos_conf_part_table[e].num_of_physical_memory_areas);
    }
}
...
void process_ipvi_table(void) {
    ...
    for (e = 0; e < prtos_conf.num_of_partitions; e++) {
        memset(&prtos_conf_part_table[e].ipvi_table, 0, sizeof(struct prtos_conf_part_ipvi) * CONFIG_PRTOS_MAX_IPVI);
        for (src = 0, i = 0; i < num_of_src_ipvi; i++) {
            src = &src_ipvi_table[i];
            if (prtos_conf_part_table[e].id == src->id) {
                prtos_conf_part_table[e].ipvi_table[src->ipvi_id].dst_offset = prtos_conf.num_of_ipvi_dsts;
                for (j = 0; j < src->num_of_dsts; j++) {
                    dst = &src->dst[j];
                    prtos_conf.num_of_ipvi_dsts++;
                    prtos_conf_part_table[e].ipvi_table[src->ipvi_id].num_of_dsts++;
