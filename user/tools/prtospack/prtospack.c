...
struct prtos_exec_partition *part_table = 0, *hypervisor;
...
static int str_tablesLen = 0;

static inline int add_string(char *str) {
    int offset;
    offset = str_tablesLen;
    str_tablesLen += strlen(str) + 1;
    DO_REALLOC(str_tables, str_tablesLen * sizeof(char));
...
static void get_partition_info(char *in) {
    ...
    num_of_partitions++;
    DO_REALLOC(part_table, num_of_partitions * sizeof(struct prtos_exec_partition));
    partition = &part_table[num_of_partitions - 1];
...
static void write_container_to_file(char *file) {
    ...
    for (e = 0; e < num_of_partitions; e++) {
        part_table[e].id = RWORD(part_table[e].id);
        part_table[e].file = RWORD(part_table[e].file);
        for (i = 0; i < part_table[e].num_of_custom_files; i++) {
            part_table[e].custom_file_table[i] = RWORD(part_table[e].custom_file_table[i]);
        }
        part_table[e].num_of_custom_files = RWORD(part_table[e].num_of_custom_files);
    }
    ...
    prtos_exec_container_hdr.part_table_offset = RWORD(pos);
    DO_WRITE(fd, part_table, sizeof(struct prtos_exec_partition) * num_of_partitions);
    pos += sizeof(struct prtos_exec_partition) * num_of_partitions;

    pos = ALIGNTO(pos, 8);
    lseek(fd, pos, SEEK_SET);
    prtos_exec_container_hdr.string_table_offset = RWORD(pos);
    prtos_exec_container_hdr.str_len = RWORD(str_tablesLen);
    DO_WRITE(fd, str_tables, str_tablesLen);
    pos += str_tablesLen;
...
static void do_build(int argc, char **argv) {
    ...
    prtos_exec_container_hdr.version = RWORD(PRTOS_SET_VERSION(PRTOSPACK_VERSION, PRTOSPACK_SUBVERSION, PRTOSPACK_REVISION));
    num_of_partitions++;
    DO_REALLOC(part_table, sizeof(struct prtos_exec_partition));
    hypervisor = &part_table[0];
...
static void read_container_from_file(char *file) {
    ...
    prtos_exec_container_hdr.part_table_offset = RWORD(prtos_exec_container_hdr.part_table_offset);
    ...
    str_tablesLen = prtos_exec_container_hdr.str_len;
    ...
    lseek(fd, 0, SEEK_SET);
    read(fd, file_img, prtos_exec_container_hdr.file_size);
    DO_REALLOC(file_data, file_data_length * sizeof(prtos_u8_t));
    DO_REALLOC(part_table, num_of_partitions * sizeof(struct prtos_exec_partition));
    DO_REALLOC(prtos_exec_file_table, num_of_files * sizeof(struct prtos_exec_file));
    lseek(fd, prtos_exec_container_hdr.part_table_offset, SEEK_SET);
    read(fd, part_table, num_of_partitions * sizeof(struct prtos_exec_partition));
    lseek(fd, prtos_exec_container_hdr.file_table_offset, SEEK_SET);
    read(fd, prtos_exec_file_table, num_of_files * sizeof(struct prtos_exec_file));
    lseek(fd, prtos_exec_container_hdr.file_data_offset, SEEK_SET);
    read(fd, file_data, file_data_length * sizeof(prtos_u8_t));
    lseek(fd, prtos_exec_container_hdr.string_table_offset, SEEK_SET);
    DO_MALLOC(str_tables, str_tablesLen);
    read(fd, str_tables, str_tablesLen);
    // Process all structures' content
    for (e = 0; e < num_of_partitions; e++) {
        part_table[e].id = RWORD(part_table[e].id);
        part_table[e].file = RWORD(part_table[e].file);
        part_table[e].num_of_custom_files = RWORD(part_table[e].num_of_custom_files);
        for (i = 0; i < part_table[e].num_of_custom_files; i++) part_table[e].custom_file_table[i] = RWORD(part_table[e].custom_file_table[i]);
    }
    for (e = 0; e < num_of_files; e++) {
...
static void do_list(int argc, char **argv) {
    ...
    for (e = 0; e < prtos_exec_container_hdr.num_of_partitions; e++) {
        if (!e)
            ...
            fprintf(stderr, _SP "<Partition id=\"%d\" ", part_table[e].id);

        fprintf(stderr, "file=\"%d\"", part_table[e].file);
        fprintf(stderr, ">\n");
        for (i = 0; i < part_table[e].num_of_custom_files; i++) {
            fprintf(stderr, _SP _SP "<CustomFile ");
            fprintf(stderr, "file=\"%d\" ", part_table[e].custom_file_table[i]);
            fprintf(stderr, "/>\n");
        }
        if (!e)
            fprintf(stderr, _SP "</PRTOSHypervisor>\n");
