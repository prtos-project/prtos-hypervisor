...
static struct prtos_conf *parse_prtos_conf(struct file_to_process *prtos_conf_file) {
    ...
    prtos_u8_t *uimg;
    if ((RWORD(prtos_conf_file->pef.hdr->flags) & PEF_TYPE_MASK) != PEF_TYPE_CUSTOMFILE)
        error_printf("File \"%s\": expecting a XML customization file", prtos_conf_file->name);

    if (RWORD(prtos_conf_file->pef.hdr->flags) & PEF_COMPRESSED) {
        prtos_s32_t ptr, e;
        DO_MALLOC(uimg, RWORD(prtos_conf_file->pef.hdr->image_length));
        for (ptr = 0, e = 0; e < RWORD(prtos_conf_file->pef.hdr->num_of_segments); e++, ptr += RWORD(prtos_conf_file->pef.segment_table[e].file_size)) {
            prtos_u8_t *r_ptr, *w_ptr;
            r_ptr = (prtos_u8_t *)&prtos_conf_file->pef.image[RWORD(prtos_conf_file->pef.segment_table[e].offset)];
            w_ptr = (prtos_u8_t *)&uimg[ptr];
            uncompress(RWORD(prtos_conf_file->pef.segment_table[e].deflated_file_size), RWORD(prtos_conf_file->pef.segment_table[e].file_size), u_read, &r_ptr,
                       u_write, &w_ptr);
        }
        prtos_conf_table = (struct prtos_conf *)uimg;
    } else {
        prtos_conf_table = (struct prtos_conf *)prtos_conf_file->pef.image;
    }

    if (!prtos_conf_table || prtos_conf_table->signature != RWORD(PRTOSC_SIGNATURE))
        error_printf("File \"%s\": invalid PRTOS Configure File signature (0x%x)", prtos_conf_file->name, prtos_conf_table->signature);
...
static void check_partition(int part_id, struct file_to_process *part, struct file_to_process *custom_table, int num_of_custom,
    ...
    prtos_conf_partition_table = (struct prtos_conf_part *)&((char *)prtos_conf_table)[RWORD(prtos_conf_table->partition_table_offset)];

    if (((RWORD(part->pef.hdr->flags) & PEF_TYPE_MASK) != PEF_TYPE_PARTITION) && ((RWORD(part->pef.hdr->flags) & PEF_TYPE_MASK) != PEF_TYPE_HYPERVISOR))
        error_printf("File \"%s\": prtos or a partition expected", part->name);

    for (e = 0; e < num_of_custom; e++)
        if ((RWORD(custom_table[e].pef.hdr->flags) & PEF_TYPE_MASK) != PEF_TYPE_CUSTOMFILE)
            error_printf("File \"%s\": customization file expected ", custom_table[e].name);

    if (num_of_custom != RWORD(part->pef.hdr->num_of_custom_files))
        // error_printf
        fprintf(stderr, "File \"%s\": %d customization files expected, %d provided\n", part->name, RWORD(part->pef.hdr->num_of_custom_files), num_of_custom);

    if ((RWORD(part->pef.hdr->flags) & PEF_TYPE_MASK) == PEF_TYPE_HYPERVISOR) {
        ...
        prtos_conf_mem_area = &prtos_conf_mem_area[RWORD(prtos_conf_partition_table[part_id].physical_memory_areas_offset)];
        ...
        if (RWORD(part->pef.custom_file_table[e].size)) {
            if (RWORD(custom_table[e].pef.hdr->image_length) > RWORD(part->pef.custom_file_table[e].size))
                error_printf("Customization file \"%s\": too big (%d bytes), partition \"%s\" only reserves %d bytes", custom_table[e].name,
                             RWORD(custom_table[e].pef.hdr->image_length), part->name, RWORD(part->pef.custom_file_table[e].size));
        }
...
static void get_partition_info(char *in, int *partition_id, struct file_to_process *part, struct file_to_process **custom_table, int *num_of_custom_files) {
    char *ptr, *save_ptr = 0, *file;
    int e, id = -1;

    for (ptr = in, e = 0; (file = strtok_r(ptr, ":", &save_ptr)); ptr = 0, e++) {
        if (id < 0) {
...
void do_check_to_build(int argc, char **argv) {
    ...
    for (e = 0; e < argc; e++) {
        ...
        if ((ret = parse_pef_file(load_file(prtos_conf.name), &prtos_conf.pef)) != PEF_OK)
            error_printf("Error loading PEF file \"%s\": %d", prtos_conf.name, ret);
