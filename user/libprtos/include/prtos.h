...
extern struct lib_prtos_params {
    part_ctl_table_t *part_ctrl_table[CONFIG_MAX_NO_VCPUS];
    struct prtos_physical_mem_map *part_mem_map[CONFIG_MAX_NO_VCPUS];
    prtos_word_t *comm_port_bitmap[CONFIG_MAX_NO_VCPUS];
} lib_prtos_params;

extern __stdcall void init_libprtos(part_ctl_table_t *part_ctrl_table);
...
static inline part_ctl_table_t *prtos_get_pct(void) {
    return lib_prtos_params.part_ctrl_table[prtos_get_vcpuid()];
}

static inline part_ctl_table_t *prtos_get_pct0(void) {
