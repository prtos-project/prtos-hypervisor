 ...
 * FILE: hypercalls.c
 *
 * prtos system calls definitions for LoongArch 64
 *
 * http://www.prtos.org/
 */
...
void prtos_x86_iret(void) {
    ...
    __asm__ __volatile__("syscall 0" : "+r"(_a0) : : "memory");
}
...
__stdcall prtos_s32_t prtos_raise_partition_trap(prtos_u32_t trap_nr) {
    ...
    __asm__ __volatile__("syscall 0" : "+r"(_a0) : "r"(_a1) : "$a2", "$a3", "$a4", "$a5", "memory");
