...
static inline prtos_s32_t create_sampling_port(prtos_obj_desc_t desc, prtos_s8_t *__g_param port_name, prtos_u32_t max_msg_size, prtos_u32_t direction,
    ...
    for (port = partition->comm_ports_offset; port < (partition->noPorts + partition->comm_ports_offset); port++)
...
static prtos_s32_t read_sampling_port(prtos_obj_desc_t desc, void *__g_param msg_ptr, prtos_u_size_t msg_size, prtos_u32_t *__g_param flags) {
    ...
    if (!(pTFlags & COMM_PORT_OPENED)) return PRTOS_INVALID_PARAM;
    ...
    if (prtos_conf_comm_ports[port].channel_id != PRTOS_NULL_CHANNEL) {
        ...
        retSize = (msg_size < channel->s.length) ? msg_size : channel->s.length;
        memcpy(msg_ptr, channel->s.buffer, retSize);
        spin_lock(&port_table[port].lock);
        ...
        comm_port_bitmap =
            (prtos_word_t *)((prtos_address_t)g->part_ctrl_table + sizeof(part_ctl_table_t) +
        ...
        prtosClearBit(comm_port_bitmap, (port - get_partition(processor_info->sched.current_kthread)->cfg->comm_ports_offset),
        ...
        if (channel->s.sender) {
            ...
            for (e = 0; e < channel->s.sender->cfg->num_of_vcpus; e++) {
                ...
                comm_port_bitmap = (prtos_word_t *)((prtos_address_t)g->part_ctrl_table + sizeof(part_ctl_table_t) +
                                                    sizeof(struct prtos_physical_mem_map) *
                                                        get_partition(processor_info->sched.current_kthread)->cfg->num_of_physical_memory_areas);
                prtosClearBit(comm_port_bitmap, channel->s.sender_port, g->part_ctrl_table->num_of_comm_ports);
            }
        ...
        if (flags) {
            *flags = 0;
            if (retSize && (prtos_conf_comm_channel->s.valid_period != PRTOS_INFINITE_TIME) &&
...
static prtos_s32_t receive_queuing_port(prtos_obj_desc_t desc, void *__g_param msg_ptr, prtos_u32_t msg_size) {
    ...
    if (prtos_conf_comm_ports[port].channel_id != PRTOS_NULL_CHANNEL) {
        ...
        if (channel->q.used_msgs > 0) {
            ...
            retSize = (msg_size < msg->length) ? msg_size : msg->length;
            memcpy(msg_ptr, msg->buffer, retSize);
#ifdef CONFIG_OBJ_STATUS_ACC
            ...
            if (!channel->q.used_msgs) {
                ...
                for (e = 0; e < get_partition(processor_info->sched.current_kthread)->cfg->num_of_vcpus; e++) {
                    ...
                    ASSERT_LOCK((port - get_partition(processor_info->sched.current_kthread)->cfg->comm_ports_offset) < g->part_ctrl_table->num_of_comm_ports,
                                &channel->q.lock);
                    prtosClearBit(comm_port_bitmap, (get_partition(processor_info->sched.current_kthread)->cfg->comm_ports_offset),
                                  g->part_ctrl_table->num_of_comm_ports);
                }
                ...
                if (channel->q.sender) {
                    for (e = 0; e < channel->q.sender->cfg->num_of_vcpus; e++) {
                        ...
                        prtosClearBit(comm_port_bitmap, channel->q.sender_port, g->part_ctrl_table->num_of_comm_ports);
                    }
                }
            }
        } else {
