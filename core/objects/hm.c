...
static prtos_s32_t read_hm_log(prtos_obj_desc_t desc, prtos_hm_log_t *__g_param log, prtos_u32_t size) {
    prtos_s32_t e, noLogs = size / sizeof(prtos_hm_log_t);
    ...
    if (!log || !noLogs) return PRTOS_INVALID_PARAM;

    for (e = 0; e < noLogs; e++)
        if (log_stream_get(&hm_log_stream, &log[e]) < 0) return e * sizeof(prtos_hm_log_t);

    return noLogs * sizeof(prtos_hm_log_t);
}
...
prtos_s32_t hm_raise_event(prtos_hm_log_t *log) {
    ...
    if (system) {
...
#ifdef CONFIG_OBJ_HM_VERBOSE
        kprintf("[HM] ");
        if ((part_table[partition_id].cfg->hm_table[event_id].action < PRTOS_HM_MAX_ACTIONS) &&
            hm_action_to_str[part_table[partition_id].cfg->hm_table[event_id].action])
            kprintf("%s", hm_action_to_str[part_table[partition_id].cfg->hm_table[event_id].action]);
        else
            kprintf("unknown");

        kprintf("(%d) ", part_table[partition_id].cfg->hm_table[event_id].action);
        kprintf("%s\n", hm_gog_to_str[part_table[partition_id].cfg->hm_table[event_id].log]);
#endif
#ifdef CONFIG_AUDIT_EVENTS
        audit_args[0] = ((part_table[partition_id].cfg->hm_table[event_id].log) ? 1 << 31 : 0) | part_table[partition_id].cfg->hm_table[event_id].action;
        raise_audit_event(TRACE_HM_MODULE, AUDIT_HM_PART_ACTION, 1, audit_args);
#endif
        if (part_table[partition_id].cfg->hm_table[event_id].log) {
            prtos_u32_t tmp_seq;
            log->checksum = 0;
            tmp_seq = seq++;
            log->op_code_hi &= ~HMLOG_OPCODE_SEQ_MASK;
        ...
        switch (part_table[partition_id].cfg->hm_table[event_id].action) {
            case PRTOS_HM_AC_IGNORE:
                // Doing nothing
                break;
...
#endif
                if (reset_partition(&part_table[partition_id], PRTOS_COLD_RESET, event_id) < 0) {
...
#endif
                if (reset_partition(&part_table[partition_id], PRTOS_WARM_RESET, event_id) < 0) {
                    HALT_PARTITION(partition_id);
                    schedule();
                }
            ...
            default:
                get_cpu_ctxt(&ctxt);
                system_panic(&ctxt, "Unknown health-monitor action %d\n", part_table[partition_id].cfg->hm_table[event_id].action);
        }
    }
