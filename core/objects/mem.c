...
static inline prtos_s32_t copy_area(prtos_address_t dst_addr, prtos_id_t dst_id, prtos_address_t src_addr, prtos_id_t src_id, prtos_s_size_t size) {
    ...
    if (dst_id != UNCHECKED_ID) {
        ...
        if (!phys_mm_find_area(dst_addr, size, (dst_id != UNCHECKED_ID) ? &partition_table[dst_id] : 0, &flags)) return PRTOS_INVALID_PARAM;
    ...
    if (src_id != UNCHECKED_ID) {
        ...
        if (!phys_mm_find_area(src_addr, size, (src_id != UNCHECKED_ID) ? &partition_table[src_id] : 0, &flags)) return PRTOS_INVALID_PARAM;
    }
