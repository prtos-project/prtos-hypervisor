...
static prtos_s32_t ctrl_status(prtos_obj_desc_t desc, prtos_u32_t cmd, union status_fmd *__g_param args) {
    ...
    switch (cmd) {
        case PRTOS_SET_PARTITION_OPMODE:
            ...
            partition_table[part_id].op_mode = args->op_mode;
            return PRTOS_OK;
        case PRTOS_GET_SYSTEM_STATUS:
            if (part_id == PRTOS_HYPERVISOR_ID) {
                ...
                if ((part_id < 0) || (part_id >= prtos_conf_table.num_of_partitions)) return PRTOS_INVALID_PARAM;
                num_of_vcpus = partition_table[part_id].cfg->num_of_vcpus;
                for (e = 0; e < num_of_vcpus; e++) {
                    if (are_kthread_flags_set(partition_table[part_id].kthread[e], KTHREAD_HALTED_F))
                        halt++;
                    else if (are_kthread_flags_set(partition_table[part_id].kthread[e], KTHREAD_SUSPENDED_F))
                        suspend++;
                    else if (are_kthread_flags_set(partition_table[part_id].kthread[e], KTHREAD_READY_F))
                        ready++;
                }
                ...
                partition_status[part_id].reset_counter = partition_table[part_id].kthread[0]->ctrl.g->part_ctrl_table->reset_counter;
                partition_status[part_id].reset_status = partition_table[part_id].kthread[0]->ctrl.g->part_ctrl_table->reset_status;
                partition_status[part_id].op_mode = partition_table[part_id].op_mode;
                partition_status[part_id].exec_clock = get_time_usec_vclock(&partition_table[part_id].kthread[0]->ctrl.g->vclock);
        ...
        case PRTOS_GET_VCPU_STATUS:
            ...
            if (vcpu_id > partition_table[part_id].cfg->num_of_vcpus) return PRTOS_INVALID_PARAM;
            if (are_kthread_flags_set(partition_table[part_id].kthread[vcpu_id], KTHREAD_HALTED_F)) {
                vcpu_status[vcpu_id].state = PRTOS_STATUS_HALTED;
            } else if (are_kthread_flags_set(partition_table[part_id].kthread[vcpu_id], KTHREAD_SUSPENDED_F)) {
                vcpu_status[vcpu_id].state = PRTOS_STATUS_SUSPENDED;
            } else if (are_kthread_flags_set(partition_table[part_id].kthread[vcpu_id], KTHREAD_READY_F)) {
                vcpu_status[vcpu_id].state = PRTOS_STATUS_READY;
            } else {
                vcpu_status[vcpu_id].state = PRTOS_STATUS_IDLE;
            }
            vcpu_status[vcpu_id].op_mode = partition_table[part_id].kthread[vcpu_id]->ctrl.g->op_mode;
        ...
        case PRTOS_GET_PHYSPAGE_STATUS: {
            ...
            if ((part_id < 0) || (part_id >= prtos_conf_table.num_of_partitions)) return PRTOS_INVALID_PARAM;
            if (!(page = pmm_find_page(args->status.phys_page.p_addr, &partition_table[part_id], 0))) return PRTOS_INVALID_PARAM;
