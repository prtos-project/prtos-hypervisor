...
static prtos_s32_t ctrl_trace(prtos_obj_desc_t desc, prtos_u32_t cmd, union traceCmd *__g_param args) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();
    struct log_stream *log;
    prtos_id_t part_id;
    ...
    if (part_id != KID2PARTID(info->sched.current_kthread->ctrl.g->id))
        if (!(get_partition(info->sched.current_kthread)->cfg->flags & PRTOS_PART_SYSTEM)) return PRTOS_PERM_ERROR;
    if (check_gp_aram(args, sizeof(union traceCmd), 4, PFLAG_NOT_NULL | PFLAG_RW) < 0) return PRTOS_INVALID_PARAM;
    if (part_id == PRTOS_HYPERVISOR_ID)
        log = &prtos_trace_log_stream;
    else {
        if ((part_id < 0) || (part_id >= prtos_conf_table.num_of_partitions)) return PRTOS_INVALID_PARAM;
    ...
    switch (cmd) {
        ...
        case PRTOS_TRACE_RESET:
            if (part_id == PRTOS_HYPERVISOR_ID)
                ...
                log_stream_init(log, find_kdev(&prtos_conf_part_table[part_id].trace.dev), sizeof(prtos_trace_event_t));
            return PRTOS_OK;
    }
...
prtos_s32_t __VBOOT setup_trace(void) {
    ...
    for (e = 0; e < prtos_conf_table.num_of_partitions; e++)
        log_stream_init(&trace_log_stream[e], find_kdev(&prtos_conf_part_table[e].trace.dev), sizeof(prtos_trace_event_t));
