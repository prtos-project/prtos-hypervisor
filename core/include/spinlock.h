...
#define SPINLOCK_INIT                          \
    (spin_lock_t) {                            \
        .arch_lock = __ARCH_SPINLOCK_UNLOCKED, \
    }
#define BARRIER_INIT \
    (barrier_t) {    \
        .v = 0,      \
...
static inline void spin_lock(spin_lock_t *s) {
    __arch_spin_lock(&s->arch_lock);
}

static inline void spin_unlock(spin_lock_t *s) {
    __arch_spin_unlock(&s->arch_lock);
}

static inline prtos_s32_t spin_is_locked(spin_lock_t *s) {
    return (prtos_s32_t)__arch_spin_is_locked(&s->arch_lock);
}
...
#define spin_unlock_irq_restore(s, flags) \
    do {                                  \
        spin_unlock(s);                   \
...
static inline prtos_s32_t spin_try_lock(spin_lock_t *s) {
    return __arch_spin_try_lock(&s->arch_lock);
}
