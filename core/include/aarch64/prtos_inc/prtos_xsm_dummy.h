...
#ifndef __PRTOS_XSM_DUMMY_H__
#define __PRTOS_XSM_DUMMY_H__
    ...
    xsm_default_t action, struct domain *src, struct domain *target)
{
    ...
    case XSM_XS_PRIV:
        if ( action == XSM_XS_PRIV &&
             evaluate_nospec(is_prtosstore_domain(src)) )
            return 0;
        fallthrough;
    case XSM_DM_PRIV:
...
static XSM_INLINE void cf_check xsm_security_domaininfo(
    struct domain *d, struct prtos_domctl_getdomaininfo *info)
{
    return;
}
    ...
    XSM_DEFAULT_ARG struct domain *d, int cmd)
{
    ...
    switch ( cmd )
    {
    case PRTOS_DOMCTL_ioport_mapping:
...
static XSM_INLINE long cf_check xsm_do_xsm_op(PRTOS_GUEST_HANDLE_PARAM(void) op)
{
    return -ENOSYS;
}

#ifdef CONFIG_COMPAT
static XSM_INLINE int cf_check xsm_do_compat_op(PRTOS_GUEST_HANDLE_PARAM(void) op)
{
    return -ENOSYS;
}
#endif
...
static XSM_INLINE int cf_check xsm_bind_pt_irq(
    XSM_DEFAULT_ARG struct domain *d, struct prtos_domctl_bind_pt_irq *bind)
{
    XSM_ASSERT_ACTION(XSM_HOOK);
    return xsm_default_action(action, current->domain, d);
}
    ...
    XSM_DEFAULT_ARG struct domain *d, uint64_t mode, uint32_t op)
{
    ...
    switch ( mode )
    {
    case PRTOS_ALTP2M_mixed:
        return xsm_default_action(XSM_TARGET, current->domain, d);
    case PRTOS_ALTP2M_external:
        return xsm_default_action(XSM_DM_PRIV, current->domain, d);
    case PRTOS_ALTP2M_limited:
    ...
    XSM_DEFAULT_ARG struct domain *d, unsigned int op)
{
    ...
    {
    case PRTOSPMU_init:
    case PRTOSPMU_finish:
    case PRTOSPMU_lvtpc_set:
    case PRTOSPMU_flush:
...
static XSM_INLINE int cf_check xsm_prtos_version(XSM_DEFAULT_ARG uint32_t op)
{
    XSM_ASSERT_ACTION(XSM_OTHER);
    switch ( op )
    {
