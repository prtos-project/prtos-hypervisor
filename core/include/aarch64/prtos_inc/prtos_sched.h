...
DEFINE_PRTOS_GUEST_HANDLE(vcpu_runstate_info_compat_t);
#endif
...
#define BITS_PER_EVTCHN_WORD(d) BITS_PER_PRTOS_ULONG
#else
#define BITS_PER_EVTCHN_WORD(d) (has_32bit_shinfo(d) ? 32 : BITS_PER_PRTOS_ULONG)
#endif
...
#define PRTOS_CONSUMER_BITS 3
#define NR_PRTOS_CONSUMERS ((1 << PRTOS_CONSUMER_BITS) - 1)
...
#endif
    unsigned char prtos_consumer:PRTOS_CONSUMER_BITS; /* Consumer in PRTOS if != 0 */
    evtchn_port_t port;
    union {
        struct {
...
# define runstate_guest(v) ((v)->runstate_guest)
    PRTOS_GUEST_HANDLE(vcpu_runstate_info_t) runstate_guest; /* guest address */
#else
# define runstate_guest(v) ((v)->runstate_guest.native)
    union {
        PRTOS_GUEST_HANDLE(vcpu_runstate_info_t) native;
        PRTOS_GUEST_HANDLE(vcpu_runstate_info_compat_t) compat;
...
struct domain
{
    ...
    struct page_list_head prtospage_list; /* linked list (size prtosheap_pages) */
    ...
    unsigned int     prtosheap_pages;     /* pages allocated from PRTOS heap */
    ...
    /*
     * Number of event channels used internally by PRTOS (not subject to
     * EVTCHNOP_reset).  Read/write access like for active_evtchns.
     */
    unsigned int     prtos_evtchns;
    ...
    bool             is_privileged;
    /* Can this guest access the PRTOS console? */
    bool             is_console;
...
{
    if ( is_prtos_heap_page(pg) )
        return &d->prtospage_list;
...
extern struct vcpu *idle_vcpu[4];
...
#define domain_crash(d, ...)                                        \
    do {                                                            \
        if ( count_args(__VA_ARGS__) == 0 )                         \
            printk(PRTOSLOG_ERR "domain_crash called from %s:%d\n",   \
                   __FILE__, __LINE__);                             \
        else                                                        \
            printk(PRTOSLOG_ERR __VA_ARGS__);                         \
        __domain_crash(d);                                          \
    } while ( 0 )
 ...
 *  'i' [unsigned] {char, int}
 *  'l' [unsigned] long
 *  'h' guest handle (PRTOS_GUEST_HANDLE(foo))
 */
...
#define VPF_down             (1UL<<_VPF_down)
 /* VCPU is blocked awaiting an event to be consumed by PRTOS. */
#define _VPF_blocked_in_prtos  2
#define VPF_blocked_in_prtos   (1UL<<_VPF_blocked_in_prtos)
...
static always_inline bool is_pv_domain(const struct domain *d)
{
    return IS_ENABLED(CONFIG_PV) &&
        evaluate_nospec(!(d->options & PRTOS_DOMCTL_CDF_hvm));
}
...
static always_inline bool hap_enabled(const struct domain *d)
{
    ...
    return IS_ENABLED(CONFIG_HVM) &&
        evaluate_nospec(d->options & PRTOS_DOMCTL_CDF_hap);
}
...
static inline bool is_prtosstore_domain(const struct domain *d)
{
    return d->options & PRTOS_DOMCTL_CDF_xs_domain;
}

static always_inline bool is_iommu_enabled(const struct domain *d)
{
    return evaluate_nospec(d->options & PRTOS_DOMCTL_CDF_iommu);
}
...
extern enum cpufreq_controller {
    FREQCTL_none, FREQCTL_dom0_kernel, FREQCTL_prtos
} cpufreq_controller;
...
static always_inline bool is_cpufreq_controller(const struct domain *d)
{
     ...
     * PRTOS's cpufreq driver, at which point dom0 gets direct access to certain
     * MSRs.
     *
 ...
 * negative, the default scheduler of PRTOS will be used.
 *
 * returns:
 *     pointer to the struct cpupool just created, or PRTOS will panic in case of
 *     error
 */
