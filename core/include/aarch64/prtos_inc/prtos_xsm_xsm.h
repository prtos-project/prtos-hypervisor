/*
 *  This file contains the XSM hook definitions for PRTOS.
 *
...
#include <prtos_alternative-call.h>
#include <prtos_sched.h>
#include <prtos_multiboot.h>
...
enum xsm_default {
    ...
    XSM_XS_PRIV,  /* PRTOSstore domain - can do some privileged operations */
...
struct xsm_ops {
    ...
    int (*bind_pt_irq)(struct domain *d, struct prtos_domctl_bind_pt_irq *bind);
    int (*unbind_pt_irq)(struct domain *d, struct prtos_domctl_bind_pt_irq *bind);
...
#ifdef CONFIG_COMPAT
    int (*do_compat_op)(PRTOS_GUEST_HANDLE_PARAM(void) op);
#endif
...
#define XSM_MMU_MACHPHYS_UPDATE  8
    ...
    int (*prtos_version)(uint32_t cmd);
...
static inline int xsm_bind_pt_irq(
    xsm_default_t def, struct domain *d, struct prtos_domctl_bind_pt_irq *bind)
{
...
static inline long xsm_do_xsm_op(PRTOS_GUEST_HANDLE_PARAM(void) op)
{
    return alternative_call(xsm_ops.do_xsm_op, op);
}

#ifdef CONFIG_COMPAT
static inline int xsm_do_compat_op(PRTOS_GUEST_HANDLE_PARAM(void) op)
{
...
static inline int xsm_prtos_version(xsm_default_t def, uint32_t op)
{
    return alternative_call(xsm_ops.prtos_version, op);
}
