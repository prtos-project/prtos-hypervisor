...
int pt_irq_create_bind(struct domain *d,
                       const struct prtos_domctl_bind_pt_irq *pt_irq_bind);
 ...
 * callback pair.
 */
typedef int iommu_grdm_t(prtos_pfn_t start, prtos_ulong_t nr, u32 id, void *ctxt);
...
/*
 * To be called by PRTOS internally, to register extra RMRR/IVMD ranges for RAM
 * pages.
...
struct domain_iommu {
     ...
     * include/prtos/mm.h). Note that hap_pt_share == false does not
...
int iommu_do_domctl(struct prtos_domctl *domctl, struct domain *d,
                    PRTOS_GUEST_HANDLE_PARAM(prtos_domctl_t) u_domctl);
#else
#define need_iommu_pt_sync(d)     ({ (void)(d); false; })

static inline int iommu_do_domctl(struct prtos_domctl *domctl, struct domain *d,
                                  PRTOS_GUEST_HANDLE_PARAM(prtos_domctl_t) u_domctl)
{
    return -ENOSYS;
}
...
#ifdef CONFIG_HAS_PCI
int iommu_do_pci_domctl(struct prtos_domctl *domctl, struct domain *d,
                        PRTOS_GUEST_HANDLE_PARAM(prtos_domctl_t) u_domctl);
#endif
