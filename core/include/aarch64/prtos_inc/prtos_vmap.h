 ...
 * purpose area (VMAP_DEFAULT) and a livepatch-specific area (VMAP_PRTOS). The
 * latter is used when loading livepatches and the former for everything else.
 */
#if !defined(__PRTOS_VMAP_H__) && defined(VMAP_VIRT_START)
#define __PRTOS_VMAP_H__
...
enum vmap_region {
     ...
     * must live close to the running PRTOS image. The caller also ensures all
...
/* Same as vmalloc(), but for the VMAP_PRTOS vmap region. */
void *vmalloc_prtos(size_t size);

/* Same as vmalloc(), but set the contents to zero before returning */
void *vzalloc(size_t size);

/*
 * Unmap and free memory from vmalloc(), vmalloc_prtos() or vzalloc()
 *
