 ...
 * processor.
 */
static inline void flush_prtos_tlb_range_va_local(vaddr_t va,
                                                unsigned long size)
{
    vaddr_t end = va + size;
    ...
    while ( va < end )
    {
        __flush_prtos_tlb_one_local(va);
        va += PAGE_SIZE;
    }
 ...
 */
static inline void flush_prtos_tlb_range_va(vaddr_t va,
                                          unsigned long size)
{
    vaddr_t end = va + size;
