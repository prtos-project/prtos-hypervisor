#ifndef _PRTOS_PARAM_H
#define _PRTOS_PARAM_H

#include <prtos_bug.h>
#include <prtos_hypfs.h>
#include <prtos_init.h>
#include <prtos_lib.h>
#include <prtos_stdbool.h>
...
#define custom_runtime_only_param(nam, variable, initfunc) \
    __paramfs __parfs_##variable = \
        { .hypfs.e.type = PRTOS_HYPFS_TYPE_STRING, \
          .hypfs.e.encoding = PRTOS_HYPFS_ENC_PLAIN, \
          .hypfs.e.name = (nam), \
...
#define boolean_runtime_only_param(nam, variable) \
    __paramfs __parfs_##variable = \
        { .hypfs.e.type = PRTOS_HYPFS_TYPE_BOOL, \
          .hypfs.e.encoding = PRTOS_HYPFS_ENC_PLAIN, \
          .hypfs.e.name = (nam), \
...
#define integer_runtime_only_param(nam, variable) \
    __paramfs __parfs_##variable = \
        { .hypfs.e.type = PRTOS_HYPFS_TYPE_UINT, \
          .hypfs.e.encoding = PRTOS_HYPFS_ENC_PLAIN, \
          .hypfs.e.name = (nam), \
          .hypfs.e.size = sizeof(variable), \
          .hypfs.e.max_size = sizeof(variable), \
                                   ...
                                   const char *s, const char *e)
{
    ...
    printk(PRTOSLOG_INFO "CONFIG_%s disabled - ignoring '%s=%.*s' setting\n",
           cfg, param, len, s);
}

#endif /* _PRTOS_PARAM_H */
