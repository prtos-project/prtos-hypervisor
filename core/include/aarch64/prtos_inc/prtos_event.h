...
#ifndef __PRTOS_EVENT_H__
#define __PRTOS_EVENT_H__
...
/* Allocate/free a PRTOS-attached event channel port. */
typedef void (*prtos_event_channel_notification_t)(
...
/* Notify remote end of a PRTOS-attached event channel.*/
...
/*
 * "usable" as in "by a guest", i.e. PRTOS consumed channels are assumed to be
 * taken care of separately where used for PRTOS's internal purposes.
 */
static bool evtchn_usable(const struct evtchn *evtchn)
{
    if ( evtchn->prtos_consumer )
        return false;
...
/* Wait on a PRTOS-attached event channel. */
#define wait_on_prtos_event_channel(port, condition)                      \
    do {                                                                \
        if ( condition )                                                \
            break;                                                      \
        set_bit(_VPF_blocked_in_prtos, &current->pause_flags);            \
        smp_mb(); /* set blocked status /then/ re-evaluate condition */ \
        if ( condition )                                                \
        {                                                               \
            clear_bit(_VPF_blocked_in_prtos, &current->pause_flags);      \
            break;                                                      \
        }                                                               \
...
#define prepare_wait_on_prtos_event_channel(port)                         \
    do {                                                                \
        set_bit(_VPF_blocked_in_prtos, &current->pause_flags);            \
        raise_softirq(SCHEDULE_SOFTIRQ);                                \
