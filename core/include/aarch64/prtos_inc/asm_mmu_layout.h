 ...
 *
 *  Reserved to identity map PRTOS
 *
 * 0x00000a0000000000 - 0x00000a7fffffffff (512GB, L0 slot [20])
 *  (Relative offsets)
 *   0  -   2M   Unmapped
 *   2M -  10M   PRTOS text, data, bss
...
#define PRTOS_VIRT_START (PRTOS_VM_MAPPING + _AT(vaddr_t, MB(2)))
#endif

/*
 * Reserve enough space for PRTOS text, data, BSS and runtime memory pool.
 * PRTOS with PRTOS helper code requires ~23MB of runtime virtual space.
 */
#define PRTOS_VIRT_SIZE _AT(vaddr_t, MB(32))
...
#define HYPERVISOR_VIRT_START PRTOS_VIRT_START

#ifdef CONFIG_ARM_32

#else /* ARM_64 */

#define VMAP_VIRT_START (PRTOS_VM_MAPPING + GB(1))
#define VMAP_VIRT_SIZE GB(1)

#define FRAMETABLE_VIRT_START (PRTOS_VM_MAPPING + GB(32))
...
#define PRTOSHEAP_VIRT_START directmap_virt_start
