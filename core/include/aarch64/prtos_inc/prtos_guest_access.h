...
#ifndef __PRTOS_GUEST_ACCESS_H__
#define __PRTOS_GUEST_ACCESS_H__

#include <asm_guest_access.h>
...
/*
 * Cast a guest handle (either PRTOS_GUEST_HANDLE or PRTOS_GUEST_HANDLE_PARAM)
 * to the specified type of PRTOS_GUEST_HANDLE_PARAM.
 */
#define guest_handle_cast(hnd, type) ({         \
    type *_x = (hnd).p;                         \
    (PRTOS_GUEST_HANDLE_PARAM(type)) { _x };      \
})
/* Same for casting to a const type. */
#define guest_handle_const_cast(hnd, type) ({      \
    const type *p_ = (hnd).p;                      \
    (PRTOS_GUEST_HANDLE_PARAM(const_##type)) { p_ }; \
})

/* Cast a PRTOS_GUEST_HANDLE to PRTOS_GUEST_HANDLE_PARAM */
#define guest_handle_to_param(hnd, type) ({                  \
    typeof((hnd).p) _x = (hnd).p;                            \
    PRTOS_GUEST_HANDLE_PARAM(type) _y = { _x };                \
    /*                                                       \
     * type checking: make sure that the pointers inside     \
     * PRTOS_GUEST_HANDLE and PRTOS_GUEST_HANDLE_PARAM are of    \
...
#define guest_handle_for_field(hnd, type, fld)          \
    ((PRTOS_GUEST_HANDLE(type)) { &(hnd).p->fld })

#define guest_handle_from_ptr(ptr, type)        \
    ((PRTOS_GUEST_HANDLE_PARAM(type)) { (type *)(ptr) })
#define const_guest_handle_from_ptr(ptr, type)  \
    ((PRTOS_GUEST_HANDLE_PARAM(const_##type)) { (const type *)(ptr) })
...
#endif /* __PRTOS_GUEST_ACCESS_H__ */
