...
#ifndef _PRTOS_MEM_ACCESS_H
#define _PRTOS_MEM_ACCESS_H
...
struct p2m_domain;
bool prtosmem_access_to_p2m_access(const struct p2m_domain *p2m,
                                 prtosmem_access_t xaccess,
...
long p2m_set_mem_access(struct domain *d, gfn_t gfn, uint32_t nr,
                        uint32_t start, uint32_t mask, prtosmem_access_t access,
                        unsigned int altp2m_idx);

long p2m_set_mem_access_multi(struct domain *d,
                              const PRTOS_GUEST_HANDLE(const_uint64) pfn_list,
                              const PRTOS_GUEST_HANDLE(const_uint8) access_list,
 ...
 */
int p2m_get_mem_access(struct domain *d, gfn_t gfn, prtosmem_access_t *access,
                       unsigned int altp2m_idx);

#ifdef CONFIG_MEM_ACCESS
int mem_access_memop(unsigned long cmd,
                     PRTOS_GUEST_HANDLE_PARAM(prtos_mem_access_op_t) arg);
#else
static inline
int mem_access_memop(unsigned long cmd,
                     PRTOS_GUEST_HANDLE_PARAM(prtos_mem_access_op_t) arg)
