...
enum membank_type {
     ...
     * bank is bound to a static PRTOS domain. It is only valid when the bank
     * is in reserved_mem.
     */
...
extern uint64_t hyp_traps_vector[];
...
extern lpae_t boot_pgtable[PRTOS_PT_LPAE_ENTRIES];

#ifdef CONFIG_ARM_64
extern lpae_t boot_first[PRTOS_PT_LPAE_ENTRIES];
extern lpae_t boot_first_id[PRTOS_PT_LPAE_ENTRIES];
#endif
extern lpae_t boot_second[PRTOS_PT_LPAE_ENTRIES];
extern lpae_t boot_second_id[PRTOS_PT_LPAE_ENTRIES];
extern lpae_t boot_third[PRTOS_PT_LPAE_ENTRIES * PRTOS_NR_ENTRIES(2)];
extern lpae_t boot_third_id[PRTOS_PT_LPAE_ENTRIES];

/* Find where PRTOS will be residing at runtime and return a PT entry */
lpae_t pte_of_prtosaddr(vaddr_t va);
