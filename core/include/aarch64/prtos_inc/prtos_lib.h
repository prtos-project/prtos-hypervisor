...
#include <prtos_inttypes.h>
#include <prtos_stdarg.h>
#include <prtos_types.h>
#include <prtos_xmalloc.h>
...
#define gprintk(lvl, fmt, args...) \
    printk(PRTOSLOG_GUEST lvl "%pv " fmt, current, ## args)
...
#define gdprintk(lvl, fmt, args...) \
    printk(PRTOSLOG_GUEST lvl "%s:%d:%pv " fmt, \
           __FILE__, __LINE__, current, ## args)

#endif

/* vsprintf.c */
#define sprintf __prtos_has_no_sprintf__
#define vsprintf __prtos_has_no_vsprintf__
