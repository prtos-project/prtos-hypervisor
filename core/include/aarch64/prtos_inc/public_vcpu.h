...
#ifndef __PRTOS_PUBLIC_VCPU_H__
#define __PRTOS_PUBLIC_VCPU_H__
...
struct vcpu_runstate_info {
     ...
     */
#define PRTOS_RUNSTATE_UPDATE          (prtos_mk_ullong(1) << 63)
    /*
...
DEFINE_PRTOS_GUEST_HANDLE(vcpu_runstate_info_t);
...
struct vcpu_register_runstate_memory_area {
    union {
        PRTOS_GUEST_HANDLE(vcpu_runstate_info_t) h;
        struct vcpu_runstate_info *v;
        uint64_t p;
    } addr;
};
typedef struct vcpu_register_runstate_memory_area vcpu_register_runstate_memory_area_t;
DEFINE_PRTOS_GUEST_HANDLE(vcpu_register_runstate_memory_area_t);
...
DEFINE_PRTOS_GUEST_HANDLE(vcpu_set_periodic_timer_t);
...
DEFINE_PRTOS_GUEST_HANDLE(vcpu_set_singleshot_timer_t);
...
DEFINE_PRTOS_GUEST_HANDLE(vcpu_register_vcpu_info_t);
...
DEFINE_PRTOS_GUEST_HANDLE(vcpu_get_physid_t);
#define prtos_vcpu_physid_to_x86_apicid(physid) ((uint32_t)(physid))
#define prtos_vcpu_physid_to_x86_acpiid(physid) ((uint32_t)((physid) >> 32))
...
DEFINE_PRTOS_GUEST_HANDLE(vcpu_time_info_t);
struct vcpu_register_time_memory_area {
    union {
        PRTOS_GUEST_HANDLE(vcpu_time_info_t) h;
        struct vcpu_time_info *v;
        uint64_t p;
    } addr;
};
typedef struct vcpu_register_time_memory_area vcpu_register_time_memory_area_t;
DEFINE_PRTOS_GUEST_HANDLE(vcpu_register_time_memory_area_t);
 ...
 *
 * PRTOSFEAT_{runstate,vcpu_time}_phys_area feature bits signal the availability
 * of these ops.
 */
#define VCPUOP_register_runstate_phys_area      14
#define VCPUOP_register_vcpu_time_phys_area     15

#endif /* __PRTOS_PUBLIC_VCPU_H__ */
