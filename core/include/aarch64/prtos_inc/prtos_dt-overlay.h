 ...
 /*
 * core/include/aarch64/prtos_inc/prtos_dt-overlay.h
 *
 * Device tree overlay support in PRTOS.
 *
 * Copyright (C) 2023, Advanced Micro Devices, Inc. All Rights Reserved.
 * Written by Vikram Garhwal <vikram.garhwal@amd.com>
 *
 */
#ifndef __PRTOS_DT_OVERLAY_H__
#define __PRTOS_DT_OVERLAY_H__
...
long dt_overlay_sysctl(struct prtos_sysctl_dt_overlay *op);
long dt_overlay_domctl(struct domain *d, struct prtos_domctl_dt_overlay *op);
#else
#include <prtos_errno.h>
static inline long dt_overlay_sysctl(struct prtos_sysctl_dt_overlay *op)
{
    return -EOPNOTSUPP;
}

static inline long dt_overlay_domctl(struct domain *d,
                                     struct prtos_domctl_dt_overlay *op)
{
    return -EOPNOTSUPP;
}
#endif

#endif /* __PRTOS_DT_OVERLAY_H__ */
