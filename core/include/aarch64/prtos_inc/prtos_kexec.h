#ifndef __PRTOS_KEXEC_H__
#define __PRTOS_KEXEC_H__
...
#include <prtos_kimage.h>

typedef struct prtos_kexec_reserve {
    unsigned long size;
    paddr_t start;
} prtos_kexec_reserve_t;

extern prtos_kexec_reserve_t kexec_crash_area;
...
void machine_kexec_reserved(prtos_kexec_reserve_t *reservation);
void machine_reboot_kexec(struct kexec_image *image);
void machine_kexec(struct kexec_image *image);
void kexec_crash(enum crash_reason reason);
void kexec_crash_save_cpu(void);
struct crash_prtos_info *kexec_crash_save_info(void);
void machine_crash_shutdown(void);
int machine_kexec_get(prtos_kexec_range_t *range);
int machine_kexec_get_prtos(prtos_kexec_range_t *range);

/* vmcoreinfo stuff */
#define VMCOREINFO_BYTES           (4096)
#define VMCOREINFO_NOTE_NAME       "VMCOREINFO_PRTOS"
