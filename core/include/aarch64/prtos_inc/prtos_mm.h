 ...
 *
 *                         +---------------------+
 *                          PRTOS Memory Management
 *                         +---------------------+
 *
 * PRTOS has to handle many different address spaces.  It is important not to
 * get these spaces mixed up.  The following is a consistent terminology which
 * should be adhered to.
 *
 * mfn: Machine Frame Number
 *   The values PRTOS puts into its own pagetables.  This is the host physical
 ...
 *   mfn.  For a non-translated guest which is aware of PRTOS, gfn == mfn.
 *
 ...
 *
 * dfn: Device DMA Frame Number (definitions in include/prtos/iommu.h)
 ...
 *   and, by default, PRTOS will ensure dfn == pfn.
 *
 * pdx: Page InDeX
...
#ifndef __PRTOS_MM_H__
#define __PRTOS_MM_H__

#include <prtos_bug.h>
#include <prtos_compiler.h>
#include <prtos_mm-frame.h>
...
/* PRTOS suballocator. These functions are interrupt-safe. */
void init_prtosheap_pages(paddr_t ps, paddr_t pe);
void prtosheap_max_mfn(unsigned long mfn);
...
#define alloc_prtosheap_page() (alloc_prtosheap_pages(0,0))
#define free_prtosheap_page(v) (free_prtosheap_pages(v,0))

/* Free an allocation, and zero the pointer to it. */
#define FREE_PRTOSHEAP_PAGES(p, o) do { \
    void *_ptr_ = (p);                \
    (p) = NULL;                       \
    free_prtosheap_pages(_ptr_, o);     \
} while ( false )
#define FREE_PRTOSHEAP_PAGE(p) FREE_PRTOSHEAP_PAGES(p, 0)
...
/* Map machine page range in PRTOS virtual address space. */
int map_pages_to_prtos(
    unsigned long virt,
    mfn_t mfn,
    unsigned long nr_mfns,
    unsigned int flags);
/* Alter the permissions of a range of PRTOS virtual address space. */
int modify_prtos_mappings(unsigned long s, unsigned long e, unsigned int nf);
void modify_prtos_mappings_lite(unsigned long s, unsigned long e,
                              unsigned int nf);
int destroy_prtos_mappings(unsigned long s, unsigned long e);
/* Retrieve the MFN mapped by VA in PRTOS virtual address space. */
mfn_t prtos_map_to_mfn(unsigned long va);

/*
 * Create only non-leaf page table entries for the
 * page range in PRTOS virtual address space.
 */
...
extern struct domain *dom_prtos, *dom_io;
...
{
    return is_prtos_heap_page(page) || (page->count_info & PGC_extra);
}
...
union add_to_physmap_extra {
    /*
     * PRTOSMAPSPACE_gmfn: When deferring TLB flushes, a page reference needs
     ...
     * non-NULL, the caller of prtosmem_add_to_physmap_one() indicates it wants
     * to have ownership of such a reference transferred in the success case.
     */
    struct page_info **ppage;

    /* PRTOSMAPSPACE_gmfn_foreign */
    domid_t foreign_domid;
};

int prtosmem_add_to_physmap_one(struct domain *d, unsigned int space,
 ...
 * with PRTOS on behalf of this domain. */
...
static inline void share_prtos_page_with_privileged_guests(
    struct page_info *page, enum PRTOSSHARE_flags flags)
{
    share_prtos_page_with_guest(page, dom_prtos, flags);
}
...
static inline void put_page_alloc_ref(struct page_info *page)
{
     ...
     * reference held.
     *
     * (It's safe for prtosheap pages, because put_page() will not cause
     * them to be freed.)
     */
    if ( test_and_clear_bit(_PGC_allocated, &page->count_info) )
    {
        BUG_ON((page->count_info & (PGC_prtos_heap | PGC_count_mask)) <= 1);
        put_page(page);
    }
}

#endif /* __PRTOS_MM_H__ */
