...
# include <asm_arm64_mm.h>
#else
# error "unknown ARM variant"
#endif

#if defined(CONFIG_MMU)
# include <asm_mmu_mm.h>
#else
# error "Unknown memory management layout"
#endif

/* Align PRTOS to a 2 MiB boundary. */
#define PRTOS_PADDR_ALIGN (1 << 21)
...
/*
 * Stored in bits [28:0] (arm32) or [60:0] (arm64) GFN if page is prtosheap page.
 */
#define PGT_gfn_width     PG_shift(3)
#define PGT_gfn_mask      ((1UL<<PGT_gfn_width)-1)

#define PGT_INVALID_PRTOSHEAP_GFN   _gfn(PGT_gfn_mask)

/*
 * An arch-specific initialization pattern is needed for the type_info field
 * as it's GFN portion can contain the valid GFN if page is prtosheap page.
 */
#define PGT_TYPE_INFO_INITIALIZER   gfn_x(PGT_INVALID_PRTOSHEAP_GFN)

 /* Cleared when the owning guest 'frees' this page. */
#define _PGC_allocated    PG_shift(1)
#define PGC_allocated     PG_mask(1, 1)
  /* Page is PRTOS heap? */
#define _PGC_prtos_heap     PG_shift(2)
#define PGC_prtos_heap      PG_mask(1, 2)
...
#ifdef CONFIG_ARM_32
#else
#define is_prtos_heap_page(page) ((page)->count_info & PGC_prtos_heap)
#define is_prtos_heap_mfn(mfn) \
    (mfn_valid(mfn) && is_prtos_heap_page(mfn_to_page(mfn)))
#endif

#define is_prtos_fixed_mfn(mfn)                                   \
    ((mfn_to_maddr(mfn) >= virt_to_maddr(&_sprtos)) &&           \
     (mfn_to_maddr(mfn) <= virt_to_maddr((vaddr_t)_eprtos - 1)))
...
static inline void *maddr_to_virt(paddr_t ma)
{
    ...
    return (void *)(PRTOSHEAP_VIRT_START -
...
/* Convert between PRTOS-heap virtual addresses and machine addresses. */
#define __pa(x)             (virt_to_maddr(x))
#define __va(x)             (maddr_to_virt(x))

/* Convert between PRTOS-heap virtual addresses and machine frame numbers. */
...
/* Convert between PRTOS-heap virtual addresses and page-info structures. */
static inline struct page_info *virt_to_page(const void *v)
{
    unsigned long va = (unsigned long)v;
    unsigned long pdx;

    ASSERT(va >= PRTOSHEAP_VIRT_START);
    ASSERT(va < directmap_virt_end);

    pdx = (va - PRTOSHEAP_VIRT_START) >> PAGE_SHIFT;
 ...
 */
static inline gfn_t page_get_prtosheap_gfn(const struct page_info *p)
{
    gfn_t gfn_ = _gfn(ACCESS_ONCE(p->u.inuse.type_info) & PGT_gfn_mask);

    ASSERT(is_prtos_heap_page(p));

    return gfn_eq(gfn_, PGT_INVALID_PRTOSHEAP_GFN) ? INVALID_GFN : gfn_;
}

static inline void page_set_prtosheap_gfn(struct page_info *p, gfn_t gfn)
{
    gfn_t gfn_ = gfn_eq(gfn, INVALID_GFN) ? PGT_INVALID_PRTOSHEAP_GFN : gfn;
