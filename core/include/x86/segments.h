...
struct x86_desc {
    union {
        struct {
            prtos_u32_t segLimit15_0 : 16, base15_0 : 16, base23_16 : 8, access : 8, segLimit19_16 : 4, granularity : 4, base31_24 : 8;
        };
        struct {
...
struct x86_tss {
    ...
    prtos_u16_t traceTrap;
    prtos_u16_t ioBitmapOffset;
} __PACKED;

struct io_tss {
    struct x86_tss t;
    prtos_u32_t ioMap[2048];
};
#endif
...
#define GetTssDesc(gdtr, tssSel) ((struct x86_desc *)((gdtr)->linear_base))[tssSel / 8]

#define TssClearBusy(gdtr, tssSel) GetTssDesc((gdtr), tssSel).access &= ~(0x2);

#define TSS_IO_MAP_DISABLED (0xFFFF)

#define DisableTssIoMap(tss) (tss)->t.ioBitmapOffset = TSS_IO_MAP_DISABLED

#define EnableTssIoMap(tss) (tss)->t.ioBitmapOffset = ((prtos_address_t) & ((tss)->ioMap) - (prtos_address_t)tss)

#define SetIoMapBit(bit, ioMap)            \
    do {                                   \
...
#define ClearIoMapBit(bit, ioMap)           \
    do {                                    \
        prtos_u32_t __entry, __offset;      \
        __entry = bit / 32;                 \
        __offset = bit % 32;                \
...
#define IsIoMapBitSet(bit, ioMap)         \
    do {                                  \
        prtos_u32_t __entry, __offset;    \
        __entry = bit / 32;               \
        __offset = bit % 32;              \
...
static inline void load_tss_desc(struct x86_desc *desc, struct io_tss *tss) {
    desc->segLimit15_0 = 0xffff & (sizeof(struct io_tss) - 1);
    desc->segLimit19_16 = 0xf & ((sizeof(struct io_tss) - 1) >> 16);
