...
#endif
    set_kthread_flags(partition_table[part_id].kthread[vcpu_id], KTHREAD_SUSPENDED_F);
}
...
#endif
    clear_kthread_flags(partition_table[part_id].kthread[vcpu_id], KTHREAD_SUSPENDED_F);
}
...
#endif
    set_kthread_flags(partition_table[part_id].kthread[vcpu_id], KTHREAD_HALTED_F);
    partition_table[part_id].kthread[vcpu_id]->ctrl.g->op_mode = PRTOS_OPMODE_IDLE;
}
...
#endif
    for (e = 0; e < partition_table[id].cfg->num_of_vcpus; e++) set_kthread_flags(partition_table[id].kthread[e], KTHREAD_SUSPENDED_F);
}
...
#endif
    for (e = 0; e < partition_table[id].cfg->num_of_vcpus; e++) clear_kthread_flags(partition_table[id].kthread[e], KTHREAD_SUSPENDED_F);
}
...
#endif
    set_part_ext_irq_pending(&partition_table[id], PRTOS_VT_EXT_SHUTDOWN);
}
/*
...
#endif
    for (e=0; e<partition_table[id].cfg->num_of_vcpus; e++)
        clear_kthread_flags(partition_table[id].kthread[e], KTHREAD_READY_F);
}
*/
...
#endif
    for (e = 0; e < partition_table[id].cfg->num_of_vcpus; e++) {
        set_kthread_flags(partition_table[id].kthread[e], KTHREAD_HALTED_F);
        partition_table[id].kthread[e]->ctrl.g->op_mode = PRTOS_OPMODE_IDLE;
    }
    partition_table[id].op_mode = PRTOS_OPMODE_IDLE;
}
