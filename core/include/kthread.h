...
#ifdef CONFIG_MMU
#define GET_GUEST_GCTRL_ADDR(k) (k)->ctrl.g->RAM + ROUNDUP(sizeof(partition_control_table_t))
#else
...
typedef struct partition {
    kthread_t **kthread;
    prtos_address_t pct_array;
    prtos_u_size_t pct_array_size;
    prtos_u32_t op_mode;
    prtos_address_t image_start; /*Partition Memory address in the container*/
    prtos_address_t pbl_stack;   /*Stack address allocated by PRTOS*/
...
static inline partition_t *get_partition(kthread_t *k) {
    if (k->ctrl.g) return &partition_table[KID2PARTID(k->ctrl.g->id)];
...
extern void setup_pct_mm(partition_control_table_t *part_ctrl_table, kthread_t *k);
extern void setup_pct_arch(partition_control_table_t *part_ctrl_table, kthread_t *k);
