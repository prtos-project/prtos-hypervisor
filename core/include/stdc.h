...
static inline void unalign_memcpy(prtos_u8_t *dst, prtos_u8_t *src, prtos_s_size_t size, rd_mem_t src_r, rd_mem_t DstR, wr_mem_t dst_w) {
    prtos_u32_t srcW, dstW;
    prtos_s32_t c1, c2, e;

    for (e = 0, c1 = (prtos_u32_t)src & 0x3, c2 = (prtos_u32_t)dst & 0x3; e < size; src++, dst++, c1 = (c1 + 1) & 0x3, c2 = (c2 + 1) & 0x3, e++) {
        srcW = src_r((prtos_u32_t *)((prtos_u32_t)src & ~0x3));
#ifdef CONFIG_TARGET_LITTLE_ENDIAN
        dstW = srcW & (0xff << ((c1 & 0x3) << 3));
        dstW >>= ((c1 & 0x3) << 3);
        dstW <<= ((c2 & 0x3) << 3);

        dstW |= (DstR((prtos_u32_t *)((prtos_u32_t)dst & ~0x3)) & ~(0xff << ((c2 & 0x3) << 3)));
#else
        dstW = srcW & (0xff000000 >> ((c1 & 0x3) << 3));
        dstW <<= ((c1 & 0x3) << 3);
        dstW >>= ((c2 & 0x3) << 3);

        dstW |= (DstR((prtos_u32_t *)((prtos_u32_t)dst & ~0x3)) & ~(0xff000000 >> ((c2 & 0x3) << 3)));
#endif
        dst_w((prtos_u32_t *)((prtos_u32_t)dst & ~0x3), dstW);
    }
}
