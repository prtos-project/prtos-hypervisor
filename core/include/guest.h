...
static inline void prtos_set_bit(prtos_word_t bm[], prtos_u32_t bp, prtos_s32_t maxBits) {
    prtos_u32_t e = bp >> PRTOS_LOG2_WORD_SZ, b = bp & ((1 << PRTOS_LOG2_WORD_SZ) - 1);
    if (bp >= maxBits) return;
    bm[e] |= (1 << b);
}

static inline void prtosClearBit(prtos_word_t bm[], prtos_u32_t bp, prtos_s32_t maxBits) {
    prtos_u32_t e = bp >> PRTOS_LOG2_WORD_SZ, b = bp & ((1 << PRTOS_LOG2_WORD_SZ) - 1);
    if (bp >= maxBits) return;
    bm[e] &= ~(1 << b);
}

static inline void prtos_clear_bitmap(prtos_word_t bm[], prtos_s32_t maxBits) {
    prtos_u32_t e;
    for (e = 0; e < ((maxBits & ((1 << (PRTOS_LOG2_WORD_SZ)) - 1)) ? (maxBits >> PRTOS_LOG2_WORD_SZ) + 1 : (maxBits >> PRTOS_LOG2_WORD_SZ)); e++) bm[e] = 0;
}

static inline void prtos_set_bitmap(prtos_word_t bm[], prtos_s32_t maxBits) {
    prtos_u32_t e;

    for (e = 0; e < ((maxBits & ((1 << (PRTOS_LOG2_WORD_SZ)) - 1)) ? (maxBits >> PRTOS_LOG2_WORD_SZ) + 1 : (maxBits >> PRTOS_LOG2_WORD_SZ)); e++) bm[e] = ~0;
}

static inline prtos_s32_t prtos_is_bit_set(prtos_word_t bm[], prtos_s32_t bp, prtos_s32_t maxBits) {
    prtos_u32_t e = bp >> PRTOS_LOG2_WORD_SZ, b = bp & ((1 << PRTOS_LOG2_WORD_SZ) - 1);
