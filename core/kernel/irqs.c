...
static void trigger_irq_handler(cpu_ctxt_t *ctxt, void *data) {
    ...
    set_part_hw_irq_pending(&part_table[part_id], ctxt->irq_nr);
}
...
static inline prtos_s32_t are_hw_irqs_pending(part_ctl_table_t *part_ctrl_table) {
...
static inline prtos_s32_t are_ext_irqs_pending(part_ctl_table_t *part_ctrl_table) {
    prtos_s32_t entry_irq;
...
static inline prtos_s32_t are_ext_traps_pending(part_ctl_table_t *part_ctrl_table) {
    prtos_s32_t entry_irq;
