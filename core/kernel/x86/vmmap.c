...
prtos_s32_t vm_map_user_page(partition_t *k, prtos_word_t *ptd_level_1, prtos_address_t p_addr, prtos_address_t v_addr, prtos_u32_t flags,
                             ...
                             prtos_address_t *pool, prtos_s_size_t *poolSize) {
    ...
    if (!(ptd_level_1[l1e] & _PG_ARCH_PRESENT)) {
        p_table = alloc(k->cfg, PTDL2SIZE, PTDL2SIZE, pool, poolSize);
...
#ifdef PRTOS_VERBOSE
        kprintf("\tptd_level_1:(v_addr:0x%x) new alloc page table(v_addr:0x%x=PhyAddr:0x%x), ptd_level_1[%d]=0x%x\n", ptd_level_1, p_ptd_level_2, p_table, l1e,
                ptd_level_1[l1e]);
#endif
    } else {
        ...
        p_ptd_level_2 = vcache_map_page(p_table, page_table_level2);
#ifdef PRTOS_VERBOSE
        kprintf("\tptd_level_1:(v_addr:0x%x) using existing page table(v_addr:0x%x=PhyAddr:0x%x), ptd_level_1[%d]=0x%x\n", ptd_level_1, p_ptd_level_2, p_table,
                l1e, ptd_level_1[l1e]);
#endif
    }
    p_ptd_level_2[l2e] = (p_addr & PAGE_MASK) | vm_attr_to_arch_attr(flags);
#ifdef PRTOS_VERBOSE
    kprintf("\tMap v_addr:0x%x -> phyAddr:0x%x  p_ptd_level_2[%d]=0x%x\n\n", v_addr, p_addr, l2e, p_ptd_level_2[l2e]);
#endif
