...
void setup_x86_idt(void) {
    extern void (*hyp_hw_irq_handlers_table[0])(void);
    extern void (*hyp_trap_handlers_table[0])(void);
    extern void unexpected_irq(void);
    prtos_s32_t irq_nr;

    for (irq_nr = 0; irq_nr < CONFIG_NO_HWIRQS; irq_nr++) {
        ASSERT(hyp_hw_irq_handlers_table[irq_nr]);
        _set_irq_gate(irq_nr + FIRST_EXTERNAL_VECTOR, hyp_hw_irq_handlers_table[irq_nr], 0);
    }
...
void fix_stack(cpu_ctxt_t *ctxt, part_ctl_table_t *part_ctrl_table, prtos_s32_t irq_nr, prtos_s32_t vector, prtos_s32_t trap) {
