...
static inline prtos_u32_t apic_wait_icr_idle(void) {
    prtos_u32_t sendStatus;
    prtos_s32_t timeOut;

    for (timeOut = 0; timeOut < 1000; ++timeOut) {
        sendStatus = lapic_read(APIC_ICR_LOW) & APIC_ICR_BUSY;
        if (!sendStatus) break;
        EarlyDelay(200);
    }
...
#ifdef PRTOS_VERBOSE
                eprintf("x86_mp_conf.ioInt[%d].dst_io_apic_id=%d\n", i, x86_mp_conf.ioInt[i].dst_io_apic_id);
                eprintf("x86_mp_conf.io_apic[%d].id=%d\n", e, x86_mp_conf.io_apic[e].id);
#endif
                setup_io_apic_entry(i, e, IO_APIC_TRIG_EDGE, IO_APIC_POL_HIGH);
            }
        }
#ifdef PRTOS_VERBOSE
        eprintf("x86_mp_conf.num_of_io_int: [%d]\n", x86_mp_conf.num_of_io_int);
#endif
        for (i = 0; i < x86_mp_conf.num_of_io_int; i++) {
            hw_disable_irq(i);
        }
...
static inline void init_lapic_map(void) {
    ...
    apicMsr = read_msr(MSR_IA32_APIC_BASE);
    if (!(apicMsr & _MSR_APICBASE_ENABLE)) x86system_panic("APIC not supported");

    lapic_phys_addr = apicMsr & APIC_BASE_MASK_MSR;
...
void send_ipi(prtos_u8_t dst, prtos_u8_t dst_short_hand, prtos_u8_t vector) {
    prtos_u32_t hIpi, lIpi;

    hIpi = ((local_id_table[dst].hwId) << 24);
    lIpi = ((dst_short_hand & 0x3) << 18) | vector;
    lapic_write(APIC_ICR_HIGH, hIpi);
    lapic_write(APIC_ICR_LOW, lIpi);
}
