...
static void set_hpet_timer(prtos_time_t interval) {
    prtos_u32_t hpet_counter = (interval * HPET_HZ) / USECS_PER_SEC;
    prtos_u32_t cnt;
...
static timer_handler_t set_hpet_timer_handler(timer_handler_t timer_handler) {
    timer_handler_t old_hpet_user_handler = hpet_handler;
    hpet_handler = timer_handler;
    return old_hpet_user_handler;
}
...
static prtos_time_t read_hpet_clock_usec(void) {
    hw_time_t hpet_time = hpet_read_reg64(HPET_MCV);
    return hwtime_to_duration(hpet_time, hpet_freq_hz);
}
