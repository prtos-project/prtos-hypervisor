...
static inline void set_pit_timer_hw_time(prtos_u16_t pit_counter) {
#ifndef CONFIG_PC_PIT_CLOCK
    // ONESHOOT_MODE
    out_byte_port(0x30, PIT_MODE);  // binary, mode 0, LSB/MSB, ch 0 */
    out_byte_port(pit_counter & 0xff, PIT_CH0);
    out_byte_port(pit_counter >> 8, PIT_CH0);
#endif
}
...
static void set_pit_timer(prtos_time_t interval) {
    prtos_u16_t pit_counter = (interval * PIT_HZ) / USECS_PER_SEC;
    set_pit_timer_hw_time(pit_counter);
}
...
static timer_handler_t set_pit_timer_handler(timer_handler_t timer_handler) {
    timer_handler_t old_pit_user_handler = pit_handler;
    pit_handler = timer_handler;
    return old_pit_user_handler;
}
...
static prtos_time_t read_pit_clock_usec(void) {
...
static hw_clock_t pit_clock = {
    ...
    .get_time_usec = read_pit_clock_usec,
    .shutdown_clock = 0,
};
