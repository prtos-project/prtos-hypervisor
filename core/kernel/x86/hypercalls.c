...
static inline prtos_s32_t is_gdt_desc_valid(struct x86_desc *desc, prtos_u32_t *type) {
    ...
    *type = EMPTY_SEG;
    ...
    base = (prtos_u32_t)GetDescBase(desc);
...
__hypercall prtos_s32_t x86_load_gdt_sys(struct x86_desc_reg *__g_param desc) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();
    prtos_s32_t gdtNoEntries, e;
    struct x86_desc *gdt;
    prtos_u32_t type;

    if (check_gp_aram(desc, sizeof(struct x86_desc_reg), 4, PFLAG_NOT_NULL) < 0) return PRTOS_INVALID_PARAM;

    gdtNoEntries = (desc->limit + 1) / sizeof(struct x86_desc);
    if (gdtNoEntries > CONFIG_PARTITION_NO_GDT_ENTRIES) return PRTOS_INVALID_PARAM;
    ...
    for (e = 0, gdt = (struct x86_desc *)desc->linear_base; e < gdtNoEntries; e++)
...
static inline prtos_s32_t is_gate_desc_valid(struct x86_gate *desc) {
    ...
    if ((desc->low0 & 0x1fe0) == 0xf00) { /* Only trap gates supported */
        ...
        if (base >= CONFIG_PRTOS_OFFSET) {
            kprintf("[GateDesc] Base (0x%x) > PRTOS_OFFSET\n", base);
            return 0;
        }
        seg = desc->high0 >> 16;
        if (!(seg & 0x3)) {
