...
extern prtos_u8_t _sprtos[], _eprtos[], phys_prtos_conf_table[];
    ...
    .custom_file_table =
        {
            [0] =
                {
                    .sAddr = (prtos_address_t)phys_prtos_conf_table,
                    .size = 0,
                },
        },
...
static void __VBOOT setup_partitions(void) {
    prtos_address_t st, end, vSt, vEnd;
    partition_t *p;
    prtos_s32_t e, a;
    ...
    for (e = 0; e < prtos_conf_table.num_of_partitions; e++) {
        if ((p = create_partition(&prtos_conf_part_table[e]))) {
            kprintf("P%d (\"%s\":%d:%d) flags: [", e, &prtos_conf_string_tab[prtos_conf_part_table[e].name_offset], prtos_conf_part_table[e].id,
                    prtos_conf_part_table[e].num_of_vcpus);
            if (prtos_conf_part_table[e].flags & PRTOS_PART_SYSTEM) kprintf(" SYSTEM");
            if (prtos_conf_part_table[e].flags & PRTOS_PART_FP) kprintf(" FP");
            kprintf(" ]:\n");
            for (a = 0; a < prtos_conf_part_table[e].num_of_physical_memory_areas; a++) {
                st = prtos_conf_phys_mem_area_table[a + prtos_conf_part_table[e].physical_memory_areas_offset].start_addr;
                end = st + prtos_conf_phys_mem_area_table[a + prtos_conf_part_table[e].physical_memory_areas_offset].size - 1;
                vSt = prtos_conf_phys_mem_area_table[a + prtos_conf_part_table[e].physical_memory_areas_offset].mapped_at;
                vEnd = vSt + prtos_conf_phys_mem_area_table[a + prtos_conf_part_table[e].physical_memory_areas_offset].size - 1;

                kprintf("    [0x%lx:0x%lx - 0x%lx:0x%lx]", st, vSt, end, vEnd);
                kprintf(" flags: 0x%x", prtos_conf_phys_mem_area_table[a + prtos_conf_part_table[e].physical_memory_areas_offset].flags);
                kprintf("\n");
            }

            if (prtos_conf_boot_part_table[e].flags & PRTOS_PART_BOOT) {
...
#define CALC_ABS_ADDR_PRTOSC(_offset) (void *)(prtos_conf_table._offset + (prtos_address_t)&prtos_conf_table)
    ...
    prtos_conf_part_table = CALC_ABS_ADDR_PRTOSC(part_table_offset);
    prtos_conf_boot_part_table = CALC_ABS_ADDR_PRTOSC(boot_part_table_offset);
...
#ifdef PRTOS_VERBOSE
    kprintf("Size of kthread [%d] Bytes\n", sizeof(*idle));
    kprintf("Size of kthread [%d] Bytes\n", sizeof((*idle).ctrl));
#endif
