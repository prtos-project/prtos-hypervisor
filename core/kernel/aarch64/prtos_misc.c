/* PRTOS miscellaneous - consolidated */
...
static prtos_commandline_t saved_cmdline;
...
/**
 *    cmdline_parse -- parses the prtos command line.
...
static int __init cf_check buildinfo_init(void)
{
    ...
    hypfs_string_set_reference(&changeset, prtos_changeset());
    hypfs_add_leaf(&buildinfo, &changeset, true);

    hypfs_add_dir(&buildinfo, &compileinfo, true);
    hypfs_string_set_reference(&compiler, prtos_compiler());
    hypfs_string_set_reference(&compile_by, prtos_compile_by());
    hypfs_string_set_reference(&compile_date, prtos_compile_date());
    hypfs_string_set_reference(&compile_domain, prtos_compile_domain());
    ...
    major_version = prtos_major_version();
    minor_version = prtos_minor_version();
    hypfs_add_dir(&buildinfo, &version, true);
    hypfs_string_set_reference(&extra, prtos_extra_version());
    hypfs_add_leaf(&version, &extra, true);
    hypfs_add_leaf(&version, &major, true);
    hypfs_add_leaf(&version, &minor, true);

#ifdef CONFIG_HYPFS_CONFIG
    config.e.encoding = PRTOS_HYPFS_ENC_GZIP;
    config.e.size = prtos_config_data_size;
    config.u.content = prtos_config_data;
...
static int __init cf_check param_init(void)
{
    ...
    for ( param = __paramhypfs_start; param < __paramhypfs_end; param++ )
    {
        ...
        else if ( param->hypfs.e.type == PRTOS_HYPFS_TYPE_STRING )
...
long do_prtos_version(int cmd, PRTOS_GUEST_HANDLE_PARAM(void) arg)
{
    bool deny = xsm_prtos_version(XSM_OTHER, cmd);

    switch ( cmd )
    {
    case PRTOSVER_version:
        return (prtos_major_version() << 16) | prtos_minor_version();

    case PRTOSVER_extraversion:
    {
        prtos_extraversion_t extraversion;

        memset(extraversion, 0, sizeof(extraversion));
        safe_strcpy(extraversion, deny ? prtos_deny() : prtos_extra_version());
        if ( copy_to_guest(arg, extraversion, ARRAY_SIZE(extraversion)) )
            return -EFAULT;
        return 0;
    }

    case PRTOSVER_compile_info:
    {
        prtos_compile_info_t info;

        memset(&info, 0, sizeof(info));
        safe_strcpy(info.compiler,       deny ? prtos_deny() : prtos_compiler());
        safe_strcpy(info.compile_by,     deny ? prtos_deny() : prtos_compile_by());
        safe_strcpy(info.compile_domain, deny ? prtos_deny() : prtos_compile_domain());
        safe_strcpy(info.compile_date,   deny ? prtos_deny() : prtos_compile_date());
        if ( copy_to_guest(arg, &info, 1) )
            return -EFAULT;
        return 0;
    }

    case PRTOSVER_capabilities:
    {
        prtos_capabilities_info_t info;

        memset(info, 0, sizeof(info));
        if ( !deny )
            arch_get_prtos_caps(&info);
    ...
    case PRTOSVER_platform_parameters:
    {
...
#endif
        {
            prtos_platform_parameters_t params = {
                /*
    ...
    case PRTOSVER_changeset:
    {
        prtos_changeset_info_t chgset;

        memset(chgset, 0, sizeof(chgset));
        safe_strcpy(chgset, deny ? prtos_deny() : prtos_changeset());
        if ( copy_to_guest(arg, chgset, ARRAY_SIZE(chgset)) )
            return -EFAULT;
        return 0;
    }

    case PRTOSVER_get_features:
    {
        prtos_feature_info_t fi;
        ...
        {
        case 0:
            fi.submap = (1U << PRTOSFEAT_memory_op_vnode_supported) |
#ifdef CONFIG_X86
                        (1U << PRTOSFEAT_vcpu_time_phys_area) |
#endif
                        (1U << PRTOSFEAT_runstate_phys_area);
            if ( VM_ASSIST(d, pae_extended_cr3) )
                fi.submap |= (1U << PRTOSFEAT_pae_pgdir_above_4gb);
            if ( paging_mode_translate(d) )
                fi.submap |= 
                    (1U << PRTOSFEAT_writable_page_tables) |
                    (1U << PRTOSFEAT_auto_translated_physmap);
            if ( is_hardware_domain(d) )
                fi.submap |= 1U << PRTOSFEAT_dom0;
#ifdef CONFIG_ARM
            fi.submap |= (1U << PRTOSFEAT_ARM_SMCCC_supported);
#endif
#ifdef CONFIG_X86
            if ( is_pv_domain(d) )
                fi.submap |= (1U << PRTOSFEAT_mmu_pt_update_preserve_ad) |
                             (1U << PRTOSFEAT_highmem_assist) |
                             (1U << PRTOSFEAT_gnttab_map_avail_bits);
            else
                fi.submap |= (1U << PRTOSFEAT_hvm_safe_pvclock) |
                             (1U << PRTOSFEAT_hvm_callback_vector) |
                             (has_pirq(d) ? (1U << PRTOSFEAT_hvm_pirqs) : 0);
            fi.submap |= (1U << PRTOSFEAT_dm_msix_all_writes);
#endif
            if ( !paging_mode_translate(d) || is_domain_direct_mapped(d) )
                fi.submap |= (1U << PRTOSFEAT_direct_mapped);
            else
                fi.submap |= (1U << PRTOSFEAT_not_direct_mapped);
            break;
        default:
            return -EINVAL;
        }
    ...
    case PRTOSVER_pagesize:
        if ( deny )
            return 0;
        return (!guest_handle_is_null(arg) ? -EINVAL : PAGE_SIZE);

    case PRTOSVER_guest_handle:
    {
        prtos_domain_handle_t hdl;
    ...
    case PRTOSVER_commandline:
    {
        size_t len = ARRAY_SIZE(saved_cmdline);

        if ( deny )
            len = strlen(prtos_deny()) + 1;

        if ( copy_to_guest(arg, deny ? prtos_deny() : saved_cmdline, len) )
            return -EFAULT;
        return 0;
    }

    case PRTOSVER_build_id:
    {
        prtos_build_id_t build_id;
        unsigned int sz;
        int rc;
        const void *p;
        ...
        if ( copy_to_guest_offset(arg, offsetof(prtos_build_id_t, buf), p, sz) )
            return -EFAULT;
                                      ...
                                      struct bootmodule *mod)
{
    ...
    /*
     * While uboot considers 0x0 to be a valid load/start address, for PRTOS
     * to maintain parity with zImage, we consider 0x0 to denote position
     * independent image. That means PRTOS is free to load such an image at
     * any valid address.
     */
    if ( info->zimage.start == 0 )
        printk(PRTOSLOG_INFO
               "No load address provided. PRTOS will decide where to load it.\n");
    else
        printk(PRTOSLOG_INFO
     ...
     * provide an entry point as PRTOS will load such an image at any appropriate
    ...
    case IH_ARCH_ARM64:
        ...
        break;
    default:
        printk(PRTOSLOG_ERR "Unsupported uImage arch type %d\n", uimage.arch);
        return -EINVAL;
    }
     ...
     * zImage or zImage64 header, PRTOS uses the attributes of uImage header only.
     * Thus, PRTOS uses uimage.load attribute to determine the load address and
                        ...
                        const struct dt_device_node *domain)
{
    ...
    if ( !mod || !mod->size )
    {
        printk(PRTOSLOG_ERR "Missing kernel boot module?\n");
        return -ENOENT;
    }
...
void __init prtos_build_init(void)
{
    ...
    rc = prtos_build_id_check(n, sz, &build_id_p, &build_id_len);
    ...
    if ( rc && efi_enabled(EFI_LOADER) )
    {
        ...
        if ( sz > sizeof(*dir) + sizeof(struct cv_info_pdb70) &&
             ...
             PRTOS_VIRT_START + dir->rva_of_data == (unsigned long)(dir + 1) )
        {
...
#endif
    if ( !rc )
        printk(PRTOSLOG_INFO "build-id: %*phN\n", build_id_len, build_id_p);
}
#endif
/*
 * Local variables:
                            ...
                            mmio_info_t *info)
{
    ...
    if ( info->dabt.valid )
    {
             ...
             info->dabt.write )
        {
            ...
            if ( rc )
            {
                gprintk(PRTOSLOG_DEBUG, "Unable to decode instruction\n");
     ...
     * PRTOS and thus it forwards the abort to the guest.
     */
    if ( info->dabt.s1ptw )
    {
        info->dabt_instr.state = INSTR_ERROR;
    ...
    /*
     * When the data abort is caused due to cache maintenance, PRTOS should check
     ...
     * instructions. So in order to process these instructions, PRTOS must
     * decode them.
     */
    rc = decode_instruction(regs, info);
    if ( rc )
    {
        gprintk(PRTOSLOG_ERR, "Unable to decode instruction\n");
        info->dabt_instr.state = INSTR_ERROR;
    }
}
                              ...
                              mmio_info_t *info)
{
     ...
     * belongs to an emulated region, PRTOS should ignore this instruction.
     */
     ...
     * decoded successfully. Thus, PRTOS should be allowed to execute the
...
static void cf_check dump_domains(unsigned char key)
{
    ...
    for_each_domain ( d )
    {
        ...
        printk("    nr_pages=%u prtosheap_pages=%u"
               ...
               domain_tot_pages(d), d->prtosheap_pages,
#ifdef CONFIG_MEM_SHARING
...
void __init initialize_keytable(void)
{
    ...
    {
        alt_key_handling = 1;
        printk(PRTOSLOG_INFO "Defaulting to alternative key handling; "
 ...
 *
 * The trace buffer code is designed to allow debugging traces of PRTOS to be
 ...
 * The t_info layout is fixed and cant be changed without breaking prtostrace.
...
static int calculate_tbuf_size(unsigned int pages, uint16_t t_info_first_offset)
{
    ...
    if ( pages > max_pages )
    {
        printk(PRTOSLOG_INFO "prtostrace: requested number of %u pages "
               "reduced to %u\n",
               pages, max_pages);
        pages = max_pages;
    }
    ...
    printk(PRTOSLOG_INFO "prtostrace: requesting %u t_info pages "
 ...
 * the %TRACE_xD macros exported in <prtos_trace.h>.
 *
...
static int alloc_trace_bufs(unsigned int pages)
{
    ...
    t_info = alloc_prtosheap_pages(get_order_from_pages(t_info_pages), 0);
    if ( t_info == NULL )
        goto out_fail;
    ...
    for_each_online_cpu(cpu)
    {
        ...
        for ( i = 0; i < pages; i++ )
        {
            void *p = alloc_prtosheap_pages(0, MEMF_bits(32 + PAGE_SHIFT));
            if ( !p )
            {
                printk(PRTOSLOG_INFO "prtostrace: memory allocation failed "
    ...
    for_each_online_cpu(cpu)
    {
        ...
        printk(PRTOSLOG_INFO "prtostrace: p%d mfn %x offset %u\n",
                   cpu, t_info_mfn_list[offset], offset);

        /* Now share the trace pages */
        for ( i = 0; i < pages; i++ )
            share_prtos_page_with_privileged_guests(
...
out_dealloc:
    for_each_online_cpu(cpu)
    {
        ...
        for ( i = 0; i < pages; i++ )
        {
            ...
            free_prtosheap_pages(mfn_to_virt(mfn), 0);
        }
    }
    free_prtosheap_pages(t_info, get_order_from_pages(t_info_pages));
    t_info = NULL;
out_fail:
    printk(PRTOSLOG_WARNING "prtostrace: allocation failed! Tracing disabled.\n");
    return -ENOMEM;
}
...
static int tb_set_size(unsigned int pages)
{
    ...
    {
        printk(PRTOSLOG_INFO "prtostrace: tb_set_size from %d to %d "
               "not implemented\n",
               opt_tbuf_size, pages);
        return -EINVAL;
    }
...
void __init init_trace_bufs(void)
{
    ...
    if ( opt_tbuf_size )
    {
        ...
        {
            printk("prtostrace: allocation size %d failed, disabling\n",
                   opt_tbuf_size);
            opt_tbuf_size = 0;
        }
        else if ( opt_tevt_mask )
        {
            printk("prtostrace: Starting tracing, enabling mask %x\n",
                   opt_tevt_mask);
 ...
 * @tbc: a pointer to a struct prtos_sysctl_tbuf_op to be filled out
 */
int tb_control(struct prtos_sysctl_tbuf_op *tbc)
{
    static DEFINE_SPINLOCK(lock);
    int rc = 0;
    ...
    switch ( tbc->cmd )
    {
    case PRTOS_SYSCTL_TBUFOP_get_info:
        tbc->evt_mask   = tb_event_mask;
        tbc->buffer_mfn = t_info ? virt_to_mfn(t_info) : 0;
        tbc->size = t_info_pages * PAGE_SIZE;
        break;
    case PRTOS_SYSCTL_TBUFOP_set_cpu_mask:
    {
        cpumask_var_t mask;

        rc = prtosctl_bitmap_to_cpumask(&mask, &tbc->cpu_mask);
        if ( !rc )
        {
        ...
        break;
    case PRTOS_SYSCTL_TBUFOP_set_evt_mask:
        tb_event_mask = tbc->evt_mask;
        break;
    case PRTOS_SYSCTL_TBUFOP_set_size:
        rc = tb_set_size(tbc->size);
        break;
    case PRTOS_SYSCTL_TBUFOP_enable:
        ...
        if ( opt_tbuf_size == 0 )
            ...
            tb_init_done = 1;
        break;
    case PRTOS_SYSCTL_TBUFOP_disable:
    {
        /*
...
static inline bool bogus(u32 prod, u32 cons)
{
    ...
    {
        tb_init_done = 0;
        printk(PRTOSLOG_WARNING "trc#%u: bogus prod (%08x) and/or cons (%08x)\n",
...
void trace(uint32_t event, unsigned int extra, const void *extra_data)
{
    ...
    if ( extra % sizeof(uint32_t) ||
        ...
        return printk_once(PRTOSLOG_WARNING
...
void relax_virtual_region_perms(void)
{
    ...
    {
        modify_prtos_mappings_lite((unsigned long)region->text_start,
                                 (unsigned long)region->text_end,
                                 PAGE_HYPERVISOR_RWX);
        if ( region->rodata_start )
            modify_prtos_mappings_lite((unsigned long)region->rodata_start,
...
static lpae_t *prtos_map_table(mfn_t mfn)
{
    /*
...
static void prtos_unmap_table(const lpae_t *table)
{
    /*
     * During early boot, prtos_map_table() will not use map_domain_page()
     * but the PMAP.
     */
    if ( system_state == SYS_STATE_early_boot )
        pmap_unmap(table);
                  ...
                  unsigned int nr_root_tables)
{
    ...
    mapping = prtos_map_table(mfn_add(root_mfn, root_table));

    for ( level = root_level; ; level++ )
    {
        if ( offsets[level] > PRTOS_PT_LPAE_ENTRIES )
            break;
        ...
        /* For next iteration */
        prtos_unmap_table(mapping);
        mapping = prtos_map_table(lpae_get_mfn(pte));
    }
...
lpae_t mfn_to_prtos_entry(mfn_t mfn, unsigned int attr)
{
    lpae_t e = (lpae_t) {
        .pt = {
    ...
    case MT_DEVICE_nGnRE:
        ...
        break;
    default:
        e.pt.sh = LPAE_SH_INNER;  /* PRTOS mappings are SMP coherent */
        break;
    }
...
void set_fixmap(unsigned int map, mfn_t mfn, unsigned int flags)
{
    ...
    res = map_pages_to_prtos(FIXMAP_ADDR(map), mfn, 1, flags);
    BUG_ON(res != 0);
}
...
void clear_fixmap(unsigned int map)
{
    ...
    res = destroy_prtos_mappings(FIXMAP_ADDR(map), FIXMAP_ADDR(map) + PAGE_SIZE);
    BUG_ON(res != 0);
}
...
static int create_prtos_table(lpae_t *entry)
{
    mfn_t mfn;
    void *p;
    lpae_t pte;
    ...
    p = prtos_map_table(mfn);
    clear_page(p);
    prtos_unmap_table(p);

    pte = mfn_to_prtos_entry(mfn, MT_NORMAL);
    pte.pt.table = 1;
    write_pte(entry, pte);
    /*
     * No ISB here. It is deferred to prtos_pt_update() as the new table
...
#define PRTOS_TABLE_MAP_FAILED 0
#define PRTOS_TABLE_SUPER_PAGE 1
#define PRTOS_TABLE_NORMAL_PAGE 2
 ...
 *
 * Return values:
 *  PRTOS_TABLE_MAP_FAILED: Either read_only was set and the entry
 *  was empty, or allocating a new page failed.
 *  PRTOS_TABLE_NORMAL_PAGE: next level mapped normally
 *  PRTOS_TABLE_SUPER_PAGE: The next entry points to a superpage.
 */
static int prtos_pt_next_level(bool read_only, unsigned int level,
                             lpae_t **table, unsigned int offset)
{
    ...
    {
        if ( read_only )
            return PRTOS_TABLE_MAP_FAILED;

        ret = create_prtos_table(entry);
        if ( ret )
            return PRTOS_TABLE_MAP_FAILED;
    }

    /* The function prtos_pt_next_level is never called at the 3rd level */
    if ( lpae_is_mapping(*entry, level) )
        return PRTOS_TABLE_SUPER_PAGE;

    mfn = lpae_get_mfn(*entry);

    prtos_unmap_table(*table);
    *table = prtos_map_table(mfn);

    return PRTOS_TABLE_NORMAL_PAGE;
}

/* Sanity check of the entry */
static bool prtos_pt_check_entry(lpae_t entry, mfn_t mfn, unsigned int level,
                               unsigned int flags)
{
    ...
    else if ( flags & _PAGE_PRESENT )
    {
         ...
         *
         * Note that the function prtos_pt_update() relies on this
...
static int prtos_pt_update_entry(mfn_t root, unsigned long virt,
                               ...
                               unsigned int flags)
{
    ...
    table = prtos_map_table(root);
    for ( level = HYP_PT_ROOT_LEVEL; level < target; level++ )
    {
        rc = prtos_pt_next_level(read_only, level, &table, offsets[level]);
        if ( rc == PRTOS_TABLE_MAP_FAILED )
        {
            /*
             * We are here because prtos_pt_next_level has failed to map
        ...
        else if ( rc != PRTOS_TABLE_NORMAL_PAGE )
            break;
    }
    ...
    rc = -EINVAL;
    if ( !prtos_pt_check_entry(*entry, mfn, level, flags) )
        goto out;
    ...
    else
    {
        ...
        {
            pte = mfn_to_prtos_entry(mfn, PAGE_AI_MASK(flags));
    ...
    write_pte(entry, pte);
    /*
     * No ISB or TLB flush here. They are deferred to prtos_pt_update()
...
static int prtos_pt_mapping_level(unsigned long vfn, mfn_t mfn, unsigned long nr,
                                unsigned int flags)
{
    unsigned int level;
    unsigned long mask;
                                        ...
                                        unsigned int flags)
{
     ...
     */
    nr_contig = BIT(PRTOS_PT_LEVEL_ORDER(level), UL) * PRTOS_PT_4K_NR_CONTIG;

    if ( (left < nr_contig) || ((mfn_x(mfn) | vfn) & (nr_contig - 1)) )
        return 1;

    return PRTOS_PT_4K_NR_CONTIG;
}

static DEFINE_SPINLOCK(prtos_pt_lock);

static int prtos_pt_update(unsigned long virt,
                         mfn_t mfn,
                         ...
                         unsigned int flags)
{
    ...
    spin_lock(&prtos_pt_lock);

    while ( left )
    {
        unsigned int order, level, nr_contig, new_flags;

        level = prtos_pt_mapping_level(vfn, mfn, left, flags);
        order = PRTOS_PT_LEVEL_ORDER(level);
         ...
         * flags accordingly.
         */
        nr_contig = prtos_pt_check_contig(vfn, mfn, level, left, flags);
        new_flags = flags | ((nr_contig > 1) ? _PAGE_CONTIG : 0);

        for ( ; nr_contig > 0; nr_contig-- )
        {
            rc = prtos_pt_update_entry(root, vfn << PAGE_SHIFT, mfn, level,
                                     new_flags);
            if ( rc )
                break;
     ...
     * as we don't allow mapping replacement (see prtos_pt_check_entry()).
     ...
     */
    if ( !((flags & _PAGE_PRESENT) && !mfn_eq(mfn, INVALID_MFN)) )
        flush_prtos_tlb_range_va(virt, PAGE_SIZE * nr_mfns);
    else
        isb();

    spin_unlock(&prtos_pt_lock);

    return rc;
}

int map_pages_to_prtos(unsigned long virt,
                     mfn_t mfn,
                     unsigned long nr_mfns,
                     unsigned int flags)
{
    return prtos_pt_update(virt, mfn, nr_mfns, flags);
}

int __init populate_pt_range(unsigned long virt, unsigned long nr_mfns)
{
    return prtos_pt_update(virt, INVALID_MFN, nr_mfns, _PAGE_POPULATE);
}

int destroy_prtos_mappings(unsigned long s, unsigned long e)
{
    ASSERT(IS_ALIGNED(s, PAGE_SIZE));
    ASSERT(IS_ALIGNED(e, PAGE_SIZE));
    ASSERT(s <= e);
    return prtos_pt_update(s, INVALID_MFN, (e - s) >> PAGE_SHIFT, 0);
}

int modify_prtos_mappings(unsigned long s, unsigned long e, unsigned int nf)
{
    ASSERT(IS_ALIGNED(s, PAGE_SIZE));
    ASSERT(IS_ALIGNED(e, PAGE_SIZE));
    ASSERT(s <= e);
    return prtos_pt_update(s, INVALID_MFN, (e - s) >> PAGE_SHIFT, nf);
}
...
#define DIRENTRY_NAME_OFF offsetof(struct prtos_hypfs_dirlistentry, name)
#define DIRENTRY_SIZE(name_len) \
    (DIRENTRY_NAME_OFF +        \
     ROUNDUP((name_len) + 1, alignof(struct prtos_hypfs_direntry)))
...
static int hypfs_get_path_user(char *buf,
                               PRTOS_GUEST_HANDLE_PARAM(const_char) uaddr,
                               unsigned long ulen)
{
    if ( ulen > PRTOS_HYPFS_MAX_PATHLEN )
        return -EINVAL;
                                               ...
                                               const char *path)
{
    ...
    for ( ; ; )
    {
        if ( dir->e.type != PRTOS_HYPFS_TYPE_DIR )
            return ERR_PTR(-ENOENT);
...
int hypfs_read_dyndir_id_entry(const struct hypfs_entry_dir *template,
                               ...
                               PRTOS_GUEST_HANDLE_PARAM(void) *uaddr)
{
    struct prtos_hypfs_dirlistentry direntry;
...
static int cf_check hypfs_read_dyndir(
    const struct hypfs_entry *entry, PRTOS_GUEST_HANDLE_PARAM(void) uaddr)
{
    const struct hypfs_dyndir_id *data;
...
static int hypfs_read(const struct hypfs_entry *entry,
                      PRTOS_GUEST_HANDLE_PARAM(void) uaddr, unsigned long ulen)
{
    struct prtos_hypfs_direntry e;
    unsigned int size = entry->funcs->getsize(entry);
    long ret = -EINVAL;
...
int cf_check hypfs_write_leaf(
    struct hypfs_entry_leaf *leaf, PRTOS_GUEST_HANDLE_PARAM(const_void) uaddr,
    unsigned int ulen)
{
    char *buf;
    int ret;
    ...
    if ( e->type != PRTOS_HYPFS_TYPE_STRING &&
         e->type != PRTOS_HYPFS_TYPE_BLOB && ulen != e->funcs->getsize(e) )
        return -EDOM;
    ...
    ret = -EINVAL;
    if ( e->type == PRTOS_HYPFS_TYPE_STRING &&
         e->encoding == PRTOS_HYPFS_ENC_PLAIN &&
...
static int hypfs_write(struct hypfs_entry *entry,
                       PRTOS_GUEST_HANDLE_PARAM(const_void) uaddr,
                       unsigned long ulen)
{
...
long do_hypfs_op(
    unsigned int cmd, PRTOS_GUEST_HANDLE_PARAM(const_char) arg1,
    unsigned long arg2, PRTOS_GUEST_HANDLE_PARAM(void) arg3, unsigned long arg4)
{
    int ret;
    struct hypfs_entry *entry;
    static char path[PRTOS_HYPFS_MAX_PATHLEN];

    if ( xsm_hypfs_op(XSM_PRIV) )
        return -EPERM;

    if ( cmd == PRTOS_HYPFS_OP_get_version )
    {
        if ( !guest_handle_is_null(arg1) || arg2 ||
             !guest_handle_is_null(arg3) || arg4 )
            return -EINVAL;

        return PRTOS_HYPFS_VERSION;
    }

    if ( cmd == PRTOS_HYPFS_OP_write_contents )
        hypfs_write_lock();
    else
        hypfs_read_lock();
    ...
    switch ( cmd )
    {
    case PRTOS_HYPFS_OP_read:
        ret = hypfs_read(entry, arg3, arg4);
        break;

    case PRTOS_HYPFS_OP_write_contents:
