...
static void __init validate_timer_frequency(void) {
     ...
     */
    if (!xen_cpu_khz) panic("Timer frequency is less than 1 KHz\n");
}
...
static void __init preinit_dt_xen_time(void) {
    ...
    if (!timer) panic("Unable to find a compatible timer in the device tree\n");
...
int __init init_xen_time(void) {
    if (acpi_disabled) init_dt_xen_time();
    ...
    printk("Generic Timer IRQ: phys=%u hyp=%u virt=%u Freq: %lu KHz\n", timer_irq[TIMER_PHYS_NONSECURE_PPI], timer_irq[TIMER_HYP_PPI],
           timer_irq[TIMER_VIRT_PPI], xen_cpu_khz);
