/* PRTOS device tree - consolidated */
...
int dt_find_node_by_gpath(PRTOS_GUEST_HANDLE(char) u_path, uint32_t u_plen,
 ...
 * are currently unused outside this file in the context of PRTOS.
 */
...
static bool dt_node_is_pci(const struct dt_device_node *np)
{
    ...
    if ( is_pci )
        printk(PRTOSLOG_WARNING "%s: Missing device_type\n", np->full_name);
                            ...
                            int pna, const char *rprop)
{
    ...
    if ( ranges == NULL )
    {
        printk(PRTOSLOG_ERR "DT: no ranges; cannot translate\n");
        return 1;
    }
    if ( rlen == 0 )
    {
                                  ...
                                  const __be32 *in_addr, const char *rprop)
{
    ...
    {
        printk(PRTOSLOG_ERR "dt_parse: Bad cell count for device %s\n",
                  dev->full_name);
        goto bail;
    }
    ...
    for ( ;; )
    {
        ...
        {
            printk(PRTOSLOG_ERR "dt_parse: Bad cell count for parent %s\n",
                   dev->full_name);
            break;
        }
                      ...
                      void *data)
{
    ...
    if ( ranges == NULL )
    {
        printk(PRTOSLOG_ERR "DT: no ranges; cannot enumerate %s\n",
               dev->full_name);
        return -EINVAL;
    }
...
{
    return (dt_find_property(device, "prtos,passthrough", NULL) != NULL);
    ...
    while ( list < list_end )
    {
        ...
        if ( phandle )
        {
            ...
            if ( cells_name || cur_index == index )
            {
                ...
                if ( !node )
                {
                    printk(PRTOSLOG_ERR "%s: could not find phandle\n",
                           np->full_name);
                    goto err;
                }
            }
            ...
            {
                printk(PRTOSLOG_ERR "%s: arguments longer than property\n",
                       np->full_name);
                goto err;
            }
        }
                                       ...
                                       unsigned long fpsize)
{
    ...
    {
        printk(PRTOSLOG_WARNING "Weird tag at start of node: %x\n", tag);
        return mem;
    }
    *p += 4;
    pathp = (char *)*p;
    ...
    *p = ROUNDUP(*p + l, 4);
    ...
    if ( tag != FDT_END_NODE )
    {
        printk(PRTOSLOG_WARNING "Weird tag at end of node: %x\n", tag);
        return mem;
    }
...
int unflatten_device_tree(const void *fdt, struct dt_device_node **mynodes)
{
    ...
    {
        printk(PRTOSLOG_ERR "Weird tag at end of tree: %08x\n",
                  *((u32 *)start));
        xfree((void *)mem);
        return -EINVAL;
    }
    ...
    {
        printk(PRTOSLOG_ERR "End of tree marker overwritten: %08x\n",
...
int iommu_release_dt_devices(struct domain *d)
{
    ...
    list_for_each_entry_safe(dev, _dev, &hd->dt_devices, domain_list)
    {
        ...
        if ( rc )
        {
            dprintk(PRTOSLOG_ERR, "Failed to deassign %s in domain %u\n",
...
int iommu_do_dt_domctl(struct prtos_domctl *domctl, struct domain *d,
                       PRTOS_GUEST_HANDLE_PARAM(prtos_domctl_t) u_domctl)
{
    ...
    case PRTOS_DOMCTL_test_assign_device:
        ret = -ENODEV;
        if ( domctl->u.assign_device.dev != PRTOS_DOMCTL_DEV_DT )
            break;
        ...
        if ( domctl->cmd == PRTOS_DOMCTL_test_assign_device )
        {
            spin_lock(&dtdevs_lock);

            if ( iommu_dt_device_is_assigned_locked(dev) )
            {
                printk(PRTOSLOG_G_ERR "%s already assigned.\n",
        ...
        if ( ret < 0 )
        {
            printk(PRTOSLOG_G_ERR "Failed to add %s to the IOMMU\n",
                   dt_node_full_name(dev));
            break;
        }
        ...
        if ( ret )
            printk(PRTOSLOG_G_ERR "PRTOS_DOMCTL_assign_dt_device: assign \"%s\""
                   " to dom%u failed (%d)\n",
                                ...
                                paddr_t *size)
{
    ...
    /*
     * PRTOS will truncate the address/size if it is greater than the maximum
                                          ...
                                          u32 address_cells, u32 size_cells)
{
    ...
    if ( fdt_node_check_compatible(fdt, node, "prtos,linux-zimage") == 0 ||
         fdt_node_check_compatible(fdt, node, "multiboot,kernel") == 0 )
        kind = BOOTMOD_KERNEL;
    else if ( fdt_node_check_compatible(fdt, node, "prtos,linux-initrd") == 0 ||
              fdt_node_check_compatible(fdt, node, "multiboot,ramdisk") == 0 )
        kind = BOOTMOD_RAMDISK;
    else if ( fdt_node_check_compatible(fdt, node, "prtos,xsm-policy") == 0 )
        kind = BOOTMOD_XSM;
    ...
    domU = fdt_node_check_compatible(fdt, parent_node, "prtos,domain") == 0;
                                      ...
                                      u32 address_cells, u32 size_cells)
{
    ...
    if ( fdt_get_property(fdt, node, "prtos,static-heap", NULL) )
    {
        int rc;

        printk("Checking for static heap in /chosen\n");

        rc = device_tree_get_meminfo(fdt, node, "prtos,static-heap",
                                      ...
                                      u32 address_cells, u32 size_cells)
{
    ...
    printk("Checking for \"prtos,static-mem\" in domain node\n");

    prop = fdt_get_property(fdt, node, "prtos,static-mem", NULL);
    if ( !prop )
        /* No "prtos,static-mem" present. */
        return 0;

    return device_tree_get_meminfo(fdt, node, "prtos,static-mem", address_cells,
                                  ...
                                  void *data)
{
    ...
    /*
     * If PRTOS has been booted via UEFI, the memory banks are
    ...
    if ( !efi_enabled(EFI_BOOT) &&
        ...
        rc = process_reserved_memory(fdt, node, name, depth,
                                     address_cells, size_cells);
    else if ( depth <= 3 && (device_tree_node_compatible(fdt, node, "prtos,multiboot-module" ) ||
              device_tree_node_compatible(fdt, node, "multiboot,module" )))
        process_multiboot_node(fdt, node, name, address_cells, size_cells);
    else if ( depth == 1 && device_tree_node_matches(fdt, node, "chosen") )
        rc = process_chosen_node(fdt, node, name, address_cells, size_cells);
    else if ( depth == 2 && device_tree_node_compatible(fdt, node, "prtos,domain") )
        rc = process_domain_node(fdt, node, name, address_cells, size_cells);
    else if ( depth <= 3 && device_tree_node_compatible(fdt, node, "prtos,domain-shared-memory-v1") )
...
const __init char *boot_fdt_cmdline(const void *fdt)
{
    ...
    prop = fdt_get_property(fdt, node, "prtos,prtos-bootargs", NULL);
    if ( prop == NULL )
    {
        struct bootcmdline *dom0_cmdline =
            boot_cmdline_find_by_kind(BOOTMOD_KERNEL);

        if (fdt_get_property(fdt, node, "prtos,dom0-bootargs", NULL) ||
                      ...
                      bool need_mapping, const char *devname)
{
    ...
    if ( res )
    {
        printk(PRTOSLOG_ERR "Unable to permit to %pd access to IRQ %u\n", d, irq);
        return res;
    }
    ...
    if ( need_mapping )
    {
        ...
        if ( res < 0 )
        {
            printk(PRTOSLOG_ERR "Unable to map IRQ%u to %pd\n", irq, d);
            return res;
        }
    }
                        ...
                        uint64_t addr, uint64_t len, void *data)
{
    ...
    if ( !mr_data->skip_mapping )
    {
        ...
        if ( res < 0 )
        {
            printk(PRTOSLOG_ERR "Unable to map 0x%"PRIx64
                              ...
                              struct rangeset *irq_ranges)
{
    ...
    for ( i = 0; i < nirq; i++ )
    {
        ...
        if ( res )
        {
            printk(PRTOSLOG_ERR "Unable to retrieve irq %u for %s\n",
        ...
        if ( irq < 0 )
        {
            printk(PRTOSLOG_ERR "Unable to get irq %u for %s\n",
                   i, dt_node_full_name(dev));
            return irq;
        }
                                ...
                                void *data)
{
    ...
    {
        printk(PRTOSLOG_ERR "%s: IRQ%u is not a SPI\n", dt_node_name(dev), irq);
        return -EINVAL;
    }
    ...
    if ( res )
    {
        printk(PRTOSLOG_ERR "%s: Unable to setup IRQ%u to %pd\n",
