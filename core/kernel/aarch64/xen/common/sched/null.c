...
static struct null_private local_null_private_prtos[10];
static int local_null_private_prtos_index = 0;
static int cf_check null_init(struct scheduler *ops)
{
    ...
    // if ( prv == NULL )
    //     return -ENOMEM;
    prv = &local_null_private_prtos[local_null_private_prtos_index++];
    if (local_null_private_prtos_index > 10) {
        printk("######### local_null_private_prtos_index overflow\n");
        return -ENOMEM;
    }
...
static struct null_pcpu local_null_pcpu_prtos[10];
static unsigned int local_null_pcpu_prtos_index = 0;
static void *cf_check null_alloc_pdata(const struct scheduler *ops, int cpu) {
    ...
    npc = &local_null_pcpu_prtos[local_null_pcpu_prtos_index++];
    if (local_null_pcpu_prtos_index > 10) {
        printk("######### local_null_pcpu_prtos_index overflow\n");
        return ERR_PTR(-ENOMEM);
    }
...
static struct null_unit local_null_unit_prtos[10];
static unsigned int local_null_unit_prtos_index = 0;
static void *cf_check null_alloc_udata(const struct scheduler *ops, struct sched_unit *unit, void *dd) {
    struct null_unit *nvc;

    // nvc = xzalloc(struct null_unit);
    // if ( nvc == NULL )
    //     return NULL;
    nvc = &local_null_unit_prtos[local_null_unit_prtos_index++];
    if (local_null_unit_prtos_index > 10) {
        printk("######### local_null_unit_prtos_index overflow\n");
        return NULL;
    }
...
static struct null_dom local_null_dom_prtos[10];
static unsigned int local_null_dom_prtos_index = 0;
    ...
    const struct scheduler *ops, struct domain *d)
{
    ...
    // if ( ndom == NULL )
    //     return ERR_PTR(-ENOMEM);
    ndom = &local_null_dom_prtos[local_null_dom_prtos_index++];
    if (local_null_dom_prtos_index > 10) {
        printk("######### local_null_dom_prtos_index overflow\n");
        return ERR_PTR(-ENOMEM);
    }
