...
void __init pl011_dt_uart_init_tmp(void)
{
    // const char *config = data;
    int res;
    paddr_t addr, size;
    ...
    //     return res;
    // }
    addr = 0x9000000; // Example address, replace with actual
    size = 0x1000; // Example size, replace with actual
    res = platform_get_irq(NULL, 0);
    if ( res < 0 )
    {
        printk("pl011: Unable to retrieve the IRQ\n");
        return -EINVAL;
