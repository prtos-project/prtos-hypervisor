...
prtos_s8_t *trap_to_str[] = {
    [AARCH64_UNDEFINED_INSTR]  = "UNDEFINED_INSTR",
    [AARCH64_SVC_TRAP]         = "SVC_TRAP",
    [AARCH64_PREFETCH_ABORT]   = "PREFETCH_ABORT",
    [AARCH64_DATA_ABORT]       = "DATA_ABORT",
    [AARCH64_IRQ_TRAP]         = "IRQ_TRAP",
    [AARCH64_FIQ_TRAP]         = "FIQ_TRAP",
    [AARCH64_SERROR]           = "SERROR",
    [AARCH64_INSTR_PAGE_FAULT] = "INSTR_PAGE_FAULT",
    [AARCH64_LOAD_PAGE_FAULT]  = "LOAD_PAGE_FAULT",
};
#endif
...
static void aarch64_ipi_handler(cpu_ctxt_t *ctxt, void *data) {
    set_sched_pending();
}

/* Timer IRQ handler - triggers scheduler */
static void aarch64_timer_irq_handler(cpu_ctxt_t *ctxt, void *data) {
    set_sched_pending();
}

void arch_setup_irqs(void) {
    set_irq_handler(1, aarch64_ipi_handler, 0);      /* IRQ 1 = SGI for IPI */
    set_irq_handler(5, aarch64_timer_irq_handler, 0); /* IRQ 5 = timer */
}
...
void fix_stack(cpu_ctxt_t *ctxt, partition_control_table_t *part_ctrl_table, prtos_s32_t irq_nr, prtos_s32_t vector, prtos_s32_t trap) {
    ...
    part_ctrl_table->arch.irq_saved_pc = regs->elr;
    part_ctrl_table->arch.irq_saved_spsr = regs->spsr;
    part_ctrl_table->arch.irq_saved_x0 = regs->regs[0];
    part_ctrl_table->arch.irq_vector = (prtos_u32_t)vector;
    regs->elr = part_ctrl_table->arch.trap_entry;
}
