/* Xen ARM architecture core - consolidated */
...
static __used void init_done(void) {
     ...
     * read-only.
     */
    rc = modify_xen_mappings((unsigned long)&__ro_after_init_start, (unsigned long)&__ro_after_init_end, PAGE_HYPERVISOR_RO);
...
void __init discard_initial_modules(void) {
    ...
    for (i = 0; i < mi->nr_mods; i++) {
        ...
        if (mi->module[i].kind == BOOTMOD_XEN) continue;
...
static void *__init relocate_fdt(paddr_t dtb_paddr, size_t dtb_size) {
    ...
    if (!fdt) panic("Unable to allocate memory for relocating the Device-Tree.\n");
...
static paddr_t __init next_module(paddr_t s, paddr_t *end) {
    ...
    for (i = 0; i < mi->nr_mods; i++) {
        ...
        if (!mi->module[i].size) continue;

        if (mod_s < s) continue;
        if (mod_s > lowest) continue;
        if (mod_s > *end) continue;
        lowest = mod_s;
        *end = min(*end, mod_e);
    }
    return lowest;
...
static void do_trap_hypercall(struct cpu_user_regs *regs, register_t *nr, const union hsr hsr) {
    ...
    if (curr->hcall_preempted) regs->pc -= 4; /* re-execute 'hvc #XEN_HYPERCALL_TAG' */
...
inject_abt:
    gdprintk(XENLOG_DEBUG, "HSR=%#" PRIregister " pc=%#" PRIregister " gva=%#" PRIvaddr " gpa=%#" PRIpaddr "\n", hsr.bits, regs->pc, gva, gpa);
    if (is_data)
...
void asmlinkage do_trap_guest_sync(struct cpu_user_regs *regs) {
    ...
    switch (hsr.ec) {
        ...
        default:
            gprintk(XENLOG_WARNING, "Unknown Guest Trap. HSR=%#" PRIregister " EC=0x%x IL=%x Syndrome=0x%" PRIx32 "\n", hsr.bits, hsr.ec, hsr.len, hsr.iss);
            inject_undef_exception(regs, hsr);
            break;
    }
}
...
static int __apply_alternatives_multi_stop(void *xenmap)
{
    ...
    //     ret = __apply_alternatives(&region, xenmap - (void *)start);
...
{
    // int ret;
    // mfn_t xen_mfn = virt_to_mfn(start);
    // paddr_t xen_size = end - start;
