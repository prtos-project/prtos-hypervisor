/* PRTOS interrupt & GIC - consolidated */
/* === BEGIN INLINED: gic.c === */
#include <prtos_prtos_config.h>
#include <prtos_prtos_config.h>
/* SPDX-License-Identifier: GPL-2.0-or-later */
/*
 * prtos/arch/arm/gic.c
 *
...
/* Program the GIC to route an interrupt to the host (i.e. PRTOS)
 * - needs to be called with desc.lock held
 */
void gic_route_irq_to_prtos(struct irq_desc *desc, unsigned int priority)
{
...
static void __init gic_dt_preinit(void)
{
    ...
    dt_device_set_used_by(node, DOMID_PRTOS);
}
...
/* SPDX-License-Identifier: GPL-2.0-or-later */
/*
 * prtos/arch/arm/gic-v3.c
 *
 * ARM Generic Interrupt Controller support v3 version
 * based on prtos/arch/arm/gic-v2.c and kernel GICv3 driver
 *
...
static void gicv3_do_wait_for_rwp(void __iomem *base)
{
    ...
    if ( timeout )
        dprintk(PRTOSLOG_ERR, "RWP timeout\n");
}
...
static void gicv3_set_irq_type(struct irq_desc *desc, unsigned int type)
{
    ...
    {
        printk(PRTOSLOG_WARNING "GICv3: WARNING: "
...
static int gicv3_enable_redist(void)
{
    ...
    if ( timeout )
    {
        dprintk(PRTOSLOG_ERR, "GICv3: Redist enable RWP timeout\n");
        return 1;
    }
...
static int __init gicv3_populate_rdist(void)
{
     ...
     */
    if ( (mpidr & 0xff) >= 16 )
          dprintk(PRTOSLOG_WARNING, "GICv3:Cluster with more than 16's cpus\n");
    ...
    dprintk(PRTOSLOG_ERR, "GICv3: CPU%d: mpidr 0x%"PRIregister" has no re-distributor!\n",
                                    ...
                                    void *fdt)
{
    ...
    if ( !compatible )
    {
        dprintk(PRTOSLOG_ERR, "Can't find compatible property for the gic node\n");
...
static int __init gicv3_init(void)
{
    ...
    if ( !cpu_has_gicv3 )
    {
        dprintk(PRTOSLOG_ERR, "GICv3: driver requires system register support\n");
        return -ENODEV;
    }
...
void gic_raise_inflight_irq(struct vcpu *v, unsigned int virtual_irq)
{
    ...
    if ( list_empty(&n->lr_queue) )
    {
        ...
        gdprintk(PRTOSLOG_DEBUG, "trying to inject irq=%u into %pv, when it is still lr_pending\n",
                 virtual_irq, v);
#endif
}
    ...
    if ( lr_val.active )
    {
             ...
             test_and_clear_bit(GIC_IRQ_GUEST_QUEUED, &p->status) )
        {
            if ( p->desc == NULL )
            {
                ...
                gdprintk(PRTOSLOG_WARNING, "unable to inject hw irq=%d into %pv: already active in LR%d\n",
                         irq, v, i);
        }
    }
...
#ifdef GIC_DEBUG
        if ( q )
            gdprintk(PRTOSLOG_DEBUG, "trying to inject irq=%d into %pv, when it is already pending in LR%d\n",
                    irq, v, i);
#endif
    }
    else
                        ...
                        void *hcpu)
{
    ...
    case CPU_UP_PREPARE:
        ...
        if ( rc )
            printk(PRTOSLOG_ERR "Unable to allocate local IRQ for CPU%u\n",
                   cpu);
        break;
    }
...
void release_irq(unsigned int irq, const void *dev_id)
{
    ...
    for ( ;; )
    {
        ...
        if ( !action )
        {
            printk(PRTOSLOG_WARNING "Trying to free already-free IRQ %u\n", irq);
...
int setup_irq(unsigned int irq, unsigned int irqflags, struct irqaction *new)
{
    ...
    if ( test_bit(_IRQ_GUEST, &desc->status) )
    {
        ...
        spin_unlock_irqrestore(&desc->lock, flags);
        printk(PRTOSLOG_ERR "ERROR: IRQ %u is already in use by the domain %u\n",
               irq, d->domain_id);
        return -EBUSY;
    }
    ...
    if ( disabled )
    {
        ...
        gic_route_irq_to_prtos(desc, GIC_PRI_IRQ);
                       ...
                       unsigned int irq, const char * devname)
{
    ...
    {
        printk(PRTOSLOG_G_ERR "IRQ can only be routed to an SPI\n");
        return -EINVAL;
    }

    if ( !is_assignable_irq(irq) )
    {
        printk(PRTOSLOG_G_ERR "the IRQ%u is not routable\n", irq);
        return -EINVAL;
    }
    desc = irq_to_desc(irq);
    ...
    {
        printk(PRTOSLOG_G_ERR "IRQ %u has not been configured\n", irq);
        retval = -EIO;
        goto out;
    }
     ...
     *  - If it's the same domain -> PRTOS doesn't need to update the IRQ desc.
    ...
    if ( desc->action != NULL )
    {
        if ( test_bit(_IRQ_GUEST, &desc->status) )
        {
            ...
            if ( d != ad )
            {
                printk(PRTOSLOG_G_ERR "IRQ %u is already used by domain %u\n",
                       irq, ad->domain_id);
                retval = -EBUSY;
            }
        ...
        {
            printk(PRTOSLOG_G_ERR "IRQ %u is already used by PRTOS\n", irq);
            retval = -EBUSY;
        }
        goto out;
    }
...
int static_handle_spi(struct cpu_user_regs *regs, unsigned int irq) {
    ...
    switch (type) {
        case STATIC_IRQ_HYP:
            /* PRTOS internal IRQ: ignore for now */
            break;
...
// #endif
    default:
        printk(PRTOSLOG_G_ERR "d%d: Unknown vGIC version %u\n",
...
void domain_vgic_free(struct domain *d)
{
    ...
    for ( i = 0; i < (d->arch.vgic.nr_spis); i++ )
    {
        ...
        if ( p->desc )
        {
            ...
            if ( ret )
                dprintk(PRTOSLOG_G_WARNING, "d%u: Failed to release virq %u ret = %d\n",
...
bool vgic_migrate_irq(struct vcpu *old, struct vcpu *new, unsigned int irq)
{
    ...
    if ( test_bit(GIC_IRQ_GUEST_MIGRATING, &p->status) )
    {
        gprintk(PRTOSLOG_WARNING, "irq %u migration failed: requested while in progress\n", irq);
        spin_unlock_irqrestore(&old->arch.vgic.lock, flags);
        return false;
    }
                 ...
                 int virq, const struct sgi_target *target)
{
    ...
    case SGI_TARGET_LIST:
        ...
        for_each_set_bit( i, &bitmap, sizeof(target->list) * 8 )
        {
            ...
            {
                gprintk(PRTOSLOG_WARNING, "VGIC: write r=%"PRIregister" \
...
/* SPDX-License-Identifier: GPL-2.0-or-later */
/*
 * prtos/arch/arm/vgic-v3.c
 *
 * ARM Virtual Generic Interrupt Controller v3 support
 * based on prtos/arch/arm/vgic.c
 *
...
bad_width:
    printk(PRTOSLOG_G_ERR "%pv vGICR: bad read width %d r%d offset %#08x\n",
                                          ...
                                          register_t r)
{
    ...
    default:
        printk(PRTOSLOG_G_ERR "%pv: vGICR: unhandled write r%d offset %#08x\n",
               v, dabt.reg, gicr_reg);
        goto write_ignore;
    }
bad_width:
...
bad_width:
    printk(PRTOSLOG_G_ERR "%pv: %s: bad read width %d r%d offset %#08x\n",
...
bad_width:
    printk(PRTOSLOG_G_ERR "%pv: vGICR: SGI: bad read width %d r%d offset %#08x\n",
           v, dabt.size, dabt.reg, gicr_reg);
    return 0;
                                   ...
                                   register_t *r, void *priv)
{
    ...
    default:
        printk(PRTOSLOG_G_ERR "%pv: vGICD: unhandled read r%d offset %#08x\n",
               v, dabt.reg, gicd_reg);
        goto read_as_zero;
    }

bad_width:
    printk(PRTOSLOG_G_ERR "%pv: vGICD: bad read width %d r%d offset %#08x\n",
...
static bool vgic_v3_to_sgi(struct vcpu *v, uint64_t sgir)
{
    ...
    case ICH_SGI_TARGET_OTHERS:
        ...
        break;
    default:
        gprintk(PRTOSLOG_WARNING, "Wrong irq mode in SGI1R_EL1 register\n");
        return false;
    }
                                  ...
                                  bool read)
{
    ...
    {
        gdprintk(PRTOSLOG_WARNING, "Reading SGI1R_EL1 - WO register\n");
        return false;
    }
}
 ...
 * common/softirq.c
 * 
 * Softirqs in PRTOS are only executed in an outermost activation (e.g., never 
