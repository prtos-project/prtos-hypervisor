...
#include <asm/guest.h>
#include <asm_p2m.h>
#include <asm_setup.h> /* for highmem_start only */
#include <asm_generic_paging.h>
#else
...
{
    return false;
    /* This function is not used in the current implementation for PRTOS. */
    struct page_info *pg;
    unsigned int zone;
...
void __init end_boot_allocator(void)
{
    ...
    init_heap_pages(mfn_to_page(_mfn(0x40000)), 0x42000 - 0x40000);
    init_heap_pages(mfn_to_page(_mfn(0x4a000)), 0x13e000 - 0x4a000);
