/* PRTOS domain management - consolidated */
...
#include <prtos_prof.h>
#include <prtos_irq.h>
#include <prtos_argo.h>
#include <asm_p2m.h>
...
/* Private domain structs for DOMID_PRTOS, DOMID_IO, etc. */
struct domain *__read_mostly dom_prtos;
...
static void vcpu_info_reset(struct vcpu *v)
{
    ...
    v->vcpu_info_area.map =
        ((v->vcpu_id < PRTOS_LEGACY_MAX_VCPUS)
...
static int sanitise_domain_config(struct prtos_domctl_createdomain *config)
{
    bool hvm = config->flags & PRTOS_DOMCTL_CDF_hvm;
    bool hap = config->flags & PRTOS_DOMCTL_CDF_hap;
    bool iommu = config->flags & PRTOS_DOMCTL_CDF_iommu;
    bool vpmu = config->flags & PRTOS_DOMCTL_CDF_vpmu;

    if ( config->flags &
         ~(PRTOS_DOMCTL_CDF_hvm | PRTOS_DOMCTL_CDF_hap |
           PRTOS_DOMCTL_CDF_s3_integrity | PRTOS_DOMCTL_CDF_oos_off |
           PRTOS_DOMCTL_CDF_xs_domain | PRTOS_DOMCTL_CDF_iommu |
           PRTOS_DOMCTL_CDF_nested_virt | PRTOS_DOMCTL_CDF_vpmu) )
    {
        dprintk(PRTOSLOG_INFO, "Unknown CDF flags %#x\n", config->flags);
        return -EINVAL;
    }

    if ( config->grant_opts & ~PRTOS_DOMCTL_GRANT_version_mask )
    {
        dprintk(PRTOSLOG_INFO, "Unknown grant options %#x\n", config->grant_opts);
        return -EINVAL;
    }

    if ( config->max_vcpus < 1 )
    {
        dprintk(PRTOSLOG_INFO, "No vCPUS\n");
        return -EINVAL;
    }

    if ( hap && !hvm )
    {
        dprintk(PRTOSLOG_INFO, "HAP requested for non-HVM guest\n");
        return -EINVAL;
    }
    ...
    if ( iommu )
    {
        ...
        {
            dprintk(PRTOSLOG_INFO, "Unknown IOMMU options %#x\n",
                    config->iommu_opts);
            return -EINVAL;
        }
        ...
        if ( !iommu_enabled )
        {
            dprintk(PRTOSLOG_INFO, "IOMMU requested but not available\n");
            return -EINVAL;
        }
    }
    else
    ...
    {
        dprintk(PRTOSLOG_INFO, "vmtrace requested but not available\n");
        return -EINVAL;
    }

    if ( vpmu && !vpmu_is_available )
    {
        dprintk(PRTOSLOG_INFO, "vpmu requested but cannot be enabled this way\n");
        return -EINVAL;
    }
...
struct domain *domain_create(domid_t domid,
                             struct prtos_domctl_createdomain *config,
                             unsigned int flags)
{
    ...
    INIT_PAGE_LIST_HEAD(&d->prtospage_list);
    ...
    /* DOMID_{PRTOS,IO,etc} (other than IDLE) are sufficiently constructed. */
...
{
    /*
     * Initialise our DOMID_PRTOS domain.
     * Any PRTOS-heap pages that we will allow to be mapped will have
     * their domain field set to dom_prtos.
     * Hidden PCI devices will also be associated with this domain
     * (but be [partly] controlled by Dom0 nevertheless).
     */
    dom_prtos = domain_create(DOMID_PRTOS, NULL, 0);
    if ( IS_ERR(dom_prtos) )
        panic("Failed to create d[PRTOS]: %ld\n", PTR_ERR(dom_prtos));

#ifdef CONFIG_HAS_PIRQ
    /* Bound-check values passed via "extra_guest_irqs=". */
    {
        unsigned int n = max(arch_hwdom_irqs(dom_prtos), nr_static_irqs);

        if ( extra_hwdom_irqs > n - nr_static_irqs )
        {
            extra_hwdom_irqs = n - nr_static_irqs;
            printk(PRTOSLOG_WARNING "hwdom IRQs bounded to %u\n", n);
        }
        if ( extra_domU_irqs >
             max(DEFAULT_EXTRA_DOMU_IRQS, n - nr_static_irqs) )
        {
            extra_domU_irqs = n - nr_static_irqs;
            printk(PRTOSLOG_WARNING "domU IRQs bounded to %u\n", n);
        }
    }
#endif
...
static void cf_check complete_domain_destroy(struct rcu_head *head)
{
    ...
    /* Free page used by prtos oprofile buffer. */
#ifdef CONFIG_PRTOSOPROF
    free_prtosoprof_pages(d);
#endif
...
int domain_soft_reset(struct domain *d, bool resuming)
{
    ...
    for_each_vcpu ( d, v )
    {
        set_prtos_guest_handle(runstate_guest(v), NULL);
        unmap_guest_area(v, &v->vcpu_info_area);
        unmap_guest_area(v, &v->runstate_guest_area);
    }
...
bool update_runstate_area(struct vcpu *v)
{
    ...
    if ( map )
    {
        ...
        runstate.state_entry_time |= PRTOS_RUNSTATE_UPDATE;
...
#ifdef PRTOS_HAVE_PV_UPCALL_MASK
...
long common_vcpu_op(int cmd, struct vcpu *v, PRTOS_GUEST_HANDLE_PARAM(void) arg)
{
    long rc = 0;
    struct domain *d = v->domain;
    ...
    case VCPUOP_register_runstate_phys_area:
    {
        ...
        rc = -ENOSYS;
        if ( 0 /* TODO: Dom's PRTOSFEAT_runstate_phys_area setting */ )
            break;
...
/* C(hyp,user), hyp is PRTOS internal name, user is user API name. */
...
int __init parse_arch_dom0_param(const char *s, const char *e)
{
    ...
    if ( !parse_signed_integer("sve", s, e, &val) )
    {
        ...
        if ( (val >= INT_MIN) && (val <= INT_MAX) )
            ...
            printk(PRTOSLOG_INFO "'sve=%lld' value out of range!\n", val);
...
# define D11PRINT(fmt, args...) printk(PRTOSLOG_DEBUG fmt, ##args)
#else
                                   ...
                                   unsigned int order, void *extra)
{
    ...
    if ( res )
    {
        dprintk(PRTOSLOG_ERR, "Failed map pages to DOMU: %d", res);
        return false;
    }
 ...
 *
 * PRTOS may not have a driver for all the host bridges. So we have
                                          ...
                                          const struct dt_device_node *node)
{
    ...
    if ( res < 0 )
    {
        ...
        if ( res < 0 )
        {
            printk(PRTOSLOG_DEBUG "Can't assign PCI segment to %s\n",
                   node->full_name);
            return -FDT_ERR_NOTFOUND;
        }

        segment = res;
        printk(PRTOSLOG_DEBUG "Assigned segment %d to %s\n",
                                          ...
                                          struct membanks *ext_regions)
{
    ...
    for ( i = 0; i < mem->nr_banks; i++ )
    {
        ...
        if ( res )
        {
            printk(PRTOSLOG_ERR "Failed to add: %#"PRIpaddr"->%#"PRIpaddr"\n",
                   start, end);
            goto out;
        }
    }
    ...
    if ( kinfo->gnttab_size )
    {
        ...
        if ( res )
        {
            printk(PRTOSLOG_ERR "Failed to remove: %#"PRIpaddr"->%#"PRIpaddr"\n",
                   start, end);
            goto out;
        }
    }
                                   ...
                                   uint64_t addr, uint64_t len, void *data)
{
    ...
    {
        printk(PRTOSLOG_ERR "%s: [0x%"PRIx64", 0x%"PRIx64"] exceeds the maximum allowed PA width (%u bits)",
                                    ...
                                    struct membanks *ext_regions)
{
    ...
    dt_for_each_device_node( dt_host, np )
    {
        ...
        for ( i = 0; i < naddr; i++ )
        {
            ...
            if ( res )
            {
                printk(PRTOSLOG_ERR "Unable to retrieve address %u for %s\n",
...
{
    const char compat[] =
        "prtos,prtos-" PRTOS_HV_VERSION_STRING "\0"
        "prtos,prtos";
    __be32 *reg, *cells;
    gic_interrupt_t intr;
    int res;
    ...
    /* See linux Documentation/devicetree/bindings/arm/prtos.txt */
    ...
    if ( !opt_ext_regions )
    {
        printk(PRTOSLOG_INFO "%pd: extended regions support is disabled\n", d);
        nr_ext_regions = 0;
    }
    ...
    else
    {
        ...
        if ( res )
            printk(PRTOSLOG_WARNING "%pd: failed to allocate extended regions\n",
                   d);
...
int __init make_cpus_node(const struct domain *d, void *fdt)
{
    ...
    if ( !cpus )
    {
        dprintk(PRTOSLOG_ERR, "Missing /cpus node in the device tree?\n");
        return -ENOENT;
    }
    ...
    if ( !compatible )
    {
        dprintk(PRTOSLOG_ERR, "Can't find cpu in the device tree?\n");
        return -ENOENT;
    }
    ...
    for ( cpu = 0; cpu < d->max_vcpus; cpu++ )
    {
        ...
        {
            printk(PRTOSLOG_ERR "Unable to handle MPIDR AFFINITY 0x%"PRIx64"\n",
                   mpidr_aff);
            return -EINVAL;
        }
...
int __init make_timer_node(const struct kernel_info *kinfo)
{
    ...
    if ( !dev )
    {
        dprintk(PRTOSLOG_ERR, "Missing timer node in the device tree?\n");
        return -FDT_ERR_PRTOS(ENOENT);
    }
    ...
    /*
     * The timer IRQ is emulated by PRTOS.
 ...
 * on ACPI systems, and DomUs started directly from PRTOS based on device
 * tree information.
 */
...
static void __init allocate_memory(struct domain *d, struct kernel_info *kinfo)
{
    ...
    printk(PRTOSLOG_INFO "Allocating mappings totalling %ldMB for %pd:\n",
    ...
    for( i = 0; i < mem->nr_banks; i++ )
    {
        printk(PRTOSLOG_INFO "%pd BANK[%d] %#"PRIpaddr"-%#"PRIpaddr" (%ldMB)\n",
               d,
               i,
               mem->bank[i].start,
...
static int __init handle_passthrough_prop(struct kernel_info *kinfo,
                                          const struct fdt_property *prtos_reg,
                                          const struct fdt_property *prtos_path,
                                          bool prtos_force,
                                          ...
                                          uint32_t size_cells)
{
    ...
    /* prtos,reg specifies where to map the MMIO region */
    cell = (const __be32 *)prtos_reg->data;
    len = fdt32_to_cpu(prtos_reg->len) / ((address_cells * 2 + size_cells) *
                                        sizeof(uint32_t));
    ...
    for ( i = 0; i < len; i++ )
    {
        ...
        if ( res )
        {
            printk(PRTOSLOG_ERR "Unable to permit to dom%d access to"
    ...
    /*
     * If prtos_force, we let the user assign a MMIO region with no
     * associated path.
     */
    if ( prtos_path == NULL )
        return prtos_force ? 0 : -EINVAL;

    /*
     * prtos,path specifies the corresponding node in the host DT.
     * Both interrupt mappings and IOMMU settings are based on it,
     * as they are done based on the corresponding host DT node.
     */
    node = dt_find_node_by_path(prtos_path->data);
    if ( node == NULL )
    {
        printk(PRTOSLOG_ERR "Couldn't find node %s in host_dt!\n",
               prtos_path->data);
        return -EINVAL;
    }
    ...
    /* If prtos_force, we allow assignment of devices without IOMMU protection. */
    if ( prtos_force && !dt_device_is_protected(node) )
        return 0;
                                   ...
                                   bool scan_passthrough_prop)
{
    ...
    const struct fdt_property *prop, *prtos_reg = NULL, *prtos_path = NULL;
    const char *name;
    bool found, prtos_force = false;
          ...
          propoff = fdt_next_property_offset(pfdt, propoff) )
    {
        ...
        {
            if ( dt_prop_cmp("prtos,reg", name) == 0 )
            {
                prtos_reg = prop;
                found = true;
            }
            else if ( dt_prop_cmp("prtos,path", name) == 0 )
            {
                prtos_path = prop;
                found = true;
            }
            else if ( dt_prop_cmp("prtos,force-assign-without-iommu",
                                  name) == 0 )
            {
                prtos_force = true;
                found = true;
        ...
        /*
         * Copy properties other than the ones above: prtos,reg, prtos,path,
         * and prtos,force-assign-without-iommu.
         */
        if ( !found )
        {
    ...
    /*
     * Only handle passthrough properties if both prtos,reg and prtos,path
     * are present, or if prtos,force-assign-without-iommu is specified.
     */
    if ( prtos_reg != NULL && (prtos_path != NULL || prtos_force) )
    {
        res = handle_passthrough_prop(kinfo, prtos_reg, prtos_path, prtos_force,
                                      address_cells, size_cells);
        if ( res < 0 )
        {
            printk(PRTOSLOG_ERR "Failed to assign device to %pd\n", kinfo->d);
            return res;
        }
    }
    else if ( (prtos_path && !prtos_reg) || (prtos_reg && !prtos_path && !prtos_force) )
    {
        printk(PRTOSLOG_ERR "prtos,reg or prtos,path missing for %pd\n",
               kinfo->d);
        return -EINVAL;
    }
...
static int __init check_partial_fdt(void *pfdt, size_t size)
{
    ...
    {
        dprintk(PRTOSLOG_ERR, "Partial FDT is not a valid Flat Device Tree");
        return -EINVAL;
    }
    ...
    if ( res )
    {
        dprintk(PRTOSLOG_ERR, "Failed to check the partial FDT (%d)", res);
        return -EINVAL;
    }

    if ( fdt_totalsize(pfdt) > size )
    {
        dprintk(PRTOSLOG_ERR, "Partial FDT totalsize is too big");
        return -EINVAL;
    }
...
static int __init alloc_prtosstore_evtchn(struct domain *d)
{
                                 ...
                                 const struct dt_device_node *node)
{
    ...
    rc = dt_property_read_u32(node, "prtos,domain-p2m-mem-mb", &p2m_mem_mb);
    /* If prtos,domain-p2m-mem-mb is not specified, use the default value. */
    p2m_pages = rc ?
    ...
    rc = dt_property_read_string(node, "prtos,enhanced", &dom0less_enhanced);
    if ( rc == -EILSEQ ||
         rc == -ENODATA ||
         (rc == 0 && !strcmp(dom0less_enhanced, "enabled")) )
    {
        if ( hardware_domain )
            ...
            panic("At the moment, PRTOSstore support requires dom0 to be present\n");
    }
    else if ( rc == 0 && !strcmp(dom0less_enhanced, "no-prtosstore") )
...
#endif
    if ( !dt_find_property(node, "prtos,static-mem", NULL) )
    ...
    if ( kinfo.dom0less_feature & DOM0LESS_PRTOSSTORE )
    {
        ASSERT(hardware_domain);
        rc = alloc_prtosstore_evtchn(d);
        if ( rc < 0 )
            return rc;
...
void __init create_domUs(void)
{
    ...
    {
        struct domain *d;
        struct prtos_domctl_createdomain d_cfg = {
            .arch.gic_version = PRTOS_DOMCTL_CONFIG_GIC_NATIVE,
            .flags = PRTOS_DOMCTL_CDF_hvm | PRTOS_DOMCTL_CDF_hap,
            /*
            ...
            .grant_opts = PRTOS_DOMCTL_GRANT_version(opt_gnttab_max_version),
        };
        unsigned int flags = 0U;
        uint32_t val;
        int rc;

        if ( !dt_device_is_compatible(node, "prtos,domain") )
            continue;
        ...
        if ( iommu_enabled &&
            ...
            d_cfg.flags |= PRTOS_DOMCTL_CDF_iommu;
        ...
        if ( dt_property_read_u32(node, "max_grant_version", &val) )
            d_cfg.grant_opts = PRTOS_DOMCTL_GRANT_version(val);
...
/* PRTOS scheduler policies removed - PRTOS uses its own cyclic scheduler */
 ...
 *              implements support functionality for the PRTOS scheduler API.
 *
 */
...
/* PRTOS uses its own cyclic scheduler - no PRTOS scheduler policies registered */
    ...
    for_each_domain_in_cpupool ( d, c )
    {
        ...
        for_each_sched_unit ( d, unit )
        {
                 ...
                 cpumask_test_cpu(cpu, unit->cpu_hard_affinity) )
            {
                ...
                printk(PRTOSLOG_DEBUG "Breaking affinity for %pv\n",
                       unit->vcpu_list);
...
static inline
int vcpuaffinity_params_invalid(const struct prtos_domctl_vcpuaffinity *vcpuaff)
{
    return vcpuaff->flags == 0 ||
           ((vcpuaff->flags & PRTOS_VCPUAFFINITY_HARD) &&
            guest_handle_is_null(vcpuaff->cpumap_hard.bitmap)) ||
           ((vcpuaff->flags & PRTOS_VCPUAFFINITY_SOFT) &&
            guest_handle_is_null(vcpuaff->cpumap_soft.bitmap));
}

int vcpu_affinity_domctl(struct domain *d, uint32_t cmd,
                         struct prtos_domctl_vcpuaffinity *vcpuaff)
{
    struct vcpu *v;
    ...
    if ( cmd == PRTOS_DOMCTL_setvcpuaffinity )
    {
        ...
        if ( vcpuaff->flags & PRTOS_VCPUAFFINITY_FORCE )
         ...
         */
        if ( vcpuaff->flags & PRTOS_VCPUAFFINITY_HARD )
        {
            ret = prtosctl_bitmap_to_bitmap(cpumask_bits(new_affinity),
            ...
            ret = cpumask_to_prtosctl_bitmap(&vcpuaff->cpumap_hard, new_affinity);
        }
        if ( vcpuaff->flags & PRTOS_VCPUAFFINITY_SOFT )
        {
            ret = prtosctl_bitmap_to_bitmap(cpumask_bits(new_affinity),
            ...
            ret = cpumask_to_prtosctl_bitmap(&vcpuaff->cpumap_soft, new_affinity);
        }
 ...
 setvcpuaffinity_out:
    ...
    {
        if ( vcpuaff->flags & PRTOS_VCPUAFFINITY_HARD )
            ret = cpumask_to_prtosctl_bitmap(&vcpuaff->cpumap_hard,
                                           unit->cpu_hard_affinity);
        if ( vcpuaff->flags & PRTOS_VCPUAFFINITY_SOFT )
            ret = cpumask_to_prtosctl_bitmap(&vcpuaff->cpumap_soft,
                                           unit->cpu_soft_affinity);
    }
...
ret_t do_sched_op(int cmd, PRTOS_GUEST_HANDLE_PARAM(void) arg)
{
    ret_t ret = 0;
...
long sched_adjust(struct domain *d, struct prtos_domctl_scheduler_op *op)
{
    long ret;
    ...
    switch ( op->cmd )
    {
    case PRTOS_DOMCTL_SCHEDOP_putinfo:
    case PRTOS_DOMCTL_SCHEDOP_getinfo:
    case PRTOS_DOMCTL_SCHEDOP_putvcpuinfo:
    case PRTOS_DOMCTL_SCHEDOP_getvcpuinfo:
        break;
    default:
        return -EINVAL;
    }
...
long sched_adjust_global(struct prtos_sysctl_scheduler_op *op)
{
    struct cpupool *pool;
    int rc;
    ...
    if ( (op->cmd != PRTOS_SYSCTL_SCHEDOP_putinfo) &&
         (op->cmd != PRTOS_SYSCTL_SCHEDOP_getinfo) )
        return -EINVAL;
...
void __init scheduler_init(void)
{
     ...
     * Use the built-in idle scheduler for PRTOS domain infrastructure only.
     */
 ...
 */
int cpupool_do_sysctl(struct prtos_sysctl_cpupool_op *op)
{
    int ret = 0;
    struct cpupool *c;
    ...
    case PRTOS_SYSCTL_CPUPOOL_OP_CREATE:
    {
        unsigned int poolid;

        poolid = (op->cpupool_id == PRTOS_SYSCTL_CPUPOOL_PAR_ANY) ?
    ...
    case PRTOS_SYSCTL_CPUPOOL_OP_DESTROY:
    {
    ...
    case PRTOS_SYSCTL_CPUPOOL_OP_INFO:
    {
        ...
        op->n_dom = c->n_dom;
        ret = cpumask_to_prtosctl_bitmap(&op->cpumap, c->cpu_valid);
        cpupool_put(c);
    }
    break;

    case PRTOS_SYSCTL_CPUPOOL_OP_ADDCPU:
    {
        unsigned int cpu;
        const cpumask_t *cpus;
        ...
        if ( c == NULL )
            goto addcpu_out;
        if ( cpu == PRTOS_SYSCTL_CPUPOOL_PAR_ANY )
        {
    ...
    case PRTOS_SYSCTL_CPUPOOL_OP_RMCPU:
    {
        unsigned int cpu;
    ...
    case PRTOS_SYSCTL_CPUPOOL_OP_MOVEDOMAIN:
    {
        struct domain *d;
    ...
    case PRTOS_SYSCTL_CPUPOOL_OP_FREEINFO:
    {
        ret = cpumask_to_prtosctl_bitmap(
...
static HYPFS_VARSIZE_INIT(cpupool_gran, PRTOS_HYPFS_TYPE_STRING, "sched-gran",
