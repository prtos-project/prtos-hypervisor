    ...
    unsigned long *module_map, const multiboot_info_t *mbi)
{
    ...
    if ( XSM_MAGIC )
    {
        ...
        {
            bootstrap_map(NULL);
            printk(PRTOSLOG_ERR "Error %d initializing XSM policy\n", ret);
            return -EINVAL;
        }
    }
...
#ifdef CONFIG_COMPAT
int compat_xsm_op(PRTOS_GUEST_HANDLE_PARAM(void) op)
{
 ...
 *
 *
 *  This file contains the XSM policy init functions for PRTOS.
 *
 */
...
int __init xsm_dt_policy_init(void **policy_buffer, size_t *policy_size)
{
    ...
    {
        printk(PRTOSLOG_ERR "xsm: Invalid magic for XSM blob\n");
        return -EINVAL;
    }
