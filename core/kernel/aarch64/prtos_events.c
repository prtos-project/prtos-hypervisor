/* PRTOS event channels & hypercalls - consolidated */
...
#define consumer_is_prtos(e) (!!(e)->prtos_consumer)
...
/*
 * The function alloc_unbound_prtos_event_channel() allows an arbitrary
 ...
 */
static prtos_event_channel_notification_t __read_mostly
    prtos_consumers[NR_PRTOS_CONSUMERS];

/* Default notification action: wake up from wait_on_prtos_event_channel(). */
static void cf_check default_prtos_notification_fn(
    struct vcpu *v, unsigned int port)
{
    /* Consumer needs notification only if blocked. */
    if ( test_and_clear_bit(_VPF_blocked_in_prtos, &v->pause_flags) )
        vcpu_wake(v);
}

/*
 * Given a notification function, return the value to stash in
 * the evtchn->prtos_consumer field.
 */
static uint8_t get_prtos_consumer(prtos_event_channel_notification_t fn)
{
    unsigned int i;

    if ( fn == NULL )
        fn = default_prtos_notification_fn;

    for ( i = 0; i < ARRAY_SIZE(prtos_consumers); i++ )
    {
        /* Use cmpxchgptr() in lieu of a global lock. */
        if ( prtos_consumers[i] == NULL )
            cmpxchgptr(&prtos_consumers[i], NULL, fn);
        if ( prtos_consumers[i] == fn )
            break;
    }

    BUG_ON(i >= ARRAY_SIZE(prtos_consumers));
    return i+1;
}

/* Get the notification function for a given PRTOS-bound event channel. */
#define prtos_notification_fn(e) (prtos_consumers[(e)->prtos_consumer-1])
...
void evtchn_free(struct domain *d, struct evtchn *chn)
{
    ...
    if ( consumer_is_prtos(chn) )
    {
        write_atomic(&d->prtos_evtchns, d->prtos_evtchns - 1);
        /* Decrement ->prtos_evtchns /before/ ->active_evtchns. */
        smp_wmb();
    }
    write_atomic(&d->active_evtchns, d->active_evtchns - 1);
...
int evtchn_alloc_unbound(evtchn_alloc_unbound_t *alloc, evtchn_port_t port)
{
    ...
    if ( rc < 0 )
    {
        gdprintk(PRTOSLOG_WARNING, "EVTCHNOP failure: error %d\n", rc);
        goto out;
    }
                            ...
                            evtchn_port_t lport)
{
    ...
    {
        rc = -EINVAL;
        gdprintk(PRTOSLOG_WARNING, "EVTCHNOP failure: %pd, error %d\n", rd, rc);
        goto out;
    }
...
int evtchn_close(struct domain *d1, int port1, bool guest)
{
 ...
 again:
    ...
    /* Guest cannot close a PRTOS-attached event channel. */
    if ( unlikely(consumer_is_prtos(chn1)) && guest )
    {
        rc = -EINVAL;
        goto out;
    }
...
int evtchn_send(struct domain *ld, unsigned int lport)
{
    ...
    /* Guest cannot send via a PRTOS-attached event channel. */
    if ( unlikely(consumer_is_prtos(lchn)) )
    {
        ret = -EINVAL;
        goto out;
    }
    ...
    case ECS_INTERDOMAIN:
        ...
        if ( consumer_is_prtos(rchn) )
        {
            /* Don't keep holding the lock for the call below. */
            prtos_event_channel_notification_t fn = prtos_notification_fn(rchn);
...
int evtchn_bind_vcpu(evtchn_port_t port, unsigned int vcpu_id)
{
    ...
    /* Guest cannot re-bind a PRTOS-attached event channel. */
    if ( unlikely(consumer_is_prtos(chn)) )
    {
        rc = -EINVAL;
        goto out;
    }
...
{
    unsigned int prtos_cnt = read_atomic(&d->prtos_evtchns);

    /*
     * Read ->prtos_evtchns /before/ active_evtchns, to prevent
     * evtchn_reset() exiting its loop early.
     */
    smp_rmb();

    return read_atomic(&d->active_evtchns) > prtos_cnt;
}
...
int evtchn_reset(struct domain *d, bool resuming)
{
    ...
    if ( d->active_evtchns > d->prtos_evtchns )
        rc = -EAGAIN;
...
long do_event_channel_op(int cmd, PRTOS_GUEST_HANDLE_PARAM(void) arg)
{
    int rc;
...
int alloc_unbound_prtos_event_channel(
    struct domain *ld, unsigned int lvcpu, domid_t remote_domid,
    prtos_event_channel_notification_t notification_fn)
{
    struct evtchn *chn;
    int            port, rc;
    ...
    chn->state = ECS_UNBOUND;
    chn->prtos_consumer = get_prtos_consumer(notification_fn);
    ...
    /*
     * Increment ->prtos_evtchns /after/ ->active_evtchns. No explicit
     * barrier needed due to spin-locked region just above.
     */
    write_atomic(&ld->prtos_evtchns, ld->prtos_evtchns + 1);
...
void free_prtos_event_channel(struct domain *d, int port)
{
    if ( !port_is_valid(d, port) )
    {
        /*
...
void notify_via_prtos_event_channel(struct domain *ld, int lport)
{
    struct evtchn *lchn = _evtchn_from_port(ld, lport), *rchn;
    struct domain *rd;
...
static void domain_dump_evtchn_info(struct domain *d)
{
    ...
    for ( port = 1; ; ++port )
    {
        ...
        printk("]: s=%d n=%d x=%d",
               chn->state, chn->notify_vcpu_id, chn->prtos_consumer);
...
static void cf_check evtchn_fifo_init(struct domain *d, struct evtchn *evtchn)
{
    ...
    if ( word && guest_test_bit(d, EVTCHN_FIFO_LINKED, word) )
        gdprintk(PRTOSLOG_WARNING, "domain %d, port %d already on a queue\n",
                                 ...
                                 uint32_t link)
{
    ...
    gdprintk(PRTOSLOG_WARNING, "domain %d, port %d not linked\n",
             d->domain_id, link);
    guest_clear_bit(d, EVTCHN_FIFO_BUSY, word);
    return 1;
}
                                         ...
                                         uint32_t *port, uint32_t *phandle)
{
    ...
    prop = dt_get_property(np, "prtos,evtchn", &len);
    if ( !prop )
    {
        printk(PRTOSLOG_ERR "prtos,evtchn property should not be empty.\n");
        return -EINVAL;
    }

    if ( !len || len < dt_cells_to_size(STATIC_EVTCHN_NODE_SIZE_CELLS) )
    {
        printk(PRTOSLOG_ERR "prtos,evtchn property value is not valid.\n");
        return -EINVAL;
    }
...
static int __init alloc_domain_evtchn(struct dt_device_node *node)
{
    ...
    if ( !dt_device_is_compatible(node, "prtos,evtchn-v1") )
        return 0;
    ...
    {
        printk(PRTOSLOG_ERR "prtos,evtchn property is not setup correctly.\n");
        return -EINVAL;
    }
    ...
    if ( !p1_node )
    {
        printk(PRTOSLOG_ERR "evtchn: evtchn parent node is NULL\n" );
        return -EINVAL;
    }
    ...
    if ( !p2_node )
    {
        printk(PRTOSLOG_ERR "evtchn: remote parent node is NULL\n" );
        return -EINVAL;
    }
    ...
    if ( !d1 || !d2 )
    {
        printk(PRTOSLOG_ERR "evtchn: could not find domains\n" );
        return -EINVAL;
    }
                          ...
                          domid_t  ldomid)
{
    ...
    for ( ; ; )
    {
        ...
        if ( retries++ == 4 )
        {
            gdprintk(PRTOSLOG_WARNING, "Shared grant entry is unstable\n");
            rc = GNTST_general_error;
            goto done;
        }
                          ...
                          domid_t  ldomid)
{
    ...
    else
    {
        if ( unlikely(scombo.flags & GTF_readonly) )
        {
            ...
            gdprintk(PRTOSLOG_WARNING, "Unstable grant readonly flag\n");
            rc = GNTST_general_error;
            goto done;
        }
    }
    ...
    struct gnttab_map_grant_ref *op)
{
    ...
    {
        gdprintk(PRTOSLOG_INFO, "Bad flags in grant map op: %x\n", op->flags);
    ...
    if ( unlikely(paging_mode_external(ld) &&
                  (op->flags & (GNTMAP_device_map|GNTMAP_application_map|
                            GNTMAP_contains_pte))) )
    {
        gdprintk(PRTOSLOG_INFO, "No device mapping in HVM domain\n");
    ...
    {
        gdprintk(PRTOSLOG_INFO, "Could not find domain %d\n", op->dom);
    ...
    {
        rcu_unlock_domain(rd);
        gdprintk(PRTOSLOG_INFO, "Failed to obtain maptrack handle\n");
    ...
    {
        gdprintk(PRTOSLOG_WARNING, "Bad ref %#x for d%d\n",
    ...
    if ( !pg || (owner == dom_io) )
    {
        ...
        {
            gdprintk(PRTOSLOG_WARNING, "HVM guests can't grant map iomem\n");
            rc = GNTST_general_error;
            goto undo_out;
        }
    ...
    could_not_pin:
        if ( !rd->is_dying )
            gdprintk(PRTOSLOG_WARNING, "Could not pin grant frame %#"PRI_mfn"\n",
                     mfn_x(mfn));
        rc = GNTST_general_error;
        goto undo_out;
    }
...
gnttab_map_grant_ref(
    PRTOS_GUEST_HANDLE_PARAM(gnttab_map_grant_ref_t) uop, unsigned int count)
{
    int i;
    struct gnttab_map_grant_ref op;
    ...
    struct gnttab_unmap_common *op)
{
    ...
    {
        gdprintk(PRTOSLOG_INFO, "Bad d%d handle %#x\n",
    ...
    {
        gdprintk(PRTOSLOG_INFO, "Zero flags for d%d handle %#x\n",
                 lgt->domain->domain_id, op->handle);
        op->status = GNTST_bad_handle;
        return;
    }
    ...
    if ( unlikely(ref >= nr_grant_entries(rgt)) )
    {
        gdprintk(PRTOSLOG_WARNING, "Unstable d%d handle %#x\n",
    ...
    {
        gdprintk(PRTOSLOG_WARNING, "Unstable handle %#x\n", op->handle);
        rc = GNTST_bad_handle;
        goto act_release_out;
    }
...
gnttab_unmap_grant_ref(
    PRTOS_GUEST_HANDLE_PARAM(gnttab_unmap_grant_ref_t) uop, unsigned int count)
{
...
gnttab_unmap_and_replace(
    PRTOS_GUEST_HANDLE_PARAM(gnttab_unmap_and_replace_t) uop, unsigned int count)
{
    int i, c, partial_done, done = 0;
    struct gnttab_unmap_and_replace op;
    struct gnttab_unmap_common common[GNTTAB_UNMAP_BATCH_SIZE];
                              ...
                              unsigned int req_nr_frames)
{
    ...
    {
        if ( (gt->status[i] = alloc_prtosheap_page()) == NULL )
            goto status_alloc_failed;
        clear_page(gt->status[i]);
    }
    /* Share the new status frames with the recipient domain */
    for ( i = nr_status_frames(gt); i < req_status_frames; i++ )
        share_prtos_page_with_guest(virt_to_page(gt->status[i]), d, SHARE_rw);
...
status_alloc_failed:
    for ( i = nr_status_frames(gt); i < req_status_frames; i++ )
    {
        free_prtosheap_page(gt->status[i]);
        gt->status[i] = NULL;
    }
    return -ENOMEM;
}
...
gnttab_unpopulate_status_frames(struct domain *d, struct grant_table *gt)
{
    ...
    for ( i = 0; i < nr_status_frames(gt); i++ )
    {
        ...
        if ( pg->count_info & ~PGC_prtos_heap )
        {
            if ( paging_mode_translate(d) )
            {
                gprintk(PRTOSLOG_ERR,
            ...
            else
            {
                ...
                while ( i-- )
                    share_prtos_page_with_guest(virt_to_page(gt->status[i]),
                                              d, SHARE_rw);
            }
            return -EBUSY;
        }
...
gnttab_grow_table(struct domain *d, unsigned int req_nr_frames)
{
    ...
    {
        if ( (gt->active[i] = alloc_prtosheap_page()) == NULL )
    ...
    {
        if ( (gt->shared_raw[i] = alloc_prtosheap_page()) == NULL )
    ...
    for ( i = nr_grant_frames(gt); i < req_nr_frames; i++ )
        share_prtos_page_with_guest(virt_to_page(gt->shared_raw[i]), d, SHARE_rw);
...
shared_alloc_failed:
    for ( i = nr_grant_frames(gt); i < req_nr_frames; i++ )
    {
        free_prtosheap_page(gt->shared_raw[i]);
...
active_alloc_failed:
    for ( i = nr_active_grant_frames(gt);
          i < num_act_frames_from_sha_frames(req_nr_frames); i++ )
    {
        free_prtosheap_page(gt->active[i]);
        gt->active[i] = NULL;
    }
    gdprintk(PRTOSLOG_INFO, "Allocation failure when expanding d%d grant table\n",
             d->domain_id);
                     ...
                     int max_maptrack_frames, unsigned int options)
{
    ...
    unsigned int max_grant_version = options & PRTOS_DOMCTL_GRANT_version_mask;
    int ret = -ENOMEM;

    if ( !max_grant_version )
    {
        dprintk(PRTOSLOG_INFO, "%pd: invalid grant table version 0 requested\n",
                d);
        return -EINVAL;
    }
    if ( max_grant_version > opt_gnttab_max_version )
    {
        dprintk(PRTOSLOG_INFO,
    ...
    {
        dprintk(PRTOSLOG_INFO, "Bad grant table size: %u frames\n", max_grant_frames);
        return -EINVAL;
    }
...
static long
gnttab_setup_table(
    PRTOS_GUEST_HANDLE_PARAM(gnttab_setup_table_t) uop, unsigned int count,
    unsigned int limit_max)
{
    ...
    {
        gdprintk(PRTOSLOG_INFO, "d%d is limited to %u grant-table frames\n",
    ...
    {
        gdprintk(PRTOSLOG_WARNING, "nr_frames for d%d is too large (%u,%u)\n",
    ...
    {
        prtos_pfn_t gmfn = gfn_x(gnttab_shared_gfn(d, gt, i));
...
static long
gnttab_query_size(
    PRTOS_GUEST_HANDLE_PARAM(gnttab_query_size_t) uop, unsigned int count)
{
...
static long
gnttab_transfer(
    PRTOS_GUEST_HANDLE_PARAM(gnttab_transfer_t) uop, unsigned int count)
{
    struct domain *d = current->domain;
    struct domain *e;
    struct page_info *page;
    ...
    for ( i = 0; i < count; i++ )
    {
        ...
        {
            gdprintk(PRTOSLOG_INFO, "error reading req %d/%u\n",
                    i, count);
            return -EFAULT;
        }
        ...
        if ( unlikely(!mfn_valid(mfn)) )
        {
            ...
            put_gfn(d, gop.mfn);
#endif
            gdprintk(PRTOSLOG_INFO, "out-of-range %lx\n", (unsigned long)gop.mfn);
        ...
        {
            gdprintk(PRTOSLOG_INFO,
                     "can't remove GFN %"PRI_prtos_pfn" (MFN %#"PRI_mfn")\n",
                     gop.mfn, mfn_x(mfn));
        ...
        {
            gdprintk(PRTOSLOG_INFO, "can't find d%d\n", gop.domid);
             ...
             unlikely(!(e->tot_pages + 1)) )
        {
            ...
            if ( e->is_dying )
                gdprintk(PRTOSLOG_INFO, "Transferee d%d is dying\n",
                         e->domain_id);
            else
                gdprintk(PRTOSLOG_INFO,
            ...
            unlikely(assign_pages(page, 1, e, MEMF_no_refcount)) )
        {
            ...
            if ( okay /* i.e. e->is_dying due to the surrounding if() */ )
                gdprintk(PRTOSLOG_INFO, "Transferee d%d is now dying\n",
                         e->domain_id);
    ...
    copyback:
        ...
        {
            gdprintk(PRTOSLOG_INFO, "error writing resp %d/%u\n", i, count);
            return -EFAULT;
        }
        guest_handle_add_offset(uop, 1);
    }
    ...
    uint16_t *length, bool allow_transitive)
{
    ...
    *page = NULL;
    ...
    {
        gdprintk(PRTOSLOG_WARNING, "Bad grant reference %#x\n", gref);
        rc = GNTST_bad_gntref;
        goto gt_unlock_out;
    }
                                 ...
                                 unsigned int gref_flag)
{
    ...
    else
    {
        ...
        {
            gdprintk(PRTOSLOG_WARNING,
                     "source frame %"PRI_prtos_pfn" invalid\n", ptr->u.gmfn);
            goto out;
        }
                           ...
                           const struct gnttab_copy_buf *src)
{
    ...
    {
        gdprintk(PRTOSLOG_WARNING, "copy beyond page area\n");
    ...
    {
        gdprintk(PRTOSLOG_WARNING, "copy dest out of bounds: %d < %d || %d > %d\n",
...
static long gnttab_copy(
    PRTOS_GUEST_HANDLE_PARAM(gnttab_copy_t) uop, unsigned int count)
{
    unsigned int i;
...
static long
gnttab_set_version(PRTOS_GUEST_HANDLE_PARAM(gnttab_set_version_t) uop)
{
     ...
     * are allowed to be in use (prtosstore/prtosconsole keeps them mapped).
...
static long
gnttab_get_status_frames(PRTOS_GUEST_HANDLE_PARAM(gnttab_get_status_frames_t) uop,
                         unsigned int count)
{
    ...
    {
        gdprintk(PRTOSLOG_INFO, "Requested addresses of d%d for %u grant "
...
static long
gnttab_get_version(PRTOS_GUEST_HANDLE_PARAM(gnttab_get_version_t) uop)
{
...
swap_grant_ref(grant_ref_t ref_a, grant_ref_t ref_b)
{
    ...
    {
        gdprintk(PRTOSLOG_WARNING, "Bad ref-a %#x\n", ref_a);
        rc = GNTST_bad_gntref;
        goto out;
    }
    if ( unlikely(ref_b >= nr_grant_entries(d->grant_table)))
    {
        gdprintk(PRTOSLOG_WARNING, "Bad ref-b %#x\n", ref_b);
        rc = GNTST_bad_gntref;
        goto out;
    }
    ...
    if ( act_a->pin )
    {
        gdprintk(PRTOSLOG_WARNING, "ref a %#x busy\n", ref_a);
        rc = GNTST_eagain;
        goto out;
    }
    ...
    if ( act_b->pin )
    {
        gdprintk(PRTOSLOG_WARNING, "ref b %#x busy\n", ref_b);
        rc = GNTST_eagain;
        goto out;
    }
...
static long
gnttab_swap_grant_ref(PRTOS_GUEST_HANDLE_PARAM(gnttab_swap_grant_ref_t) uop,
                      unsigned int count)
{
    int i;
...
static long
gnttab_cache_flush(PRTOS_GUEST_HANDLE_PARAM(gnttab_cache_flush_t) uop,
                      grant_ref_t *cur_ref,
                      unsigned int count)
{
    unsigned int i;
...
long do_grant_table_op(
    unsigned int cmd, PRTOS_GUEST_HANDLE_PARAM(void) uop, unsigned int count)
{
    long rc;
    ...
    {
        PRTOS_GUEST_HANDLE_PARAM(gnttab_map_grant_ref_t) map =
    ...
    {
        PRTOS_GUEST_HANDLE_PARAM(gnttab_unmap_grant_ref_t) unmap =
    ...
    {
        PRTOS_GUEST_HANDLE_PARAM(gnttab_unmap_and_replace_t) unmap =
    ...
    {
        PRTOS_GUEST_HANDLE_PARAM(gnttab_transfer_t) transfer =
    ...
    case GNTTABOP_copy:
    {
        PRTOS_GUEST_HANDLE_PARAM(gnttab_copy_t) copy =
    ...
    {
        PRTOS_GUEST_HANDLE_PARAM(gnttab_swap_grant_ref_t) swap =
    ...
    {
        PRTOS_GUEST_HANDLE_PARAM(gnttab_cache_flush_t) cflush =
...
int gnttab_release_mappings(struct domain *d)
{
    ...
    for ( handle = gt->maptrack_limit; handle; )
    {
        ...
        if ( handle < gt->maptrack_limit && !(handle % MAPTRACK_PER_PAGE) )
        {
            ...
            FREE_PRTOSHEAP_PAGE(gt->maptrack[nr_maptrack_frames(gt)]);
        ...
        gdprintk(PRTOSLOG_INFO, "Grant release %#x ref %#x flags %#x d%d\n",
    ...
    gt->maptrack_limit = 0;
    FREE_PRTOSHEAP_PAGE(gt->maptrack[0]);
...
#define WARN_GRANT_MAX 10
    ...
    for ( ref = 0; ref != nr_ents; ref++ )
    {
        ...
        if ( nr_active <= WARN_GRANT_MAX )
            printk(PRTOSLOG_G_DEBUG "d%d has active grant %x ("
#ifndef NDEBUG
                   "GFN %lx, "
#endif
                   "MFN: %#"PRI_mfn")\n",
    ...
    if ( nr_active > WARN_GRANT_MAX )
        printk(PRTOSLOG_G_DEBUG "d%d has too many (%d) active grants to report\n",
    ...
    struct domain *d)
{
    ...
    for ( i = 0; i < nr_grant_frames(t); i++ )
        free_prtosheap_page(t->shared_raw[i]);
    xfree(t->shared_raw);
    ...
    for ( i = 0; i < nr_active_grant_frames(t); i++ )
        free_prtosheap_page(t->active[i]);
    xfree(t->active);

    for ( i = 0; i < nr_status_frames(t); i++ )
        free_prtosheap_page(t->status[i]);
    xfree(t->status);
...
unsigned int gnttab_resource_max_frames(const struct domain *d, unsigned int id)
{
    ...
    switch ( id )
    {
    case PRTOSMEM_resource_grant_table_id_shared:
        nr = gt->max_grant_frames;
        break;

    case PRTOSMEM_resource_grant_table_id_status:
...
int gnttab_map_frame(struct domain *d, unsigned long idx, gfn_t gfn, mfn_t *mfn)
{
    ...
    if ( evaluate_nospec(gt->gt_version == 2) && (idx & PRTOSMAPIDX_grant_table_status) )
    {
        idx &= ~PRTOSMAPIDX_grant_table_status;
        status = true;
...
static int hvm_allow_set_param(const struct domain *d, unsigned int param)
{
    ...
    case HVM_PARAM_MONITOR_RING_PFN:
        ...
        /* Writeable only by PRTOS, hole, deprecated, or out-of-range. */
    default:
        return -EINVAL;
    }
}
...
long do_hvm_op(unsigned long op, PRTOS_GUEST_HANDLE_PARAM(void) arg)
{
    long rc = 0;
    ...
    default:
    {
        gdprintk(PRTOSLOG_DEBUG, "HVMOP op=%lu: not implemented\n", op);
        rc = -ENOSYS;
        break;
    }
    }
...
static int nodemask_to_prtosctl_bitmap(struct prtosctl_bitmap *prtosctl_nodemap,
                                     const nodemask_t *nodemask)
{
    return bitmap_to_prtosctl_bitmap(prtosctl_nodemap, nodemask_bits(nodemask),
                                   MAX_NUMNODES);
}

static int prtosctl_bitmap_to_nodemask(nodemask_t *nodemask,
                                     const struct prtosctl_bitmap *prtosctl_nodemap)
{
    return prtosctl_bitmap_to_bitmap(nodemask_bits(nodemask), prtosctl_nodemap,
                                   MAX_NUMNODES);
}
...
void getdomaininfo(struct domain *d, struct prtos_domctl_getdomaininfo *info)
{
    struct vcpu *v;
    u64 cpu_time = 0;
    int flags = PRTOS_DOMINF_blocked;
    struct vcpu_runstate_info runstate;

    memset(info, 0, sizeof(*info));

    info->domain = d->domain_id;
    info->max_vcpu_id = PRTOS_INVALID_MAX_VCPU_ID;
    ...
    for_each_vcpu ( d, v )
    {
        ...
        if ( !(v->pause_flags & VPF_down) )
        {
            if ( !(v->pause_flags & VPF_blocked) )
                flags &= ~PRTOS_DOMINF_blocked;
            if ( v->is_running )
                flags |= PRTOS_DOMINF_running;
    ...
    info->flags = (info->nr_online_vcpus ? flags : 0) |
        ((d->is_dying == DOMDYING_dead) ? PRTOS_DOMINF_dying     : 0) |
        (d->is_shut_down                ? PRTOS_DOMINF_shutdown  : 0) |
        (d->controller_pause_count > 0  ? PRTOS_DOMINF_paused    : 0) |
        (d->debugger_attached           ? PRTOS_DOMINF_debugged  : 0) |
        (is_prtosstore_domain(d)          ? PRTOS_DOMINF_xs_domain : 0) |
        (is_hvm_domain(d)               ? PRTOS_DOMINF_hvm_guest : 0) |
        d->shutdown_code << PRTOS_DOMINF_shutdownshift;
    ...
    memcpy(info->handle, d->handle, sizeof(prtos_domain_handle_t));
                                      ...
                                      unsigned int nr_vcpus)
{
    ...
    vnuma->vmemrange = xmalloc_array(prtos_vmemrange_t, nr_ranges);
 ...
 */
static struct vnuma_info *vnuma_init(const struct prtos_domctl_vnuma *uinfo,
...
long do_domctl(PRTOS_GUEST_HANDLE_PARAM(prtos_domctl_t) u_domctl)
{
    long ret = 0;
    bool copyback = false;
    struct prtos_domctl curop, *op = &curop;
    struct domain *d;

    if ( copy_from_guest(op, u_domctl, 1) )
        return -EFAULT;

    if ( op->interface_version != PRTOS_DOMCTL_INTERFACE_VERSION )
        return -EACCES;

    switch ( op->cmd )
    {
    case PRTOS_DOMCTL_createdomain:
        d = NULL;
        break;

    case PRTOS_DOMCTL_assign_device:
    case PRTOS_DOMCTL_deassign_device:
        if ( op->domain == DOMID_IO )
        {
            ...
            return -ESRCH;
        fallthrough;
    case PRTOS_DOMCTL_test_assign_device:
    case PRTOS_DOMCTL_vm_event_op:
    ...
    case PRTOS_DOMCTL_setvcpucontext:
    {
    ...
    case PRTOS_DOMCTL_pausedomain:
        ret = -EINVAL;
        if ( d != current->domain )
            ret = domain_pause_by_systemcontroller(d);
        break;

    case PRTOS_DOMCTL_unpausedomain:
        ret = domain_unpause_by_systemcontroller(d);
        break;

    case PRTOS_DOMCTL_resumedomain:
        if ( d == current->domain ) /* no domain_pause() */
            ret = -EINVAL;
        else
            domain_resume(d);
    ...
    case PRTOS_DOMCTL_soft_reset:
    case PRTOS_DOMCTL_soft_reset_cont:
        if ( d == current->domain ) /* no domain_pause() */
        {
            ret = -EINVAL;
            break;
        }
        ret = domain_soft_reset(d, op->cmd == PRTOS_DOMCTL_soft_reset_cont);
        if ( ret == -ERESTART )
        {
            op->cmd = PRTOS_DOMCTL_soft_reset_cont;
    ...
    case PRTOS_DOMCTL_destroydomain:
        ret = domain_kill(d);
        if ( ret == -ERESTART )
            ret = hypercall_create_continuation(
                __HYPERVISOR_domctl, "h", u_domctl);
    ...
    case PRTOS_DOMCTL_setnodeaffinity:
    {
        nodemask_t new_affinity;

        ret = prtosctl_bitmap_to_nodemask(&new_affinity,
    ...
    case PRTOS_DOMCTL_getnodeaffinity:
        ret = nodemask_to_prtosctl_bitmap(&op->u.nodeaffinity.nodemap,
                                        &d->node_affinity);
        break;

    case PRTOS_DOMCTL_setvcpuaffinity:
    case PRTOS_DOMCTL_getvcpuaffinity:
        ret = vcpu_affinity_domctl(d, op->cmd, &op->u.vcpuaffinity);
        break;

    case PRTOS_DOMCTL_scheduler_op:
        ret = sched_adjust(d, &op->u.scheduler_op);
        copyback = 1;
        break;

    case PRTOS_DOMCTL_getdomaininfo:
    ...
    case PRTOS_DOMCTL_getvcpucontext:
    {
        vcpu_guest_context_u c = { .nat = NULL };
        struct vcpu         *v;
    ...
    case PRTOS_DOMCTL_setdomainhandle:
        memcpy(d->handle, op->u.setdomainhandle.handle,
               sizeof(prtos_domain_handle_t));
        break;

    case PRTOS_DOMCTL_setdebugging:
...
#ifdef CONFIG_HAS_PIRQ
    case PRTOS_DOMCTL_irq_permission:
    {
    ...
    case PRTOS_DOMCTL_iomem_permission:
    {
    ...
    case PRTOS_DOMCTL_memory_mapping:
    {
    ...
    case PRTOS_DOMCTL_settimeoffset:
        domain_set_time_offset(d, op->u.settimeoffset.time_offset_seconds);
        break;

    case PRTOS_DOMCTL_set_target:
    {
        struct domain *e;
    ...
    case PRTOS_DOMCTL_set_access_required:
    ...
    case PRTOS_DOMCTL_set_virq_handler:
        ret = set_global_virq_handler(d, op->u.set_virq_handler.virq);
        break;

    case PRTOS_DOMCTL_setvnumainfo:
    {
        struct vnuma_info *vnuma;
    ...
    case PRTOS_DOMCTL_test_assign_device:
    case PRTOS_DOMCTL_deassign_device:
    case PRTOS_DOMCTL_get_device_group:
        ret = iommu_do_domctl(op, d, u_domctl);
        break;

    case PRTOS_DOMCTL_get_paging_mempool_size:
        ret = arch_get_paging_mempool_size(d, &op->u.paging_mempool.size);
        if ( !ret )
            copyback = 1;
        break;

    case PRTOS_DOMCTL_set_paging_mempool_size:
...
void arch_get_domain_info(const struct domain *d,
                          struct prtos_domctl_getdomaininfo *info)
{
    /* All ARM domains use hardware assisted paging. */
    info->flags |= PRTOS_DOMINF_hap;
                             ...
                             struct prtos_domctl_vuart_op *vuart_op)
{
    ...
    if ( vuart_op->type != PRTOS_DOMCTL_VUART_TYPE_VPL011 )
        return -EOPNOTSUPP;
...
long arch_do_domctl(struct prtos_domctl *domctl, struct domain *d,
                    PRTOS_GUEST_HANDLE_PARAM(prtos_domctl_t) u_domctl)
{
    switch ( domctl->cmd )
    {
    case PRTOS_DOMCTL_cacheflush:
    {
    ...
    case PRTOS_DOMCTL_bind_pt_irq:
    {
        int rc;
        struct prtos_domctl_bind_pt_irq *bind = &domctl->u.bind_pt_irq;
    ...
    case PRTOS_DOMCTL_unbind_pt_irq:
    {
        int rc;
        struct prtos_domctl_bind_pt_irq *bind = &domctl->u.bind_pt_irq;
        uint32_t irq = bind->u.spi.spi;
    ...
    case PRTOS_DOMCTL_vuart_op:
    {
        int rc;
        unsigned int i;
        struct prtos_domctl_vuart_op *vuart_op = &domctl->u.vuart_op;
        ...
        switch( vuart_op->cmd )
        {
        case PRTOS_DOMCTL_VUART_OP_INIT:
...
#include <prtos_xsm_xsm.h>
#include <prtos_pmstat.h>
#include <prtos_livepatch.h>
#include <prtos_coverage.h>

long do_sysctl(PRTOS_GUEST_HANDLE_PARAM(prtos_sysctl_t) u_sysctl)
{
    long ret = 0;
    int copyback = -1;
    struct prtos_sysctl curop, *op = &curop;
    static DEFINE_SPINLOCK(sysctl_lock);

    if ( copy_from_guest(op, u_sysctl, 1) )
        return -EFAULT;

    if ( op->interface_version != PRTOS_SYSCTL_INTERFACE_VERSION )
        return -EACCES;
    ...
    case PRTOS_SYSCTL_tbuf_op:
        ret = tb_control(&op->u.tbuf_op);
        break;

    case PRTOS_SYSCTL_sched_id:
        op->u.sched_id.sched_id = scheduler_id();
        break;

    case PRTOS_SYSCTL_getdomaininfolist:
    { 
        struct domain *d;
        struct prtos_domctl_getdomaininfo info;
        u32 num_domains = 0;
    ...
    case PRTOS_SYSCTL_perfc_op:
        ret = perfc_control(&op->u.perfc_op);
        break;
#endif

#ifdef CONFIG_DEBUG_LOCK_PROFILE
    case PRTOS_SYSCTL_lockprof_op:
        ret = spinlock_profile_control(&op->u.lockprof_op);
        break;
#endif
    case PRTOS_SYSCTL_debug_keys:
    {
        char c;
        uint32_t i;
    ...
    case PRTOS_SYSCTL_getcpuinfo:
    {
        uint32_t i, nr_cpus;
        struct prtos_sysctl_cpuinfo cpuinfo = { 0 };
    ...
    case PRTOS_SYSCTL_get_pmstat:
        ret = do_get_pm_info(&op->u.get_pmstat);
        break;

    case PRTOS_SYSCTL_pm_op:
    ...
    case PRTOS_SYSCTL_page_offline_op:
    {
        uint32_t *status, *ptr;
        mfn_t mfn;
        ...
        if ( !status )
        {
            dprintk(PRTOSLOG_WARNING, "Out of memory for page offline op\n");
            ret = -ENOMEM;
            break;
        }
    ...
    case PRTOS_SYSCTL_cpupool_op:
        ret = cpupool_do_sysctl(&op->u.cpupool_op);
        break;

    case PRTOS_SYSCTL_scheduler_op:
        ret = sched_adjust_global(&op->u.scheduler_op);
        break;

    case PRTOS_SYSCTL_physinfo:
    {
        struct prtos_sysctl_physinfo *pi = &op->u.physinfo;
        ...
        pi->scrub_pages = 0;
        pi->prtos_cpu_khz = prtos_cpu_khz;
        pi->max_mfn = get_upper_mfn_bound();
        arch_do_physinfo(pi);
        if ( iommu_enabled )
        {
            pi->capabilities |= PRTOS_SYSCTL_PHYSCAP_directio;
            if ( iommu_hap_pt_share )
                pi->capabilities |= PRTOS_SYSCTL_PHYSCAP_iommu_hap_pt_share;
        }
        if ( vmtrace_available )
            pi->capabilities |= PRTOS_SYSCTL_PHYSCAP_vmtrace;

        if ( vpmu_is_available )
            pi->capabilities |= PRTOS_SYSCTL_PHYSCAP_vpmu;

        if ( opt_gnttab_max_version >= 1 )
            pi->capabilities |= PRTOS_SYSCTL_PHYSCAP_gnttab_v1;
        if ( opt_gnttab_max_version >= 2 )
            pi->capabilities |= PRTOS_SYSCTL_PHYSCAP_gnttab_v2;
    ...
    case PRTOS_SYSCTL_numainfo:
    {
        unsigned int i, j, num_nodes;
        struct prtos_sysctl_numainfo *ni = &op->u.numainfo;
        ...
        {
            struct prtos_sysctl_meminfo meminfo = { };
            ...
            for ( i = 0; i < num_nodes; ++i )
            {
                ...
                if ( do_meminfo )
                {
                    if ( node_online(i) )
                    {
                        ...
                        meminfo.memsize = meminfo.memfree = PRTOS_INVALID_MEM_SZ;
                ...
                if ( do_distance )
                {
                    for ( j = 0; j < num_nodes; j++ )
                    {
                        ...
                        if ( distance[j] == NUMA_NO_DISTANCE )
                            distance[j] = PRTOS_INVALID_NODE_DIST;
                    }
    ...
    case PRTOS_SYSCTL_cputopoinfo:
    {
        unsigned int i, num_cpus;
        struct prtos_sysctl_cputopoinfo *ti = &op->u.cputopoinfo;

        num_cpus = cpumask_last(&cpu_present_map) + 1;
        if ( !guest_handle_is_null(ti->cputopo) )
        {
            struct prtos_sysctl_cputopo cputopo = { };
            ...
            for ( i = 0; i < num_cpus; ++i )
            {
                if ( cpu_present(i) )
                {
                    ...
                    if ( cputopo.node == NUMA_NO_NODE )
                        cputopo.node = PRTOS_INVALID_NODE_ID;
                }
                else
                {
                    cputopo.core = PRTOS_INVALID_CORE_ID;
                    cputopo.socket = PRTOS_INVALID_SOCKET_ID;
                    cputopo.node = PRTOS_INVALID_NODE_ID;
                }
    ...
    case PRTOS_SYSCTL_coverage_op:
        ret = sysctl_cov_op(&op->u.coverage_op);
        copyback = 1;
        break;

#ifdef CONFIG_HAS_PCI
    case PRTOS_SYSCTL_pcitopoinfo:
    {
        struct prtos_sysctl_pcitopoinfo *ti = &op->u.pcitopoinfo;
        unsigned int i = 0;
        ...
        while ( i < ti->num_devs )
        {
            ...
            if ( !pdev )
                node = PRTOS_INVALID_DEV;
            else if ( pdev->node == NUMA_NO_NODE )
                node = PRTOS_INVALID_NODE_ID;
            else
                node = pdev->node;
            pcidevs_unlock();
    ...
    case PRTOS_SYSCTL_livepatch_op:
...
void arch_do_physinfo(struct prtos_sysctl_physinfo *pi)
{
    pi->capabilities |= PRTOS_SYSCTL_PHYSCAP_hvm | PRTOS_SYSCTL_PHYSCAP_hap;

    pi->arch_capabilities |= MASK_INSR(sve_encode_vl(get_sys_vl_len()),
                                       PRTOS_SYSCTL_PHYSCAP_ARM_SVE_MASK);
}

long arch_do_sysctl(struct prtos_sysctl *sysctl,
...
int monitor_domctl(struct domain *d, struct prtos_domctl_monitor_op *mop)
{
    int rc;
    ...
    switch ( mop->op )
    {
    case PRTOS_DOMCTL_MONITOR_OP_ENABLE:
        requested_status = true;
        /* fallthrough */
    case PRTOS_DOMCTL_MONITOR_OP_DISABLE:
    ...
    case PRTOS_DOMCTL_MONITOR_OP_GET_CAPABILITIES:
    ...
    switch ( mop->event )
    {
    case PRTOS_DOMCTL_MONITOR_EVENT_GUEST_REQUEST:
    {
...
#define prtos_mb()   smp_mb()
#define prtos_rmb()  smp_rmb()
#define prtos_wmb()  smp_wmb()

static int vm_event_enable(
    struct domain *d,
    struct prtos_domctl_vm_event_op *vec,
    ...
    prtos_event_channel_notification_t notification_fn)
{
    ...
    rc = alloc_unbound_prtos_event_channel(d, 0, current->domain->domain_id,
                                         notification_fn);
    if ( rc < 0 )
        goto err;

    ved->prtos_port = vec->u.enable.port = rc;
...
static int vm_event_disable(struct domain *d, struct vm_event_domain **p_ved)
{
    ...
    if ( vm_event_check_ring(ved) )
    {
        ...
        free_prtos_event_channel(d, ved->prtos_port);
                          ...
                          vm_event_request_t *req)
{
    ...
    if ( curr->domain != d )
    {
        ...
        if ( !(req->flags & VM_EVENT_FLAG_VCPU_PAUSED) )
            gdprintk(PRTOSLOG_WARNING, "d%dv%d was not paused.\n",
    ...
    notify_via_prtos_event_channel(d, ved->prtos_port);
}
...
static int vm_event_resume(struct domain *d, struct vm_event_domain *ved)
{
    ...
    /*
     * vm_event_resume() runs in either PRTOS_DOMCTL_VM_EVENT_OP_*, or
    ...
    while ( vm_event_get_response(d, ved, &rsp) )
    {
        ...
        {
            printk(PRTOSLOG_G_WARNING "vm_event interface version mismatch\n");
            continue;
        }
...
#ifdef CONFIG_MEM_PAGING
/* Registered with PRTOS-bound event channel for incoming notifications. */
...
int vm_event_domctl(struct domain *d, struct prtos_domctl_vm_event_op *vec)
{
    int rc;

    if ( vec->op == PRTOS_VM_EVENT_GET_VERSION )
    {
    ...
    if ( unlikely(d == current->domain) ) /* no domain_pause() */
    {
        gdprintk(PRTOSLOG_INFO, "Tried to do a memory event op on itself.\n");
        return -EINVAL;
    }

    if ( unlikely(d->is_dying) )
    {
        gdprintk(PRTOSLOG_INFO, "Ignoring memory event op on dying domain %u\n",
                 d->domain_id);
        return 0;
    }
    ...
    {
#ifdef CONFIG_MEM_PAGING
    case PRTOS_DOMCTL_VM_EVENT_OP_PAGING:
    {
        rc = -EINVAL;

        switch( vec->op )
        {
        case PRTOS_VM_EVENT_ENABLE:
        {
            rc = -EOPNOTSUPP;
    ...
    case PRTOS_DOMCTL_VM_EVENT_OP_MONITOR:
    {
        rc = -EINVAL;
...
#ifdef CONFIG_MEM_SHARING
    case PRTOS_DOMCTL_VM_EVENT_OP_SHARING:
    {
        rc = -EINVAL;
...
int arch_monitor_domctl_event(struct domain *d,
                              struct prtos_domctl_monitor_op *mop)
{
    struct arch_domain *ad = &d->arch;
    bool requested_status = (PRTOS_DOMCTL_MONITOR_OP_ENABLE == mop->op);

    switch ( mop->event )
    {
    case PRTOS_DOMCTL_MONITOR_EVENT_PRIVILEGED_CALL:
    {
...
ret_t do_multicall(
    PRTOS_GUEST_HANDLE_PARAM(multicall_entry_t) call_list, uint32_t nr_calls)
{
    struct vcpu *curr = current;
    ...
    {
        gdprintk(PRTOSLOG_INFO, "Multicall reentry is disallowed.\n");
        return -EINVAL;
    }
...
static DEFINE_SPINLOCK(prtospf_lock);

long do_platform_op(PRTOS_GUEST_HANDLE_PARAM(prtos_platform_op_t) u_prtospf_op)
{
    long ret;
    struct prtos_platform_op curop, *op = &curop;
    struct domain *d;

    if ( copy_from_guest(op, u_prtospf_op, 1) )
        return -EFAULT;

    if ( op->interface_version != PRTOSPF_INTERFACE_VERSION )
        return -EACCES;
     ...
     * with this vcpu.
     */
    while ( !spin_trylock(&prtospf_lock) )
        if ( hypercall_preempt_check() )
            return hypercall_create_continuation(
                __HYPERVISOR_platform_op, "h", u_prtospf_op);

    switch ( op->cmd )
    {
    case PRTOSPF_settime64:
...
int do_arm_physdev_op(int cmd, PRTOS_GUEST_HANDLE_PARAM(void) arg)
{
#ifdef CONFIG_HAS_PCI
    return pci_physdev_op(cmd, arg);
#else
    gdprintk(PRTOSLOG_DEBUG, "PHYSDEVOP cmd=%d: not implemented\n", cmd);
    return -ENOSYS;
#endif
}
...
void machine_restart(unsigned int delay_millisecs)
{
    ...
    while ( 1 )
    {
        ...
        mdelay(100);
        if ( (count % 50) == 0 )
            printk(PRTOSLOG_ERR "PRTOS: Platform reset did not work properly!\n");
        count++;
    }
}
                                          ...
                                          register_t context_id)
{
     ...
     * as this simplifies PRTOS implementation.
     */
...
#define PRTOS_SMCCC_FUNCTION_COUNT 3

/* Number of functions currently supported by Standard Service Service Calls. */
#define SSSC_SMCCC_FUNCTION_COUNT (3 + VPSCI_NR_FUNCS + FFA_NR_FUNCS)

static bool fill_uid(struct cpu_user_regs *regs, prtos_uuid_t uuid)
{
    int n;
...
static bool handle_hypervisor(struct cpu_user_regs *regs)
{
    ...
    case ARM_SMCCC_CALL_COUNT_FID(HYPERVISOR):
        return fill_function_call_count(regs, PRTOS_SMCCC_FUNCTION_COUNT);
    case ARM_SMCCC_CALL_UID_FID(HYPERVISOR):
        return fill_uid(regs, PRTOS_SMCCC_UID);
    case ARM_SMCCC_REVISION_FID(HYPERVISOR):
        return fill_revision(regs, PRTOS_SMCCC_MAJOR_REVISION,
                             PRTOS_SMCCC_MINOR_REVISION);
    default:
        return false;
    }
}
...
static bool vsmccc_handle_call(struct cpu_user_regs *regs)
{
    ...
    if ( !handled )
    {
        gprintk(PRTOSLOG_INFO, "Unhandled SMC/HVC: %#x\n", funcid);
...
static int __init psci_is_smc_method(const struct dt_device_node *psci)
{
    ...
    /* Since PRTOS runs in HYP all of the time, it does not make sense to
...
static void __init psci_init_smccc(void)
{
    ...
    printk(PRTOSLOG_INFO "Using SMC Calling Convention v%u.%u\n",
...
int __init psci_init(void)
{
    ...
    printk(PRTOSLOG_INFO "Using PSCI v%u.%u\n",
           PSCI_VERSION_MAJOR(psci_ver), PSCI_VERSION_MINOR(psci_ver));
