    ...
    DYNLIST_FOR_EACH_ELEMENT_BEGIN(l, ktimer, 1) {
        ...
        if (ktimer->flags & KTIMER_ARMED) {
            if (ktimer->value <= current_time) {
                if (ktimer->Action) ktimer->Action(ktimer, ktimer->action_args);
...
prtos_s32_t init_vtimer(int cpu_id, vtimer_t *vtimer, void *k) {
    ...
    init_ktimer(cpu_id, &vtimer->ktimer, vtimer_handle, k, ck);
    return 0;
}
...
prtos_s32_t u_delay(prtos_u32_t usec) {
    prtos_time_t wait_until = (prtos_time_t)usec + get_sys_clock_usec();

    while (wait_until > get_sys_clock_usec())
        ;
    return 0;
}
