...
static inline prtos_u32_t acpi_create_tmp_mapping(prtos_address_t addr) {
    /* On amd64, the boot identity map covers the first 1GB (set up in start.S).
     * ACPI tables are in BIOS/system memory well within this range,
     * so no additional mapping is needed. */
    return 0;
}

static inline void acpi_clear_tmp_mapping(prtos_address_t addr) {
    /* No-op: identity map is persistent on amd64 */
}
...
prtos_address_t __VBOOT acpi_scan(const char *signature, prtos_address_t ebda_start, prtos_address_t ebda_end) {
    prtos_address_t addr;

    for (addr = ebda_start; addr < ebda_end; addr += 16) {
...
static void __VBOOT acpi_parse_madt_table(struct acpi_madt *madt) {
    prtos_address_t madt_end, entry;
    struct acpi_madt_ics *ics;

    madt_end = (prtos_address_t)madt + madt->header.length;
    entry = (prtos_address_t)madt + sizeof(struct acpi_madt);
    while (entry < madt_end) {
        ics = (struct acpi_madt_ics *)entry;
        if (ics->length == 0) break;
...
static void __VBOOT acpi_parse_mp(struct acpi_rsdp *rsdp) {
    ...
    rsdt = (struct acpi_rsdt *)(unsigned long)rsdp->rsdt_addr;
    header = &rsdt->header;
    entries = (header->length - sizeof(struct acpi_header)) >> 2;
    for (e = 0; e < entries; ++e) {
        header = (struct acpi_header *)(unsigned long)rsdt->entry[e];
