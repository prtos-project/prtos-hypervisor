...
/* Use .quad (8-byte) for function pointers on amd64 */
#undef HYPERCALLR_TAB
#undef HYPERCALL_TAB
...
static inline prtos_s32_t is_gdt_desc_valid(struct x86_desc *desc, prtos_u32_t *type) {
    ...
    *type = EMPTY_SEG;
    ...
    base = (prtos_u64_t)(unsigned long)get_desc_base(desc);
...
__hypercall prtos_s32_t amd64_load_cr0_sys(prtos_word_t val) {
...
__hypercall prtos_s32_t amd64_load_cr3_sys(prtos_word_t val) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();
    ...
    if (old_cr3 == val) {
        ...
        if (new_cr3_page->type != PPAG_PTDL1) {
            PWARN("Page %llx is not page_table_directory_level_1\n", (prtos_u64_t)(val & PAGE_MASK));
...
__hypercall prtos_s32_t amd64_load_cr4_sys(prtos_word_t val) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();

    info->sched.current_kthread->ctrl.g->part_ctrl_table->arch.cr4 = val;
    val |= _CR4_PAE | _CR4_PGE | _CR4_PSE;
...
__hypercall prtos_s32_t amd64_load_tss_sys(struct x86_tss *__g_param t) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();

    if (check_gp_param(t, sizeof(struct x86_tss), 8, PFLAG_NOT_NULL) < 0) return PRTOS_INVALID_PARAM;

    info->sched.current_kthread->ctrl.g->karch.tss.t.rsp0 = t->rsp0;
    info->sched.current_kthread->ctrl.g->karch.tss.t.rsp1 = t->rsp1;
    info->sched.current_kthread->ctrl.g->karch.tss.t.rsp2 = t->rsp2;
...
__hypercall prtos_s32_t amd64_load_gdt_sys(struct x86_desc_reg *__g_param desc) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();
...
__hypercall prtos_s32_t amd64_load_idtr_sys(struct x86_desc_reg *__g_param desc) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();
...
__hypercall prtos_s32_t amd64_update_ss_sp_sys(prtos_word_t ss, prtos_word_t sp, prtos_u32_t level) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();
    ...
    switch (level) {
        case 0:
            info->sched.current_kthread->ctrl.g->karch.tss.t.rsp0 = sp;
            break;
        case 1:
            info->sched.current_kthread->ctrl.g->karch.tss.t.rsp1 = sp;
            break;
        case 2:
            info->sched.current_kthread->ctrl.g->karch.tss.t.rsp2 = sp;
            break;
        default:
            return PRTOS_INVALID_PARAM;
    }
...
__hypercall prtos_s32_t amd64_update_gdt_sys(prtos_s32_t entry, struct x86_desc *__g_param gdt) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();
    prtos_u32_t type;
...
static inline prtos_s32_t is_gate_desc_valid(struct x86_gate *desc) {
    ...
    if ((desc->access & 0x0f) == 0x0f) { /* Trap gate */
        base = ((prtos_u64_t)desc->offset63_32 << 32) | ((prtos_u64_t)desc->offset31_16 << 16) | desc->offset15_0;
        if (base >= CONFIG_PRTOS_OFFSET) {
            kprintf("[gate_desc] Base (0x%llx) > PRTOS_OFFSET\n", base);
            return 0;
        }
        seg = desc->seg_selector;
        if (!(seg & 0x3)) {
...
__hypercall prtos_s32_t amd64_update_idt_sys(prtos_s32_t entry, struct x86_gate *__g_param desc) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();

    if (check_gp_param(desc, sizeof(struct x86_gate), 8, PFLAG_NOT_NULL) < 0) return PRTOS_INVALID_PARAM;
    if (entry < FIRST_USER_IRQ_VECTOR || entry >= IDT_ENTRIES) return PRTOS_INVALID_PARAM;

    if ((desc->access & 0x80) && !is_gate_desc_valid(desc)) return PRTOS_INVALID_PARAM;
...
__hypercall void amd64_set_if_sys(void) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();
    info->sched.current_kthread->ctrl.g->part_ctrl_table->iflags |= _CPU_FLAG_IF;
}

__hypercall void amd64_clear_if_sys(void) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();
    info->sched.current_kthread->ctrl.g->part_ctrl_table->iflags &= ~_CPU_FLAG_IF;
}

__hypercall void amd64_iret_sys(void) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();
    ...
    /* In long mode, iret always pops SS:RSP */
...
HYPERCALLR_TAB(amd64_load_cr0_sys, 1);      // 33
HYPERCALLR_TAB(amd64_load_cr3_sys, 1);      // 34
HYPERCALLR_TAB(amd64_load_cr4_sys, 1);      // 35
HYPERCALLR_TAB(amd64_load_tss_sys, 1);      // 36
HYPERCALLR_TAB(amd64_load_gdt_sys, 1);      // 37
HYPERCALLR_TAB(amd64_load_idtr_sys, 1);     // 38
HYPERCALLR_TAB(amd64_update_ss_sp_sys, 3);  // 39
HYPERCALLR_TAB(amd64_update_gdt_sys, 2);    // 40
HYPERCALLR_TAB(amd64_update_idt_sys, 2);    // 41
HYPERCALL_TAB(amd64_set_if_sys, 0);         // 42
HYPERCALL_TAB(amd64_clear_if_sys, 0);       // 43
