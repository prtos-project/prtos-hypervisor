...
partition_t *create_partition(struct prtos_conf_part *cfg) {
    ...
    pct_size = sizeof(partition_control_table_t) + sizeof(struct prtos_physical_mem_map) * cfg->num_of_physical_memory_areas + (cfg->num_of_ports >> PRTOS_LOG2_WORD_SZ);
    ...
    for (i = 0; i < cfg->num_of_vcpus; i++) {
        ...
        ck = (prtos_conf_table.hpv.cpu_table[cpu_id].sched_policy == CYCLIC_SCHED) ? k : 0;

        init_ktimer(cpu_id, &k->ctrl.g->ktimer, kthread_timer_handle, k, ck);
        init_ktimer(cpu_id, &k->ctrl.g->watchdog_timer, kthread_watchdog_timer_handle, k, ck);
...
static inline void setup_pct(partition_control_table_t *part_ctrl_table, kthread_t *k, struct prtos_conf_part *cfg) {
    ...
    part_ctrl_table->part_ctrl_table_size = sizeof(partition_control_table_t) + part_ctrl_table->num_of_physical_mem_areas * sizeof(struct prtos_physical_mem_map);
