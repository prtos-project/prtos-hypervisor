...
static void kthread_watchdog_timer_handle(ktimer_t *ktimer, void *args) {
    kthread_t *k = (kthread_t *)args;

    CHECK_KTHR_SANITY(k);
    set_ext_irq_pending(k, PRTOS_VT_EXT_WATCHDOG_TIMER);
    raise_hm_part_event(PRTOS_HM_EV_WATCHDOG_TIMER, KID2PARTID(k->ctrl.g->id), KID2VCPUID(k->ctrl.g->id), 0);
}
...
void setup_kthreads(void) {
    ...
    ASSERT(GET_CPU_ID() == 0);
    for (e = 0, num_of_vcpus = 0; e < prtos_conf_table.num_of_partitions; e++) num_of_vcpus += prtos_conf_part_table[e].num_of_vcpus;
}
...
void init_idle(kthread_t *idle, prtos_s32_t cpu) {
    ...
    idle->ctrl.kstack = 0;
    idle->ctrl.g = 0;
    idle->ctrl.irqMask = hw_irq_get_mask() | info->cpu.global_irq_mask;
...
partition_t *create_partition(struct prtos_conf_part *cfg) {
    ...
    pct_size = sizeof(part_ctl_table_t) + sizeof(struct prtos_physical_mem_map) * cfg->num_of_physical_memory_areas + (cfg->noPorts >> PRTOS_LOG2_WORD_SZ);

    if (cfg->noPorts & ((1 << PRTOS_LOG2_WORD_SZ) - 1)) pct_size += sizeof(prtos_word_t);

    GET_MEMAZ(pct, pct_size * cfg->num_of_vcpus, PAGE_SIZE);
    p->pctArraySize = pct_size * cfg->num_of_vcpus;
    p->pctArray = (prtos_address_t)pct;
#ifdef PRTOS_VERBOSE
    kprintf("File: [%s] Line[%d]\n", __FILE__, __LINE__);
    kprintf("pctArraySize: [%d]\n", p->pctArraySize);
    kprintf("cfg->num_of_vcpus: [%d]\n", cfg->num_of_vcpus);
#endif
    for (i = 0; i < cfg->num_of_vcpus; i++) {
        ...
        init_ktimer(cpu_id, &k->ctrl.g->ktimer, kthread_timer_handle, k, scK);
        init_ktimer(cpu_id, &k->ctrl.g->watchdogTimer, kthread_watchdog_timer_handle, k, scK);
        ...
        k->ctrl.irqMask = local_irq_mask;
        k->ctrl.g->part_ctrl_table = (part_ctl_table_t *)(pct + pct_size * i);
...
static inline void setup_pct(part_ctl_table_t *part_ctrl_table, kthread_t *k, struct prtos_conf_part *cfg) {
    struct prtos_physical_mem_map *mem_map = (struct prtos_physical_mem_map *)((prtos_address_t)part_ctrl_table + sizeof(part_ctl_table_t));
    ...
    memset(part_ctrl_table, 0, sizeof(part_ctl_table_t));
    ...
    part_ctrl_table->sched_policy =
        prtos_conf_table.hpv.cpu_table[prtos_conf_vcpu_table[(KID2PARTID(k->ctrl.g->id) * prtos_conf_table.hpv.num_of_cpus) + KID2VCPUID(k->ctrl.g->id)].cpu]
            .sched_policy;
    ...
    part_ctrl_table->part_ctrl_table_size = sizeof(part_ctl_table_t) + part_ctrl_table->num_of_physical_mem_areas * sizeof(struct prtos_physical_mem_map);
    ...
    prtos_clear_bitmap(comm_port_bitmap, cfg->noPorts);
    part_ctrl_table->num_of_comm_ports = cfg->noPorts;
...
prtos_s32_t reset_partition(partition_t *p, prtos_u32_t cold, prtos_u32_t status) {
    ...
    prtos_conf_boot_part = &prtos_conf_boot_part_table[p->cfg->id];
    ...
    switch (cold) {
        case PRTOS_WARM_RESET: /*WARM RESET*/
            ...
            reset_part_ports(p);
            reset_kthread(p->kthread[0], ptd_level_1, prtos_conf_boot_part_table[p->cfg->id].entry_point, status);
            break;
