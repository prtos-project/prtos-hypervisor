...
#define BATCH_GET_PARAM(_addr, _arg) *(prtos_u32_t *)((_addr) + sizeof(prtos_u32_t) * (2 + (_arg)))
    ...
    for (addr = (prtos_address_t)start_addr; addr < (prtos_address_t)end_addr;) {
        ...
        *(prtos_u32_t *)(addr + sizeof(prtos_u32_t)) |=
            hypercalls_table[hypercall_nr](BATCH_GET_PARAM(addr, 0), BATCH_GET_PARAM(addr, 1), BATCH_GET_PARAM(addr, 2), BATCH_GET_PARAM(addr, 3),
...
__hypercall prtos_s32_t halt_part_sys(prtos_id_t partition_id) {
    ...
    if (partition_id != KID2PARTID(info->sched.current_kthread->ctrl.g->id)) {
        ...
        for (e = 0; e < partition_table[partition_id].cfg->num_of_vcpus; e++)
            if (!are_kthread_flags_set(partition_table[partition_id].kthread[e], KTHREAD_HALTED_F)) break;

        if (e >= partition_table[partition_id].cfg->num_of_vcpus) return PRTOS_NO_ACTION;
...
__hypercall prtos_s32_t switch_sched_plan_sys(prtos_u32_t new_plan_id, prtos_u32_t *__g_param current_plan_id) {
    ...
    // Plan zero is the initial plan and cannot be called back (it can only be activated by means of a system reset).
    // Thus, the change to plan zero is expected to fail. The execution never ends.
    if ((new_plan_id < 1) || (new_plan_id >= prtos_conf_table.hpv.cpu_table[GET_CPU_ID()].num_of_sched_cyclic_plans)) return PRTOS_INVALID_PARAM;
...
__hypercall prtos_s32_t suspend_part_sys(prtos_id_t partition_id) {
    ...
    if (partition_id != KID2PARTID(info->sched.current_kthread->ctrl.g->id)) {
        ...
        for (e = 0; e < partition_table[partition_id].cfg->num_of_vcpus; e++)
            if (!are_kthread_flags_set(partition_table[partition_id].kthread[e], KTHREAD_SUSPENDED_F | KTHREAD_HALTED_F)) break;
...
__hypercall prtos_s32_t resume_part_sys(prtos_id_t partition_id) {
    ...
    for (e = 0; e < partition_table[partition_id].cfg->num_of_vcpus; e++)
        if (are_kthread_flags_set(partition_table[partition_id].kthread[e], KTHREAD_SUSPENDED_F) &&
            !are_kthread_flags_set(partition_table[partition_id].kthread[e], KTHREAD_HALTED_F))
            break;
...
__hypercall prtos_s32_t reset_partition_sys(prtos_id_t partition_id, prtos_u32_t reset_mode, prtos_u32_t status) {
    ...
    if ((reset_mode == PRTOS_WARM_RESET) || (reset_mode == PRTOS_COLD_RESET)) {
        if (!reset_partition(&partition_table[partition_id], reset_mode & PRTOS_RESET_MODE, status))
            return PRTOS_OK;
        else
...
__hypercall prtos_s32_t set_timer_sys(prtos_u32_t clock_id, prtos_time_t abs_stime, prtos_time_t interval) {
    local_processor_t *info = GET_LOCAL_PROCESSOR();
    prtos_s32_t ret = PRTOS_OK;

    ASSERT(!hw_is_sti());

    if ((abs_stime < 0) || (interval < 0)) return PRTOS_INVALID_PARAM;
    ...
    if (!abs_stime) {
        switch (clock_id) {
            ...
            case PRTOS_WATCHDOG_TIMER:
                disarm_ktimer(&info->sched.current_kthread->ctrl.g->watchdog_timer);
                return PRTOS_OK;
            default:
                return PRTOS_INVALID_PARAM;
        }
    ...
    switch (clock_id) {
        case PRTOS_HW_CLOCK:
            ret = arm_ktimer(&info->sched.current_kthread->ctrl.g->ktimer, abs_stime, interval);
            break;
        case PRTOS_EXEC_CLOCK:
            ret = arm_vtimer(&info->sched.current_kthread->ctrl.g->vtimer, &info->sched.current_kthread->ctrl.g->vclock, abs_stime, interval);
            break;
        case PRTOS_WATCHDOG_TIMER:
            ret = arm_ktimer(&info->sched.current_kthread->ctrl.g->watchdog_timer, abs_stime, interval);
            break;
        default:
            return PRTOS_INVALID_PARAM;
    }
...
__hypercall prtos_s32_t raise_part_ipvi_sys(prtos_id_t partition_id, prtos_u8_t ipvi_number) {
    ...
    for (e = 0; e < ipvi->num_of_dsts; e++) {
        if (partition_id == prtos_conf_dst_ipvi[ipvi->dst_offset + e]) {
            partition_t *p = &partition_table[prtos_conf_dst_ipvi[ipvi->dst_offset + e]];
    ...
    switch (entity) {
        case PRTOS_PARTITION_NAME:
            for (e = 0; e < prtos_conf_table.num_of_partitions; e++)
                if (!strncmp(&prtos_conf_string_tab[prtos_conf_partition_table[e].name_offset], name, CONFIG_ID_STRING_LENGTH)) {
                    id = prtos_conf_partition_table[e].id;
                    break;
                }
            break;
...
void audit_hcall(prtos_u32_t hyp_number, ...) {
    ...
    if (is_audit_event_masked(TRACE_HCALLS_MODULE)) {
        ...
        arg_list[0] = (hyp_number << 16) | num_of_args;
        va_start(arg_ptr, hyp_number);
        for (e = 0; e < num_of_args; e++) arg_list[e + 1] = va_arg(arg_ptr, prtos_u32_t);
        va_end(arg_ptr);
        if (num_of_args < PRTOSTRACE_PAYLOAD_LENGTH)
            raise_audit_event(TRACE_HCALLS_MODULE, AUDIT_HCALL_BEGIN, num_of_args + 1, arg_list);
        else {
            raise_audit_event(TRACE_HCALLS_MODULE, AUDIT_HCALL_BEGIN, PRTOSTRACE_PAYLOAD_LENGTH, arg_list);
            raise_audit_event(TRACE_HCALLS_MODULE, AUDIT_HCALL_BEGIN2, num_of_args - (PRTOSTRACE_PAYLOAD_LENGTH - 1), &arg_list[3]);
        }
    }
}
