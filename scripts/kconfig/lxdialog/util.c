...
void dialog_clear(void) {
    ...
    if (dlg.backtitle != NULL) {
        ...
        wmove(stdscr, 1, 1);
        for (i = 1; i < COLS - 1; i++) waddch(stdscr, ACS_HLINE);
    }
    wnoutrefresh(stdscr);
}
...
void print_autowrap(WINDOW *win, const char *prompt, int width, int y, int x) {
    ...
    if (prompt_len <= width - x * 2) { /* If prompt is short */
        ...
        while (word && *word) {
            ...
            room = width - cur_x;
            wlen = strlen(word);
            if (wlen > room || (newl && wlen < 4 && sp && wlen + 1 + strlen(sp) > room && (!(sp2 = strchr(sp, ' ')) || wlen + 1 + (sp2 - sp) > room))) {
                cur_y++;
                cur_x = x;
            }
...
void draw_shadow(WINDOW *win, int y, int x, int height, int width) {
    ...
    if (has_colors()) { /* Whether terminal supports color? */
        ...
        for (i = 0; i < width; i++) waddch(win, winch(win) & A_CHARTEXT);
