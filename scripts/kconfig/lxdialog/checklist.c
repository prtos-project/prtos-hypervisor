...
static void print_item(WINDOW *win, int choice, int selected) {
    ...
    wmove(win, choice, 0);
    for (i = 0; i < list_width; i++) waddch(win, ' ');
...
int dialog_checklist(const char *title, const char *prompt, int height, int width, int list_height) {
    ...
    item_foreach() {
        if (item_is_tag('X')) choice = item_n();
        if (item_is_selected()) {
            choice = item_n();
            break;
        }
...
do_resize:
    if (getmaxy(stdscr) < (height + 6)) return -ERRDISPLAYTOOSMALL;
    if (getmaxx(stdscr) < (width + 6)) return -ERRDISPLAYTOOSMALL;
    ...
    while (key != KEY_ESC) {
        ...
        for (i = 0; i < max_choice; i++) {
            item_set(i + scroll);
            if (toupper(key) == toupper(item_str()[0])) break;
        }
        ...
        if (i < max_choice || key == KEY_UP || key == KEY_DOWN || key == '+' || key == '-') {
            if (key == KEY_UP || key == '-') {
                ...
                if (choice == max_choice - 1) {
                    if (scroll + choice >= item_count() - 1) continue;
                    /* Scroll list up */
                    if (list_height > 1) {
