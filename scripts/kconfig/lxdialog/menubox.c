...
static void do_print_item(WINDOW *win, const char *item, int line_y, int selected, int hotkey) {
    ...
    {
        int i;
        for (i = 0; i < menu_width; i++) waddch(win, ' ');
    }
#else
    wclrtoeol(win);
#endif
...
do_resize:
    ...
    if (height < 15 || width < 65) return -ERRDISPLAYTOOSMALL;
    ...
    while (key != KEY_ESC) {
        ...
        if (strchr("ynmh", key))
            ...
            for (i = choice + 1; i < max_choice; i++) {
                ...
                if (key == tolower(item_str()[j])) break;
            }
            if (i == max_choice)
                for (i = 0; i < max_choice; i++) {
                    item_set(scroll + i);
        ...
        if (i < max_choice || key == KEY_UP || key == KEY_DOWN || key == '-' || key == '+' || key == KEY_PPAGE || key == KEY_NPAGE) {
