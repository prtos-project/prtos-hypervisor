# RISC-V 64 Platform — API Reference
...
## RISC-V64-Specific Features

### Hardware Virtualization

When `hw_virt="true"` is configured, the hypervisor uses:

- **H-Extension**: RISC-V Hypervisor Extension provides hardware support for virtualization
- **G-Stage Page Tables**: Guest physical to host physical address translation via `hgatp` register
- **VS-Mode**: Guests run in Virtual Supervisor mode, trapped instructions handled by HS-mode hypervisor
- **Virtual Timer**: `htimedelta` CSR provides time offset for virtual timer in guest
- **SBI Interface**: Supervisor Binary Interface for firmware-level services (timer, IPI, HSM)

### RISC-V CSR Registers

| CSR | Description |
|---|---|
| `hgatp` | Hypervisor Guest Address Translation and Protection (G-stage page table pointer) |
| `hstatus` | Hypervisor Status Register (controls VS-mode behavior) |
| `htimedelta` | Hypervisor Time Delta (virtual timer offset for guest) |
| `hideleg` | Hypervisor Interrupt Delegation (delegates interrupts to VS-mode) |
| `hedeleg` | Hypervisor Exception Delegation (delegates exceptions to VS-mode) |
| `hvip` | Hypervisor Virtual Interrupt Pending (inject virtual interrupts) |
| `hip` / `hie` | Hypervisor Interrupt Pending / Enable |
...
| HS-mode | Hypervisor Supervisor | PRTOS Hypervisor |
| VS-mode | Virtual Supervisor | Guest OS kernel |
| VU-mode | Virtual User | Guest user applications |
...
- [*Embedded Hypervisor: Architecture, Principles, and Implementation*](http://www.prtos.org/embedded_hypervisor_book/) — for the theoretical foundations of RISC-V H-extension virtualization and G-stage address translation
