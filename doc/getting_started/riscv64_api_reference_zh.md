...
- **H 扩展**：RISC-V Hypervisor 扩展提供硬件虚拟化支持
- **G-Stage 页表**：通过 `hgatp` 寄存器实现客户物理地址到宿主物理地址的转换
- **VS 模式**：客户机运行在虚拟 Supervisor 模式，敏感指令由 HS 模式的 Hypervisor 处理
- **虚拟定时器**：`htimedelta` CSR 为客户机提供虚拟定时器时间偏移
- **SBI 接口**：Supervisor Binary Interface 提供固件级服务（定时器、IPI、HSM）

### RISC-V CSR 寄存器

| CSR | 描述 |
|---|---|
| `hgatp` | Hypervisor 客户地址转换和保护（G-stage 页表指针） |
| `hstatus` | Hypervisor 状态寄存器（控制 VS 模式行为） |
| `htimedelta` | Hypervisor 时间增量（客户机虚拟定时器偏移） |
| `hideleg` | Hypervisor 中断委托（将中断委托给 VS 模式） |
| `hedeleg` | Hypervisor 异常委托（将异常委托给 VS 模式） |
| `hvip` | Hypervisor 虚拟中断挂起（注入虚拟中断） |
| `hip` / `hie` | Hypervisor 中断挂起 / 使能 |
| `vsstatus` | 虚拟 Supervisor 状态 |
| `vstvec` | 虚拟 Supervisor 陷阱向量 |
| `vscause` | 虚拟 Supervisor 原因 |

### 特权级别

| 级别 | 模式 | 用途 |
|---|---|---|
| M 模式 | Machine | OpenSBI 固件 |
| HS 模式 | Hypervisor Supervisor | PRTOS Hypervisor |
| VS 模式 | Virtual Supervisor | 客户 OS 内核 |
| VU 模式 | Virtual User | 客户用户应用 |
...
- [《嵌入式 Hypervisor：架构、原理与应用》](http://www.prtos.org/embedded_hypervisor_book/) — 深入了解 RISC-V H 扩展虚拟化和 G-stage 地址转换的理论基础
