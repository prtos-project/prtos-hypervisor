...
LoongArch 定义了 4 个特权级（PLV0..PLV3）和 LVZ 硬件虚拟化扩展。PRTOS 的使用方式如下：

| 模式 | 在 PRTOS 中的角色 |
|---|---|
| Host PLV0 | PRTOS Hypervisor（持有 CSR、TLB-refill 处理程序、异常入口） |
| LVZ Guest 模式（`GSTAT.PVM=1`） | 硬件虚拟化分区（Linux、FreeRTOS）—— 客户机运行在 Guest PLV0，拥有独立的 CSR/TLB 命名空间 |
| Host PLV0（半虚拟化） | 轻量级半虚拟化分区（BAIL 程序）—— 运行在 PLV0，通过协作式 Hypercall 接口与 Hypervisor 交互 |

PLV1、PLV2 和 PLV3 未使用。主要的虚拟化模式是 **LVZ 硬件辅助虚拟化**：客户机在 `GSTAT.PVM=1` 的 Guest 模式下运行，需要 Hypervisor 干预的特权操作会触发 GSPR（Guest Sensitive Privileged Resource）异常，陷入到 Host PLV0 处理。轻量级分区仍支持半虚拟化模式，无需完整的 Guest CSR/TLB 命名空间。
...
其他体系结构 CSR（`EENTRY`、`TLBRENTRY`、`STLBPS`、`PWCL`、`PWCH`、`ASID`、`PGDL`、`PGDH`、`EUEN`、`CRMD` 等）均做了虚拟化处理。在 LVZ 模式下，客户机直接访问硬件提供的 Guest CSR 命名空间（GCSR 寄存器）；在 `GCSRFLAG` 中配置为敏感的 CSR 访问将触发 GSPR 异常，由 Hypervisor 处理。在半虚拟化模式下，客户机读取得到的是每 vCPU 的影子值，写入由 `core/kernel/loongarch64/traps.c` 中的陷入处理程序模拟。

### 陷入入口分发

`core/kernel/loongarch64/entry.S` 中的统一陷入入口在保存寄存器之前先区分三种来源：

1. **LVZ Guest 退出**（`GSTAT.PVM == 1` 时）：跳转到 `_trap_from_lvz_guest`。这是硬件虚拟化分区的主要路径；退出原因（GSPR、Guest TLB 重填、Guest 硬件中断等）从 `ESTAT` 中解码。
2. **半虚拟化 Guest 陷入**（`PRMD.PPLV != 0`）：跳转到 `_trap_from_guest`。用于轻量级半虚拟化分区。
...
> **说明**：在 LoongArch64 上，设置 `hw_virt="true"` 可启用 LVZ 硬件辅助虚拟化。客户机将运行在 LVZ Guest 模式（`GSTAT.PVM=1`），拥有独立的 Guest CSR/TLB 命名空间，定时器通过 `GCFG.TIT` 注入，陷入通过 GSPR 异常处理。未设置 `hw_virt` 或设为 `"false"` 的分区以半虚拟化模式运行在 PLV0，使用协作式 Hypercall。
...
- 《[嵌入式 Hypervisor：架构、原理与应用](http://www.prtos.org/embedded_hypervisor_book/)》 —— 深入了解 LoongArch 上的虚拟化理论基础（涵盖 LVZ 硬件辅助虚拟化与半虚拟化两种方案）
