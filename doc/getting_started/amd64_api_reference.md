# AMD64 (x86_64) Platform — API Reference
...
### Hardware Virtualization (VMX/EPT)

When `hw_virt="true"` is configured, the hypervisor uses:

- **VMX (Virtual Machine Extensions)**: Intel VT-x technology for hardware-assisted virtualization
- **VMCS (Virtual Machine Control Structure)**: Per-vCPU state management, VM-entry/VM-exit control fields
- **EPT (Extended Page Tables)**: Hardware-enforced guest physical to host physical address translation
- **VIOAPIC Emulation**: Virtual I/O APIC for interrupt routing in hardware-virtualized guests
- **x2APIC MSR Virtualization**: Transparent MSR access for local APIC operations
- **VM-Exit Handling**: Intercepts sensitive instructions, I/O port access, MSR access, and CPUID

### AMD64-Specific Constraints

| Constraint | Details |
|---|---|
| EPT Range | Identity-maps first 1GB only; addresses must be < `0x40000000` |
| Reserved IRQs | 2, 4, 24, 26, 27 |
| Reserved I/O Ports | 0x20-0x21 (PIC), 0xA0-0xA1 (PIC), 0x3F8-0x3FC (COM1) |
| ACPI Region | 512KB at 0x3FF80000 |

### Key VMX Registers and Structures

| Component | Description |
|---|---|
| VMCS | Virtual Machine Control Structure (per-vCPU state) |
| EPT Pointer | Base of Extended Page Table hierarchy |
| VM-Entry Controls | Guest state load controls |
| VM-Exit Controls | Host state save/restore controls |
| Pin-based Controls | External interrupt, NMI exiting |
| Primary/Secondary Proc Controls | I/O bitmap, MSR bitmap, EPT enable |
...
| `iommu` | IOMMU-protected (AMD64 specific) |
...
- [*Embedded Hypervisor: Architecture, Principles, and Implementation*](http://www.prtos.org/embedded_hypervisor_book/) — for the theoretical foundations of VMX/EPT virtualization and VMCS design
