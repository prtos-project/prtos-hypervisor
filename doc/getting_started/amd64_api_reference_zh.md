...
- **VMX（虚拟机扩展）**：Intel VT-x 技术提供硬件辅助虚拟化
- **VMCS（虚拟机控制结构）**：每 vCPU 状态管理，VM-entry/VM-exit 控制字段
- **EPT（扩展页表）**：硬件强制的客户物理地址到宿主物理地址转换
- **VIOAPIC 模拟**：虚拟 I/O APIC 用于硬件虚拟化客户机的中断路由
- **x2APIC MSR 虚拟化**：透明的 MSR 访问用于本地 APIC 操作
- **VM-Exit 处理**：拦截敏感指令、I/O 端口访问、MSR 访问和 CPUID

### AMD64 平台约束

| 约束 | 详情 |
|---|---|
| EPT 范围 | 仅恒等映射前 1GB；地址必须 < `0x40000000` |
| 保留 IRQ | 2、4、24、26、27 |
| 保留 I/O 端口 | 0x20-0x21 (PIC)、0xA0-0xA1 (PIC)、0x3F8-0x3FC (COM1) |
| ACPI 区域 | 0x3FF80000 处 512KB |

### 关键 VMX 寄存器和结构

| 组件 | 描述 |
|---|---|
| VMCS | 虚拟机控制结构（每 vCPU 状态） |
| EPT 指针 | 扩展页表层次结构基地址 |
| VM-Entry 控制 | 客户状态加载控制 |
| VM-Exit 控制 | 宿主状态保存/恢复控制 |
| Pin-based 控制 | 外部中断、NMI 退出 |
| Primary/Secondary 处理器控制 | I/O 位图、MSR 位图、EPT 使能 |
...
- [《嵌入式 Hypervisor：架构、原理与应用》](http://www.prtos.org/embedded_hypervisor_book/) — 深入了解 VMX/EPT 虚拟化和 VMCS 设计的理论基础
