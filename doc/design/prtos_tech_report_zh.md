...
在平台适配方面，PRTOS 深度挖掘 ARMv8 (AArch64)、AMD64 (x86_64) 及 RISC-V (RV64) 的硬件辅助虚拟化扩展能力。同时，针对 32 位 x86、LoongArch64 及上述三大 64 位主流平台，PRTOS 均实现了完善的半虚拟化（Para-virtualization）支持，覆盖了当前嵌入式领域的主流处理器架构，为不同工业场景提供了极高的部署灵活性。
...
| LoongArch64 | 半虚拟化（陷入模拟） | PLV0/PLV3，CSR/TLB 陷入 | 客户机管理的页表 |
...
这种组织方式确保了跨架构功能演进不会导致代码分叉，且为未来扩展至 ARMv9 等新平台提供了清晰的可延续性。

---

## 3. 虚拟化技术路线

PRTOS 在 ARMv8 / RISC-V / AMD64 / LoongArch64 平台上提供三种运行模式。LoongArch64 当前仅支持半虚拟化（PLV3 陷入模拟），其余三个 64 位平台同时支持硬件辅助全虚拟化、半虚拟化及混合模式：
...
- **LoongArch64**：采用陷入模拟（trap-and-emulate）的半虚拟化方式。客户机 Linux 内核运行在 PLV3（最低特权级），所有特权 CSR / TLB / 定时器操作均陷入 Hypervisor 进行模拟。`core/kernel/loongarch64/entry.S` 提供统一的陷阱入口，`core/kernel/loongarch64/traps.c` 模拟虚拟 CSR（PWCL/PWCH/STLBPS/EUEN 等）及 TLB-fill 行为，`core/kernel/loongarch64/mmu.c` 实现按分区的内存隔离。该平台不依赖 U-Boot，QEMU 通过 `-kernel` 直接加载 `user/bootloaders/rsw/loongarch64/` 提供的 RSW（Resident Software）引导桩，再由其拉起 Hypervisor。
...
半虚拟化模式的优势在于可实现更精细的调度控制和 I/O 优化。例如，FreeRTOS 半虚拟化示例（`freertos_para_virt_aarch64`、`freertos_para_virt_riscv`、`freertos_para_virt_amd64`、`freertos_para_virt_loongarch64`）通过 Hypercall 直接与 Hypervisor 交互，实现时钟管理、中断路由等功能。
...
在 PRTOS 附带的混合操作系统示例（`mix_os_demo_aarch64`、`mix_os_demo_riscv64`、`mix_os_demo_amd64`、`mix_os_demo_loongarch64`）中，Linux（3 vCPU）处理 HMI 与通信管理，FreeRTOS（1 vCPU，半虚拟化）负责实时电机控制，两者之间通过共享内存进行数据交换，在物理隔离的前提下实现低延迟协作。
...
PRTOS 利用硬件辅助的二级地址转换（EPT/Stage-2/G-stage）实现空间隔离（LoongArch64 通过 PLV3 陷入机制实现等效隔离），结合循环调度机制实现时间隔离。每个分区拥有独立且不可越界的物理地址空间，共享内存区域需在 XML 配置中显式声明。
...
PRTOS 已在四大 64 位平台（AArch64、AMD64、RISC-V、LoongArch64）上实现了基于共享内存的 Virtio 设备虚拟化方案：
...
| Linux 6.19 (陷入模拟) | 半虚拟化 | LoongArch64 | SMP (1-4 vCPU)、Virtio I/O |
| Linux 3.4.4 (半虚拟化内核) | 半虚拟化 | x86 | 遗留系统支持 |
| FreeRTOS | 硬件虚拟化 + 半虚拟化 | AArch64, AMD64, RISC-V | 实时控制任务 |
| FreeRTOS | 半虚拟化 + LVZ shim | LoongArch64 | 实时控制任务 |
...
PRTOS 提供了完整的多平台自动化测试框架，覆盖 32 个测试用例：
...
bash scripts/run_test.sh --arch loongarch64 check-all    # LoongArch64: 16 Pass
```
...
| 示例 | 平台 | 说明 |
|---|---|---|
| `linux_4vcpu_1partion_*` | AArch64/AMD64/RISC-V/LoongArch64 | 4 vCPU Linux SMP |
| `freertos_hw_virt_*` | AArch64/AMD64/RISC-V/LoongArch64 | FreeRTOS 硬件虚拟化 |
| `freertos_para_virt_*` | AArch64/AMD64/RISC-V/LoongArch64 | FreeRTOS 半虚拟化 |
| `mix_os_demo_*` | AArch64/AMD64/RISC-V/LoongArch64 | Linux + FreeRTOS 混合 |
| `virtio_linux_demo_2p_*` | AArch64/AMD64/RISC-V/LoongArch64 | 双 Linux 分区 + Virtio 设备 |
...
PRTOS Hypervisor 已形成一条覆盖 x86/AArch64/AMD64/RISC-V/LoongArch64 五大平台的完整虚拟化技术路线，在四个 64 位平台上同时支持硬件辅助全虚拟化、半虚拟化及混合模式（LoongArch64 当前采用陷入模拟半虚拟化）。其以分离内核为基础的静态分区设计，使之在实时性和确定性方面具有显著优势。结合 Linux 6.19 与 FreeRTOS 的多 Guest OS 验证、Virtio 设备虚拟化方案、以及完善的健康管理与追踪能力，PRTOS 已不再是概念原型，而是具备真实工程部署价值的嵌入式虚拟化平台。