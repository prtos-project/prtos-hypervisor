...
在平台适配方面，PRTOS 深度挖掘 ARMv8 (AArch64)、AMD64 (x86_64)、RISC-V (RV64) 及 LoongArch64 (LVZ) 的硬件辅助虚拟化扩展能力。同时，针对 32 位 x86 及上述四大 64 位主流平台，PRTOS 均实现了完善的半虚拟化（Para-virtualization）支持，覆盖了当前嵌入式领域的主流处理器架构，为不同工业场景提供了极高的部署灵活性。
...
| LoongArch64 | 硬件辅助虚拟化 + 半虚拟化 | LVZ（GCFG/GTLBC/GID） | 基于GID的TLB隔离 |
...
PRTOS 在四个 64 位平台（ARMv8 / RISC-V / AMD64 / LoongArch64）上均提供三种运行模式：硬件辅助全虚拟化、半虚拟化及混合模式：
...
- **LoongArch64**：基于 LVZ（龙芯虚拟化扩展）硬件扩展，通过 GCFG 配置客户机 CSR/TLB/定时器陷入，并利用基于 GID 的 TLB 隔离实现二级地址转换。`core/kernel/loongarch64/entry.S` 提供统一的陷阱入口，`core/kernel/loongarch64/traps.c` 处理客户机 CSR 模拟（GCFG/GTLBC/GSTAT 等）及 VM-exit 处理，`core/kernel/loongarch64/mmu.c` 通过基于 GID 的 TLB 标记实现按分区的内存隔离。PRTOS 同时保留了半虚拟化路径，客户机运行在 PLV3 并通过陷入模拟语义进行管理。该平台不依赖 U-Boot，QEMU 通过 `-kernel` 直接加载 `user/bootloaders/rsw/loongarch64/` 提供的 RSW（Resident Software）引导桩，再由其拉起 Hypervisor。
...
PRTOS 利用硬件辅助的二级地址转换（EPT/Stage-2/G-stage/基于GID的TLB隔离）在四个 64 位平台上实现空间隔离，结合循环调度机制实现时间隔离。每个分区拥有独立且不可越界的物理地址空间，共享内存区域需在 XML 配置中显式声明。
...
PRTOS Hypervisor 已形成一条覆盖 x86/AArch64/AMD64/RISC-V/LoongArch64 五大平台的完整虚拟化技术路线，在四个 64 位平台上同时支持硬件辅助全虚拟化、半虚拟化及混合模式。其以分离内核为基础的静态分区设计，使之在实时性和确定性方面具有显著优势。结合 Linux 6.19 与 FreeRTOS 的多 Guest OS 验证、Virtio 设备虚拟化方案、以及完善的健康管理与追踪能力，PRTOS 已不再是概念原型，而是具备真实工程部署价值的嵌入式虚拟化平台。